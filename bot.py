            ...
            for coin in coins:
                try:
                    ...
                    if (is_in_zone and zone_type == "demand") or (is_in_fvg and fvg and fvg['type'] == "bullish"):
                        if ob_delta < CONFLUENCE_CONFIG["min_ob_delta_long"]:  # FIX: pakai config, bukan hardcode -20
                            continue
...
🔥 R:R = 1:{rr_conf:.1f}"""
                    ...
                    if (is_in_zone and zone_type == "supply") or (is_in_fvg and fvg and fvg['type'] == "bearish"):
                        if ob_delta > CONFLUENCE_CONFIG["min_ob_delta_short"]:  # FIX: pakai config, bukan hardcode +20
                            continue
    ...
    while True:
        try:
            ...
            if regime != "RANGING":
                ...
                logger.debug("[SQUEEZE_ALERT] Skip — regime RANGING")
...
def check_smc_alert():
    ...
    try:
        ...
        for coin in top_coins:
            with state_lock:
                last_alert_time = _smc_alert_last.get(coin, 0)
