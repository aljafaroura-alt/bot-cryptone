...
def is_market_chaos(symbol, chaos_pct=1.5):
    """Return True kalo market chaos. chaos_pct bisa diatur sesuai mode"""
    try:
        end_time = int(__import__('time').time() * 1000)
        start_time = end_time - (60 * 1000)
        candles = info.candles_snapshot(symbol, "1m", start_time, end_time)
        if not candles: return True
        open_price = float(candles[-1]['o'])
        close_price = float(candles[-1]['c'])
        change_pct = abs((close_price - open_price) / open_price * 100) if open_price > 0 else 0
    ...
    while True:
        try:
            ...
            if SNIPER_ALL_COIN:
                ...
                coins = list(all_mids.keys())[:80]  # FIX: HL ga pake -PERP suffix
                print(f"Update list: {len(coins)} perps Hyperliquid")

                for coin in coins:
                    symbol = coin  # FIX: coin udah bener dari all_mids
                    ...
                    try:
                        ...
                        funding = get_funding_pct(ctx)
                        price = float(all_mids.get(symbol, 0))  # FIX: ambil harga koin yang bener
