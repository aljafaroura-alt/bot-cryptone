...
def get_correlation_matrix(force_refresh: bool = False, use_cache: bool = True) -> dict:
    ...
    try:
        ...
        # Fetch candles — serial dengan throttle, bukan parallel blast 35 req sekaligus
        # Pakai cache dulu kalau ada, baru fetch yang belum ada
        candles_by_coin = {}
        batch_size = 5   # fetch 5 coins per batch, delay 0.3s antar batch

        for i in range(0, len(selected_coins), batch_size):
            batch = selected_coins[i:i + batch_size]
            ...
            # Jeda antar batch biar rate limiter gak meledak
            if i + batch_size < len(selected_coins):
                time.sleep(0.4)
        ...
        if alerts:
            ...
            for a in alerts[:5]:
                ...
                teks = (
                    ...
                    f"\u2705 TP: {md_escape(fmt_price(a['tp']))} (+{md_safe(a['tp_pct'], '.2f')}%)\n"
...
def smc_full_analysis(coin):
    ...
    OPT: Parallel fetch untuk semua timeframe (3x lebih cepat).
    Returns ringkasan alignment dan level entry
    """
    timeframes = ["4h", "1h", "15m", "5m"]
    results = {}

    # Parallel fetch menggunakan ThreadPoolExecutor
    with concurrent.futures.ThreadPoolExecutor(max_workers=len(timeframes)) as ex:
        futures = {ex.submit(analyze_tf, coin, tf): tf for tf in timeframes}
        for f in concurrent.futures.as_completed(futures):
            tf = futures[f]
