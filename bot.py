...
def api_call_with_retry(func, *args, max_retries=3, delay=1, **kwargs):
    """Wrapper retry + token bucket rate limiting untuk Hyperliquid."""
...
def md_escape(text: str) -> str:
    """
    Escape karakter spesial MarkdownV2 Telegram dari string dinamis
    (coin name, price, trendline label, dsb).
    Gunakan ini untuk semua data yang datang dari market/API sebelum
    dimasukkan ke template pesan Markdown.
    """
...
def md_safe(value, fmt=None) -> str:
    """Format angka/nilai lalu escape. Contoh: md_safe(price, '.2f')"""
    if fmt:
        try:
...
def check_squeeze_alert():
    ...
    try:
        ...
        for coin in top_coins:
            if coin in _squeeze_alert_last and now_time - _squeeze_alert_last[coin] < 2700:  # 45 min cooldown
                continue
            ...
            try:
                ...
                if squeeze_trigger_score < 20:
                    logger.debug(f"[SQUEEZE_ALERT] {coin} skip — trigger score {squeeze_trigger_score}/45 too low")
                    continue
...
🚨 *AUTO ALERTS (TOGGLE)*
/warroomalert on/off — Warroom (≥60)
/entryalert on/off — Entry (≥70)
/squeezealert on/off — Squeeze (≥55)
/smcalert on/off — SMC (≥60%)
...
def master_market_scan(force: bool = False) -> dict:
    ...
    with _master_scan_lock:
        ...
        # Set penanda sedang scanning agar thread lain tidak double-fetch
        _master_scan_last = now  # optimistic lock
            ...
            for direction in ["LONG", "SHORT"]:
                ...
                try:
                    ...
                    if is_sector_conflict(coin, direction):
                        ...
                        continue
                    # Cross validation untuk SMC → bonus kalau ada, penalti kalau sendirian
                    cross_bonus_smc = 0
                    if has_cross_validation(coin, direction, min_scanners=2):
                        ...
                        cross_bonus_smc = -10  # penalti kecil, bukan skip
                    ...
                    if cross_bonus_smc > 0:
                        ...
                        logger.debug(f"[SMC_ALERT] {coin} cross_val solo penalti {cross_bonus_smc}")
...
def status_cmd(message):
    ...
    # ========== BUILD OUTPUT ==========
    teks = f"""⚠️ *SYSTEM STATUS* ─ {get_wib()}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
def sniper_status_cmd(message):
    ...
    if SNIPER_ALL_COIN and _sniper_auto_state == "auto_on":
        ...
        auto_info = "\n✋ *Manual off* — Kamu matiin sendiri, auto tidak akan nyala"

    teks = f"""🕶️ *SNIPER STATUS*
━━━━━━━━━━━━━━━━━━━━━━
Status      : {status}
...
def aggro_mode_toggle(message):
    ...
    if parts[1].lower() == "on":
        ...
        bot.reply_to(message, "✅ *AGGRESSIVE MODE ON*\nFilter entry dilonggarkan, lebih banyak sinyal di ASIA session.", parse_mode='Markdown')
    elif parts[1].lower() == "off":
        _AGGRESSIVE_MODE = False
        bot.reply_to(message, "❌ *AGGRESSIVE MODE OFF*\nKembali ke filter normal.", parse_mode='Markdown')
    else:
...
def correlation_matrix_cmd(message):
    ...
    try:
        ...
        teks = f"📊 *CORRELATION MATRIX*\n"
        teks += f"━━━━━━━━━━━━━━━━━━━━━━━━\n"
        teks += f"📡 `{coin}` vs {len(correlations)} coins\n"
        teks += f"⏰ {get_wib()}\n"
        teks += f"📈 Lookback: `{matrix_data.get('lookback', 50)}` candles (5m)\n"
        teks += f"🎯 Regime: `{matrix_data.get('market_regime', 'UNKNOWN')}`\n"
        teks += f"━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
