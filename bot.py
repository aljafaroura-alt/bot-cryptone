...
def get_smc_levels_advanced(coin, direction="LONG"):
    ...
    try:
        ...
        for tf_candles, tf_name in [(candles_15m, "15m"), (candles_1h, "1h"), (candles_4h, "4h")]:
            ...
            ob = find_ob_zone(tf_candles, ob_bias, max_distance_pct=2.5)
            if ob:
                zone = ob
                zone_type = f"OB ({tf_name})"
                zone_tf = tf_name
                break
            # FIX BUG3: Pass fvg_type filter langsung ke find_fvg_smc
            # Sebelumnya: return FVG apapun → cek arah → kalau salah, skip TF ini
            # Sekarang: langsung cari FVG yang arahnya sesuai di TF yang sama
