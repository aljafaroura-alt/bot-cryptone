...
def api_call_with_retry(func, *args, max_retries=3, delay=1, **kwargs):
    """Wrapper retry untuk API calls yang rawan gagal."""
        ...
        try:
            ...
            if int(now) % 3600 < 5:  # Setiap ~1 jam
                ...
                with state_lock:
                    ...
                    for k in stale_ema:
                        del _ob_delta_ema[k]
                if stale_dyn or stale_ema:
                    logger.debug(f"[PRUNE] Removed {len(stale_dyn)} dyn_mult + {len(stale_ema)} ob_ema cache entries")
