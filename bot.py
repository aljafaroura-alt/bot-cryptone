...
def positions(message):
    try:
        parts = message.text.split()
        if len(parts) < 2:
            return bot.reply_to(message, "❌ Format: /positions 0xWallet")
        wallet = parts[1]
...
def pnl(message):
    try:
        parts = message.text.split()
        if len(parts) < 2:
            return bot.reply_to(message, "❌ Format: /pnl 0xWallet")
        wallet = parts[1]
        state = info.user_state(wallet)
...
def entrywhale(message):
    try:
        msg = bot.reply_to(message, "🐋 Scanning whale entry live...")
        meta_ctxs = info.meta_and_asset_ctxs()
        coins_meta = meta_ctxs[0]['universe']
        coins_data = meta_ctxs[1]
        whale_entries = []
        ...
        if not whale_entries:
            txt = f"🐋 WHALE SNIPER\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n😴 Ga ada whale entry >$30k dalam 3 menit terakhir.\n\n⏰ {get_wib()}"
            return bot.edit_message_text(txt, msg.chat.id, msg.message_id)
        whale_entries.sort(key=lambda x: x['size'], reverse=True)
        txt = f"🐋 WHALE SNIPER\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
        for w in whale_entries[:5]:
            txt += f"{w['emoji']} {w['side']} {w['coin']}\n"
            txt += f"   Size ${w['size']:,.0f} | Price {fmt_price(w['price'])}\n"
            txt += f"   ⏱️ {w['time']}s ago\n\n"
        txt += f"⏰ {get_wib()}"
        bot.edit_message_text(txt, msg.chat.id, msg.message_id)
    except Exception as e:
        bot.edit_message_text(f"❌ Error: {str(e)[:100]}", msg.chat.id, msg.message_id)
