...
def temen_on(message):
    ...
    bot.reply_to(
        ...
        "Kalo ada koin gerak dikit = gw chat\n"
        "Threshold: Delta 5% | Wall $20k | Harga 0.8%\n"
        "Cooldown 5m/koin biar ga spam banget\n"
...
def temen_off(message):
    ...
    bot.reply_to(
        message,
        "🤐 Oke gw diem dulu... Chat lagi /temen kalo kangen"
    )
...
def run_temen_scan(chat_id):
    """Scan market dan bacot kalo ada yang gerak"""
    ...
    try:
        ...
        for asset, ctx in zip(data[0]["universe"], data[1]):
            try:
                ...
                try:
                    l2 = info.l2_snapshot(coin)
                    bids = sum(float(b['sz'])*float(b['px']) for b in l2['levels'][0][:3])
                    asks = sum(float(a['sz'])*float(a['px']) for a in l2['levels'][1][:3])
                    total = bids + asks
                    ob_delta = (bids-asks)/total*100 if total > 0 else 0
                except: pass
                triggered = []
                if abs(ob_delta) > 5:
                    direction = "BID DOM 💚" if ob_delta > 0 else "ASK DOM ❤️"
                    triggered.append(f"OB Delta {ob_delta:+.0f}% {direction}")
                if abs(change) > 0.8:
                    arrow = "🚀" if change > 0 else "📉"
                    triggered.append(f"Harga {change:+.1f}% {arrow}")
                if abs(funding) > 0.02:
                    triggered.append(f"Funding {funding:.4f}%")
                if triggered:
                    alerts.append((coin, mark, triggered, abs(change) + abs(ob_delta)))
                    TEMEN_COOLDOWN[coin] = now
            except: continue

        if not alerts:
            bot.send_message(chat_id, f"😴 <b>TEMEN UPDATE</b> | {get_wib()}\nMarket lagi sepi, ga ada yang gerak. Santuy dulu.", parse_mode='HTML')
            return

        alerts.sort(key=lambda x: x[3], reverse=True)
        teks = f"👥 <b>TEMEN BACOT</b> | {get_wib()}\n━━━━━━━━━━━━━━━━━━━━━━━\n"
        for coin, mark, triggered, _ in alerts[:5]:
            teks += f"\n<b>{coin}</b> {fmt_price(mark)}\n"
            for t in triggered:
                teks += f"  • {t}\n"
        teks += f"\n━━━━━━━━━━━━━━━━━━━━━━━\n"
        teks += f"🎯 /warroom {alerts[0][0]} buat detail"
...
def temen_on(message):
    ...
    bot.reply_to(message,
        ...
        "Udah nyala bro, gw bakal bacot tiap 5 menit\n"
        "Kalo ada koin gerak dikit = gw chat\n"
        "Threshold: Delta 5% | Harga 0.8% | Fund 0.02%\n"
        "Cooldown 5m/koin biar ga spam banget\n"
        "Ketik /diem buat nyuruh gw diem 🤐",
        parse_mode='HTML')
...
def set_schedule(message):
    ...
    try:
        ...
        if mode == 'insane':
            ...
            bot.reply_to(message, "❌ Mode ga ada. Pake: <b>insane</b> atau <b>temen</b>", parse_mode='HTML')
