...
def rate_limited_api_call(func, *args, **kwargs):
    """Wrapper untuk throttle semua Hyperliquid API call — min 200ms antar request."""
...
# ========== PUBLIC RATE LIMITER (group/channel) ==========
# Berlaku untuk non-owner yang pakai heavy commands di grup/channel
_pub_cooldown_ts = {}     # {user_id_cmd: last_timestamp}
_pub_daily_count = {}     # {user_id_cmd_date: count}

PUBLIC_LIMIT_CMDS = {
    # cmd : (cooldown_detik, daily_quota)
    "entry":   (300, 5),   # 5 menit cooldown, max 5x/hari
    "smc":     (300, 5),
    "warroom": (300, 5),
}

def _pub_date_key(user_id: int, cmd: str) -> str:
    """Key harian per user per command (reset tiap hari WIB)."""
    today = datetime.now(WIB).strftime("%Y%m%d")
    return f"{user_id}_{cmd}_{today}"

def check_public_limit(message, cmd: str):
    """
    Cek cooldown + daily quota untuk publik di grup/channel.
    Return (blocked: bool, reason_msg: str | None)
    - blocked=True  -> tolak, kirim reason_msg ke user
    - blocked=False -> lanjut
    Owner (USER_ID) selalu lolos.
    """
    ...
    # Hanya limit di grup/channel
    chat_type = message.chat.type
    if chat_type not in ("group", "supergroup", "channel"):
        return False, None

    if cmd not in PUBLIC_LIMIT_CMDS:
        return False, None

    cooldown_sec, daily_max = PUBLIC_LIMIT_CMDS[cmd]
    now = time.time()
    cd_key = f"{user_id}_{cmd}"
    day_key = _pub_date_key(user_id, cmd)

    with state_lock:
        # --- Cooldown check ---
        last_ts = _pub_cooldown_ts.get(cd_key, 0)
        elapsed = now - last_ts
        if elapsed < cooldown_sec:
            sisa_menit = int((cooldown_sec - elapsed) / 60) + 1
            msg = (
                f"\u23f3 *Cooldown aktif!*\n"
                f"Command `/{cmd}` bisa dipakai lagi dalam *{sisa_menit} menit*.\n"
                f"_(Limit publik: 1x per {cooldown_sec//60} menit)_"
            )
            return True, msg

        # --- Daily quota check ---
        used = _pub_daily_count.get(day_key, 0)
        if used >= daily_max:
            msg = (
                f"\U0001f6ab *Kuota harian habis!*\n"
                f"Kamu sudah pakai `/{cmd}` sebanyak *{used}x* hari ini (maks {daily_max}x).\n"
                f"_(Reset tiap jam 00:00 WIB)_"
            )
            return True, msg

        # Lolos -- update state
        _pub_cooldown_ts[cd_key] = now
        _pub_daily_count[day_key] = used + 1
...
def get_dynamic_thresholds(coin: str, direction: str, alert_type: str) -> dict:
    ...
    if alert_type == "entry":
        base_score = 40 + (noise_factor * 15); base_rr = 1.0 + (noise_factor * 0.3); need_align = 3 if noise_factor > 1.3 else 2
    elif alert_type == "smc":
        base_score = 45 + (noise_factor * 12); base_rr = 1.2 + (noise_factor * 0.25); need_align = 2
    elif alert_type == "squeeze":
        base_score = 35 + (noise_factor * 10); base_rr = 0.8 + (noise_factor * 0.2); need_align = 1
    else:
        base_score = 40 + (noise_factor * 13); base_rr = 1.0 + (noise_factor * 0.28); need_align = 2
    if regime == "PANIC":
        regime_mult = 1.8
    ...
    return {
        "min_score": max(35, min(95, int(base_score * final_mult))),
...
🎯 /warroom {result['coin']}"""
            ...
            time.sleep(get_dynamic_liq_config()["scan_interval"])
...
def get_divergence_stack_score(coin: str, direction: str) -> tuple:
    ...
    try:
        ...
        try:
            oi_pct, is_oi, oi_dir = oi_impulse(coin)
            if is_oi and oi_dir == direction:
                # OI naik + arah sama = smart money masuk
                oi_bonus = min(20, int(oi_pct * 0.8))
                score += oi_bonus
                confirmations += 1
    ...
    try:
        ...
        for coin in top_coins:
            ...
            try:
                ...
                # ============================================================
                # MANUAL FINGERPRINT MATCH BOOST
                # ============================================================
                try:
                    fp_score, fp_label = score_manual_fingerprint_match(coin, deriv_bias)
                    if fp_score > 0:
                        trendline_bonus += fp_score
                        trendline_tags.append(fp_label)
                        logger.info(f"[ENTRY_ALERT] {coin} fingerprint match: {fp_label} +{fp_score}")
                except Exception:
                    pass
...
def entry(message):
    try:
        blocked, reason = check_public_limit(message, "entry")
        if blocked:
...
# ========== ADMIN: SET PUBLIC LIMIT ==========

@bot.message_handler(commands=['setlimit'])
def cmd_set_limit(message):
    """
    Owner-only: ubah cooldown/quota publik on-the-fly.
    Usage: /setlimit <cmd> <cooldown_menit> <daily_quota>
    Contoh: /setlimit entry 10 3
    """
    if not is_owner(message):
        return
    parts = message.text.split()
    if len(parts) != 4:
        lines = [f"`{cmd}`: {cd//60}m cooldown, {dq}x/hari"
                 for cmd, (cd, dq) in PUBLIC_LIMIT_CMDS.items()]
        bot.reply_to(message,
            "⚙️ *Public Limit Config*\n\n" + "\n".join(lines) +
            "\n\nUbah: `/setlimit <cmd> <menit> <quota>`\nContoh: `/setlimit entry 10 3`",
            parse_mode="Markdown")
        return
    cmd_name = parts[1].lower()
    if cmd_name not in PUBLIC_LIMIT_CMDS:
        bot.reply_to(message,
            f"❌ Command `{cmd_name}` tidak ada di limit list.\n"
            f"Tersedia: {', '.join(PUBLIC_LIMIT_CMDS.keys())}",
            parse_mode="Markdown")
        return
    try:
        cd_min = int(parts[2])
        quota  = int(parts[3])
        if cd_min < 1 or quota < 1:
            raise ValueError
    except ValueError:
        bot.reply_to(message, "❌ Format salah. Menit dan quota harus angka positif.")
        return
    PUBLIC_LIMIT_CMDS[cmd_name] = (cd_min * 60, quota)
    bot.reply_to(message,
        f"✅ Limit `/{cmd_name}` diupdate:\n"
        f"• Cooldown: *{cd_min} menit*\n"
        f"• Daily quota: *{quota}x/hari*",
        parse_mode="Markdown")
    logger.info(f"[SETLIMIT] owner updated /{cmd_name}: {cd_min}m cooldown, {quota}x/day")


@bot.message_handler(commands=['limitstatus'])
def cmd_limit_status(message):
    """Owner-only: lihat status limit publik semua command + usage hari ini."""
    if not is_owner(message):
        return
    today = datetime.now(WIB).strftime("%Y%m%d")
    teks = "📊 *Public Rate Limiter Status*\n\n"
    with state_lock:
        for cmd_name, (cd_sec, dq) in PUBLIC_LIMIT_CMDS.items():
            # Hitung total usage hari ini semua user untuk command ini
            prefix = f"_{cmd_name}_{today}"
            total_today = sum(v for k, v in _pub_daily_count.items() if k.endswith(prefix))
            # Hitung berapa user yang sedang dalam cooldown
            now = time.time()
            in_cd = sum(1 for k, ts in _pub_cooldown_ts.items()
                        if k.endswith(f"_{cmd_name}") and now - ts < cd_sec)
            teks += (
                f"🔹 `/{cmd_name}`\n"
                f"   Cooldown: {cd_sec//60}m | Quota: {dq}x/hari\n"
                f"   Usage hari ini: {total_today}x | Sedang cooldown: {in_cd} user\n\n"
            )
...
def smc_command(message):
    try:
        blocked, reason = check_public_limit(message, "smc")
        if blocked:
            bot.reply_to(message, reason, parse_mode="Markdown")
            return
            ...
            for direction in ["LONG", "SHORT"]:
                ...
                try:
                    ...
                    try:
                        ...
                        if div_stack_saa > 0:
                            confidence = min(99, confidence + min(20, div_stack_saa // 2))
...
def warroom(message):
    try:
        blocked, reason = check_public_limit(message, "warroom")
        if blocked:
            bot.reply_to(message, reason, parse_mode="Markdown")
            return
...
# ============================================================
# MANUAL TRADE LOGGER — Telegram Commands
# ============================================================
...
def cmd_log_trade(message):
    ...
    try:
        ...
        bot.reply_to(message, "❌ Harga harus angka. Contoh: `/log LONG BTC 105000`", parse_mode="Markdown")
    except Exception as e:
        bot.reply_to(message, f"❌ Error: {e}")
