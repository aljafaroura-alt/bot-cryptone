...
USER_ID     = 8347576377        # ID lu (owner)
CHANNEL_ID  = -1003898060549    # @oncryptone
ALLOWED_USERS = [USER_ID]       # Tambahin ID lain kalo mau
...
_signal_pending = {}  # {key: {coin, direction, entry_price, entry_time, indicators, session}}
...
# Volatility profile per coin untuk dynamic SL/TP
VOLATILITY_PROFILE = {
    "low":    ["BTC", "ETH"],                                          # SL 0.8% TP 1.6%
    "medium": ["SOL", "BNB", "AVAX", "ARB", "OP", "MATIC", "SUI",
               "APT", "INJ", "TIA", "NEAR", "TON", "ADA", "XRP",
               "LINK", "DOT", "ATOM", "LDO", "AAVE", "UNI"],          # SL 1.2% TP 2.4%
    "high":   ["DOGE", "SHIB", "PEPE", "FLOKI", "BONK", "WIF",
               "POPCAT", "BOME", "MEW", "NEIRO", "TURBO", "BRETT",
               "MOODENG", "PNUT", "GOAT", "FWOG", "MOG"],             # SL 2.0% TP 4.0%
}

def get_volatility_params(coin):
    """Return (sl_pct, tp_pct) berdasarkan volatilitas coin"""
    coin = coin.upper()
    ...
    try:
        ...
        for asset, ctx in zip(data[0]["universe"], data[1]):
            try:
                ...
                oi_prev = _narrative_oi_history.get(prev_key, oi_usd)
                ...
                if abs(oi_change) > 1:  # hanya simpan kalo gerak signifikan
                    narrative_stats[narrative]["oi_changes"].append(oi_change)
                
                # Update history
                _narrative_oi_history[prev_key] = oi_usd
                
            except Exception as e:
                continue
        
        # Hitung rata-rata perubahan per narrative
        result = {}
        for narrative, stats in narrative_stats.items():
            ...
            # Cari top coin di narrative ini (berdasarkan OI)
            result[narrative] = {
        ...
        print(f"Narrative flow error: {e}")
        return {}
        
def check_smart_money_rotation():
    """Deteksi rotasi antar narrative"""
    try:
        ...
        # Cek narrative dengan perubahan ekstrem
        ...
        # Kirim alert yang unik (cooldown biar ga spam)
        now = time.time()
        for alert in alerts:
...
def get_bid_wall_level(coin):
    """Return (wall_usd, wall_price) — level bid wall terbesar di top 10"""
    try:
        l2 = info.l2_snapshot(coin)
        best = max(l2['levels'][0][:10], key=lambda b: float(b['sz'])*float(b['px']))
        wall_px = float(best['px'])
        wall_usd = float(best['sz']) * wall_px
        return wall_usd, wall_px
    except:
        return 0, 0

def get_ask_wall_level(coin):
    """Return (wall_usd, wall_price) — level ask wall terbesar di top 10"""
    try:
        l2 = info.l2_snapshot(coin)
...
def get_ob_delta(coin):
    ...
    try:
        ...
        # Minimum liquidity check — kalau salah satu sisi < $5k, data ga valid
        if bids < 5000 or asks < 5000: return 0
        delta = (bids - asks) / (bids + asks) * 100
        # Cap di ±60% — lebih dari itu anomali/data jelek
...
def get_trade_history(wallet, limit=20):
    ...
    try:
        ...
        # Ambil user fills via Hyperliquid public API (FIX: tambah startTime yang wajib)
        url = "https://api.hyperliquid.xyz/info"
        start_time_ms = int((time.time() - 7 * 24 * 3600) * 1000)  # 7 hari terakhir
        payload = {
...
def sniper_aggro(message):
    ...
    text += f"Scan semua coin Hyperliquid:\n"
    text += f"🛡️ Bid Wall > ${cfg['wall_min']/1000:.0f}k\n"
    text += f"📡 OB Delta > +{cfg['delta_min']}%\n"
    text += f"💰 Funding ≤ {cfg['funding_max']}%\n"
    text += f"⏱️ Cooldown: {cfg['cooldown']//60} menit/koin\n"
    text += f"─────────────────────────────────\n"
    text += f"✅ Semua coin aktif dipantau\n"
    text += f"🔔 Notif per coin (BTC, ETH, SOL, dll)\n"
    text += f"Spam oke — tiap coin punya cooldown sendiri"
    bot.send_message(message.chat.id, text, reply_markup=markup)
...
def sniper_insane(message):
    ...
    text += f"Filter ketat, sinyal paling kuat:\n"
    text += f"🛡️ Bid Wall > ${cfg['wall_min']/1000:.0f}k\n"
    text += f"📡 OB Delta > +{cfg['delta_min']}%\n"
    text += f"💰 Funding ≤ {cfg['funding_max']}%\n"
    text += f"⏱️ Cooldown: {cfg['cooldown']//60} menit/koin\n"
    ...
    try:
        ...
        try:
            ...
            if len(c1h) >= 21:
                ...
                def _ema(px, n):
                    k = 2 / (n + 1); e = px[0]
                    for p in px[1:]: e = p * k + e * (1 - k)
                    return e

                ema8  = _ema(closes, 8)
                ema21 = _ema(closes, 21)
                if ema8 > ema21 * 1.002:   trend_signal = "BULLISH_TREND"
                elif ema8 < ema21 * 0.998: trend_signal = "BEARISH_TREND"

                if len(vols) >= 6:
                    rv = sum(vols[-3:]) / 3
                    pv = sum(vols[-6:-3]) / 3
                    if rv > pv * 1.3:   vol_trend = "INCREASING"
                    elif rv < pv * 0.7: vol_trend = "DECREASING"
        except Exception:
            pass
        ...
        if bull > bear + 2:
            ...
            if vol_trend == "INCREASING": reason += ", volume naik 🔥"
        elif bear > bull + 2:
            direction  = "bearish"
            target     = mark * (1 - 0.01 - align * 0.003)
            confidence = base_conf
            reason = (f"EMA {trend_signal.replace('_',' ')}, funding {funding:+.3f}%, OB {ob_delta:+.0f}%")
            if vol_trend == "INCREASING": reason += ", distribusi volume ⚠️"
        else:
            direction  = "sideways"
            target     = mark
            confidence = 62
...
def run_liquidation_scanner():
    ...
    while True:
        try:
            # PERF FIX: Fetch meta sekali untuk semua coin — hemat ~60 API calls per scan
...
def find_demand_zone(coin):
    ...
    for i in range(len(candles)-1, 5, -1):
        ...
        # Cara 2: Hammer / long lower wick (FIX: cast ke float)
...
def find_supply_zone(coin):
    ...
    for i in range(len(candles)-1, 5, -1):
        ...
        # Cara 2: Shooting star / long upper wick (FIX: cast ke float)
        body = abs(float(c['c']) - float(c['o']))
    ...
    try:
        if os.path.exists(LEARNING_FILE):
            with open(LEARNING_FILE, 'r') as f:
                ...
                print(f"[LEARNING] Loaded weights={LEARNING_WEIGHTS}, outcomes={len(SIGNAL_OUTCOMES_HISTORY)}")
    except Exception as e:
...
def track_signal_entry(coin, direction, entry_price, indicators):
    ...
    h = get_wib_hour()
    if 8 <= h < 15:        session = "ASIA"
    elif 15 <= h < 20:     session = "LONDON"
    elif 20 <= h or h < 5: session = "NY"
    else:                  session = "OFF"
    ...
    if len(_signal_pending) > 200:
        oldest = sorted(_signal_pending.keys(),
                        key=lambda k: _signal_pending[k]["entry_time"])
        for k in oldest[:50]:
...
def evaluate_signal_outcomes():
    ...
    if new_outcomes > 0:
        ...
        save_learning_data()
        print(f"[LEARNING] Evaluated {new_outcomes} new signals")
...
def _update_learning_weights(recent_outcomes):
    ...
    print(f"[LEARNING] Weights updated: {LEARNING_WEIGHTS}")

# ========== MARKET REGIME DETECTOR ==========

def get_market_regime():
    """
    Deteksi market regime dari BTC 4H candles.
    Returns: TRENDING_UP | TRENDING_DOWN | VOLATILE | RANGING | UNKNOWN
    Cache 15 menit biar hemat API.
    """
    ...
    try:
        ...
        # Volatility: rata-rata range 5 candle terakhir
        ranges = []
        for c in candles[-5:]:
            h, lv = float(c['h']), float(c['l'])
            mid   = (h + lv) / 2
            if mid > 0: ranges.append((h - lv) / mid * 100)
...
def get_atr(coin, period=14, timeframe="15m"):
    """Hitung Average True Range untuk dynamic SL/TP yang lebih adaptif"""
    try:
        end_ms   = int(time.time() * 1000)
        mins_per = 15 if timeframe == "15m" else 60
        start_ms = end_ms - (period * 3 * mins_per * 60 * 1000)
...
def run_scheduler():
    ...
    last_learning_eval     = 0  # NEW: learning engine eval tiap 2 jam
    ...
    while True:
        try:
            ...
            # ========== 5. LEARNING EVALUATION (tiap 2 jam) ==========
            if now - last_learning_eval >= 7200:
                evaluate_signal_outcomes()
                last_learning_eval = now

            # ========== 6. SMART MONEY FLOW (ADAPTIF) ==========
...
def learning_stat_cmd(message):
    ...
    try:
        ...
        teks += f"━━━━━━━━━━━━━━━━━━━━━━\n"
        teks += f"⏰ {get_wib()}\n\n"
        teks += f"📊 Total signals tracked : {total}\n"
        teks += f"✅ Akurasi keseluruhan   : {acc:.0f}%\n"
        teks += f"⏳ Pending evaluasi      : {pending}\n\n"
        teks += f"⚖️ LEARNING WEIGHTS:\n"
        teks += f"   💰 Funding   : {LEARNING_WEIGHTS['funding']:.2f}x\n"
        teks += f"   📡 OB Delta  : {LEARNING_WEIGHTS['ob_delta']:.2f}x\n"
        teks += f"   🐋 Wall      : {LEARNING_WEIGHTS['wall']:.2f}x\n"
        teks += f"   💀 Liquidity : {LEARNING_WEIGHTS['liquidity']:.2f}x\n\n"
        teks += f"📈 WIN RATE PER SESSION (30 terbaru):\n"
...
def regime_cmd(message):
    """Lihat market regime sekarang + adaptive sniper config preview"""
    try:
        ...
        teks += f"💡 Advice:\n{advice}\n\n"
        teks += f"⚙️ ADAPTIVE SNIPER [{SNIPER_MODE}]:\n"
        teks += f"   Wall min : ${base['wall_min']//1000}k → ${cfg['wall_min']//1000}k\n"
        teks += f"   OB Delta : {base['delta_min']}% → {cfg['delta_min']}%\n"
        teks += f"   Cooldown : {base['cooldown']//60}m → {cfg['cooldown']//60}m\n"
...
if __name__ == "__main__":
    ...
    # Load learning state — jaga weights antar restart
    load_learning_data()
    ...
    print("🤖 HL Terminal Bot MONSTER v15 - ONLINE")
    while True:
        try:
