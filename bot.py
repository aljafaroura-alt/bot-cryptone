...
def get_smc_levels_advanced(coin, direction="LONG"):
    ...
    try:
        ...
        confidence = 55
        if zone_tf == "15m":
            confidence += 5   # 15m lebih noise dari 1H/4H
        elif zone_tf == "1h":
            confidence += 10
        elif zone_tf == "4h":
            confidence += 15
        ...
        if ctx:
            ...
            if direction == "LONG" and ob_delta > 10:
                ...
                confidence += 10
            elif direction == "LONG" and ob_delta < -10:
                confidence -= 15  # FIX: OB contra LONG → penalty
            elif direction == "SHORT" and ob_delta > 10:
                confidence -= 15  # FIX: OB contra SHORT → penalty
            # ob_delta ~0 = tidak ada tambahan dan tidak ada penalty
            ...
            if direction == "LONG" and funding < -0.02:
                ...
                confidence -= 10  # funding sangat positif = shorts udah overcrowded, tapi longs mau masuk? risky
            elif direction == "SHORT" and funding < -0.05:
                confidence -= 10

            # FIX: Momentum penalty — kalau harga lagi kencang berlawanan arah, confidence turun
            try:
                change_pct = get_change(ctx)
                if direction == "SHORT" and change_pct > 3.0:
                    confidence -= 10  # harga lagi naik kencang tapi mau short → risky
                elif direction == "LONG" and change_pct < -3.0:
                    confidence -= 10  # harga lagi turun kencang tapi mau long → risky
            except Exception:
                pass
