        ...
        for coin in top_coins:
            ...
            try:
                ...
                if aligned >= need_align and ((dominant == "BULLISH" and deriv_bias == "LONG") or (dominant == "BEARISH" and deriv_bias == "SHORT")):
                    ...
                    mq_wr = min(1.08, max(0.6, mq_wr))
                    final_score_wr = max(40, min(MAX_CONFIDENCE_BY_SOURCE["warroom"], int(deriv_score * mq_wr)))
                    logger.info(f"[WARROOM] {coin} deriv={deriv_score} ×{mq_wr:.2f} → {final_score_wr}")
    ...
    try:
        ...
        for coin in top_coins:
            ...
            try:
                ...
                market_mult_ea = min(1.1, max(0.5, market_mult_ea))
                final_score_ea = max(35, min(MAX_CONFIDENCE_BY_SOURCE["entry"], int(boosted_score * market_mult_ea) + tf_penalty_ea + conf_penalty_ea))
            ...
            try:
                ...
                if short_score >= dyn_sq_score and short_score > long_score:
                    ...
                    if result:
                        ...
                        mq_sq = min(1.05, max(0.6, mq_sq))
                        result["score"] = max(30, min(MAX_CONFIDENCE_BY_SOURCE["squeeze"], int(result["score"] * mq_sq)))
            ...
            for direction in ["LONG", "SHORT"]:
                ...
                try:
                    ...
                    if market_mult_smc < 0.6:  # FIX SMC #5: dari 0.75 → 0.6 (lebih toleran)
                        ...
                        continue
                    market_mult_smc = min(1.1, market_mult_smc)
                    final_conf_smc = min(MAX_CONFIDENCE_BY_SOURCE["smc"], max(50, int(confidence * market_mult_smc)))
                    logger.info(f"[SMC_ALERT] {coin} {direction}: conf={confidence} ×{market_mult_smc:.2f} → final={final_conf_smc}/{MAX_CONFIDENCE_BY_SOURCE['smc']}")
...
└ Learning    : {total_signals} signals ({accuracy:.0f}% WR)
