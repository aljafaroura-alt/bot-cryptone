...
# TEMEN MODE
TEMEN_MODE = False
TEMEN_COOLDOWN = {}  # cooldown per koin  # PASTIIN ID TELEGRAM LU UDAH ADA
...
# Cache chaos check biar ga spam API
_chaos_cache = {}

def is_market_chaos(symbol, chaos_pct=1.5):
    """Cek chaos dari prevDayPx vs markPx — tanpa candle API call biar ga 429"""
    global _chaos_cache
    now = time.time()
    # Cache 60 detik per coin
    ...
    try:
        ...
        # Chaos kalo gerak > chaos_pct dalam 24h (kasar tapi ga spam API)
...
def get_ob_delta(coin):
    """Orderbook delta weighted by USD value"""
    try:
        l2 = info.l2_snapshot(coin)
...
# TEMEN_MODE + handlers ada di bawah (sudah di-upgrade)
...
<b>🎭 MOOD & RADAR</b>
/mood — Market mood sekarang
/schedule 10 insane — Radar anomali tiap N menit
/schedule 30 mood — Auto mood tiap N menit
/schedule 5 temen — Bacot tiap N menit
/stopschedule — Stop semua auto

<b>🎯 AUTO SNIPER</b>
/sniper — Smart money sniper ON
/sniperaggro — Mode AGGRO (sensitif)
/sniperinsane — Mode INSANE (strict)
/stopsniper — Stop sniper

<b>👥 TEMEN MODE</b>
/temen — Bacot mode ON
/diem — Diem dulu
/temenstatus — Status temen
    ...
    try:
        ...
        for asset, ctx in zip(data[0]["universe"], data[1]):
            try:
                ...
                is_ob_extreme      = abs(ob_delta) > 15
                is_funding_extreme = abs(funding) > 0.03

                if is_moving or is_ob_extreme or is_funding_extreme:
                    signals = get_smart_money_signal(change, ob_delta, funding)
                    alerts.append({
                        'coin': coin, 'price': mark,
                        'change': change, 'ob_delta': ob_delta,
...
def temen_off(message):
    global TEMEN_MODE
    TEMEN_MODE = False
    bot.reply_to(message, "🤐 Oke gw diem dulu... Chat lagi /temen kalo kangen 😄")
...
def get_anomaly_data(coin, meta_cache):
    ...
    try:
        ...
        OI_HISTORY[coin] = oi_now  # FIX: update per coin, bukan timpa seluruh dict
...
def get_market_mood_data():
    try:
        ...
        if avg_funding > 0.08:
            ...
            signal = "💀 LIQUIDATION INCOMING! Ambil profit, jangan serakah."
        elif avg_funding > 0.02:
            mood, emoji = "GREEDY", "😊"
            signal = "⚠️ WASPADA LONG SQUEEZE! Turunkan posisi long."
        elif avg_funding < -0.08:
            mood, emoji = "EXTREME FEAR", "😱"
            signal = "🚀 BOTTOM SIGNAL! Siap2 beli di harga murah."
        elif avg_funding < -0.02:
            mood, emoji = "FEAR", "😨"
            signal = "🔥 SIAP2 SHORT SQUEEZE! Cari setup long."
        else:
            mood, emoji = "NEUTRAL", "😎"
            signal = "Santai trading, ikutin plan aja."
        return {
...
def build_mood_text(data):
    ...
    teks  = f"{data['emoji']} <b>MOOD: {data['mood']}</b> | {get_wib()}\n"
    teks += "━━━━━━━━━━━━━━━━━━━━━━━\n"
    teks += f"💸 Avg Funding : <code>{data['funding']:+.4f}%</code>\n"
    teks += f"🟢 Green       : {data['green_pct']:.0f}% ({data['green']} coins)\n"
    teks += f"🔴 Red         : {100-data['green_pct']:.0f}% ({data['red']} coins)\n"
    teks += f"📊 Total Scan  : {data['total']} coins\n"
    teks += f"\n{bar}\n\n"
    teks += f"{data['signal']}"
    return teks
...
def send_mood_message(chat_id):
    ...
    if not data:
        ...
        return
    bot.send_message(chat_id, build_mood_text(data), parse_mode='HTML')
...
def market_mood(message):
    ...
    if not data:
        ...
        return
    bot.reply_to(message, build_mood_text(data), parse_mode='HTML')
...
def set_schedule(message):
    ...
    try:
        ...
        if mode == 'insane':
            ...
            schedule_jobs[chat_id] = job
            bot.reply_to(message, f"👥 <b>TEMEN MODE ON</b>\nGw bakal bacot tiap {interval} menit.", parse_mode='HTML')
        elif mode == 'mood':
            job = schedule.every(interval).minutes.do(send_mood_message, chat_id=chat_id)
            schedule_jobs[chat_id] = job
            mood_preview = get_market_mood_data()
            mood_emoji = mood_preview['emoji'] if mood_preview else '😊'
            bot.reply_to(message,
                f"{mood_emoji} <b>MOOD MODE ON</b>\n"
                f"Tiap {interval} menit gw kirim mood pasar.\n"
                f"Mode: insane | temen | mood",
                parse_mode='HTML')
        else:
            bot.reply_to(message,
                "❌ Mode ga ada. Pake:\n"
                "• <b>insane</b> — Radar anomali\n"
                "• <b>temen</b> — Bacot market\n"
                "• <b>mood</b> — Market mood",
                parse_mode='HTML')
...
def stop_schedule(message):
    ...
    if chat_id in schedule_jobs:
        ...
        bot.reply_to(message, "🛑 Auto schedule dimatikan.")
    else:
...
def status_cmd(message):
    ...
    teks += f"Bot     : ✅ ONLINE\n"
    teks += f"Token   : <code>{token_preview}</code> [{token_src}]\n"
...
# ===== RUN_SCHEDULER - SATU DEFINISI =====
TEMEN_LAST_RUN = 0  # tracking kapan terakhir temen scan

def run_scheduler():
    global SNIPER_ALL_COIN, TEMEN_MODE, TEMEN_LAST_RUN
    while True:
        try:
            # 1. JALANIN SCHEDULE JOBS (insane radar dll)
            schedule.run_pending()

            # 2. TEMEN MODE — scan tiap 5 menit
            if TEMEN_MODE:
                now = time.time()
            ...
            # 3. JALANIN SNIPER KALO ON
            if SNIPER_ALL_COIN:
                cfg = SNIPER_CONFIG[SNIPER_MODE]
                all_mids = info.all_mids()
                # Ambil meta sekaligus biar ga spam get_ctx per coin
                try:
                ...
                for coin in coins:
                    try:
                        ...
                        # Skip chaos pakai cache (ga panggil candle API)
                        ...
                        if wall >= cfg['wall_min'] and delta >= cfg['delta_min'] and funding <= cfg['funding_max']:
                            alert  = f"🐋 <b>SMART MONEY ENTRY {coin} [{SNIPER_MODE}]</b>\n"
                            alert += f"⏰ {get_wib()}\n"
                            alert += "━━━━━━━━━━━━━━━━━━━━━━━\n"
                            alert += f"💰 Harga   : <code>${price:.4f}</code>\n"
                            alert += f"💸 Funding : <code>{funding:.4f}%</code>\n"
                            alert += f"📡 OB Delta: <code>{delta:.1f}%</code>\n"
                            alert += f"🐋 Bid Wall: <code>${wall/1e6:.2f}M</code>\n"
                            alert += "━━━━━━━━━━━━━━━━━━━━━━━\n"
                        ...
                        time.sleep(0.3)  # Rate limit protection antar coin
            ...
            time.sleep(10)  # Cek tiap 10 detik (sebelumnya 5, lebih aman)
