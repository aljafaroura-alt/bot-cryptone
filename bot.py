...
def entry(message):
    try:
        ...
        # SMC analysis M15 + M5 untuk trigger
        m15 = analyze_tf(coin, "15m")
        m5 = analyze_tf(coin, "5m")
        h1 = analyze_tf(coin, "1h")

        # Tentukan SMC trigger
        smc_trigger = None
        smc_zone = None
        ...
        # Cari zone entry dari M5 atau M15
        ...
        # H1 bias untuk context
        h1_bias = h1["bias"] if h1 else "UNKNOWN"
        h1_struct = h1["structure"] if h1 else "-"

        # Cek alignment SMC vs derivatives
        smc_agrees = (
            (bias == "LONG" and h1_bias == "BULLISH" and (m5 and m5["bias"] in ["BULLISH", "NEUTRAL"])) or
            (bias == "SHORT" and h1_bias == "BEARISH" and (m5 and m5["bias"] in ["BEARISH", "NEUTRAL"]))
        )
        ...
        if smc_zone:
            ...
            teks += f"📍 {smc_zone['type'].upper().replace('_',' ')} ({zone_type}): {fmt_price(smc_zone['low'])} - {fmt_price(smc_zone['high'])}\n"
        ...
        if bias in ["LONG", "SHORT"] and score >= 50:
            ...
            # Override SL dari zone SMC kalau ada dan lebih baik
            ...
            confirm_tag = "✅ SMC KONFIRM" if smc_agrees else "⚠️ DERIV ONLY"
...
def warroom(message):
    try:
        ...
        # Final verdict
        # Cek agreement antara SMC dan derivatives
        ...
        if (smc_bull and deriv_long) or (smc_bear and deriv_short):
            ...
            if smc["aligned_count"] >= 3:
                ...
                teks += "⏳ Tunggu M5 konfirmasi sebelum entry"
        elif smc["dominant_bias"] == "NEUTRAL" or deriv_bias == "NEUTRAL":
            teks += f"⚪ NEUTRAL | {smc['prob']}\n"
            teks += "📊 Belum ada setup yang valid"
        else:
    ...
    try:
        ...
        # Pakai cached wall functions — konsisten dengan /entry
        bid_wall_usd, _ = get_bid_wall_level(coin)
        ask_wall_usd, _ = get_ask_wall_level(coin)
        ...
        # Minimum gap 15 poin sebelum declare bias — cegah flip cepat
        gap = abs(long_score - short_score)
        if gap < 15:
            bias, emoji = "NEUTRAL", "⚪"
