...
_CVD_CACHE_TTL = 30        # 30 detik TTL untuk CVD cache (lebih responsif untuk momentum detection)
_oi_history_cache = {}
_oi_history_time = {}
_history_cache = {}
_history_cache_time = {}
...
SQUEEZE_MULT = 1.2  # FIX: 1.2 supaya target squeeze > spread+fee
...
def get_threshold_breakdown(coin: str, direction: str, alert_type: str) -> str:
    """
    Tampilkan threshold aktif dengan breakdown tiap layer:
    base (dynamic) → IAS adjust → cross-scanner discount → killzone override → FINAL
    Format: '🎯 Threshold: 47 (base 52 → IAS 0.88x → KZ -5)'
    """
    try:
        # Layer 1: dynamic base
        dyn = get_dynamic_thresholds(coin, direction, alert_type)
        base = dyn["min_score"]
        steps = [f"base {base}"]
        ...
        try:
            ...
            after_ias = max(min_thresh, min(max_thresh, after_ias))
            if abs(after_ias - base) >= 2:
                steps.append(f"IAS {ias_mult:.2f}x→{after_ias}")
        except Exception:
            after_ias = base

        # Layer 3: cross-scanner discount
        try:
            with state_lock:
                cross_records = _cross_scanner.get(f"{coin}_{direction}", [])
            high_conf = sum(
                {"warroom": 1.5, "entry": 1.3, "smc": 1.2, "squeeze": 1.0}.get(sc, 0.8)
                for sc, ts in cross_records
                if time.time() - ts < 1800
            )
            if high_conf >= 2.5:
                cross_disc = -10
                ...
                cross_disc = 0
            after_cross = max(40, after_ias + cross_disc)
            if cross_disc != 0:
                steps.append(f"cross{cross_disc:+d}→{after_cross}")
        except Exception:
            after_cross = after_ias

        # Layer 4: killzone override
        kz_active = False
        after_kz = after_cross
        try:
            kz = update_killzone_status()
            if kz.get("is_killzone"):
                mult = _killzone_threshold_multiplier.get(alert_type, 0.85)
                after_kz = max(min_thresh, int(after_cross * mult))
                kz_active = True
                if abs(after_kz - after_cross) >= 2:
                    steps.append(f"KZ×{mult}→{after_kz}")
        except Exception:
            pass

        final = after_kz
        parts = " → ".join(steps)
        kz_tag = " 🕐KZ" if kz_active else ""
        return f"🎯 Threshold aktif: {final} ({parts}){kz_tag}"

    except Exception as e:
        logger.debug(f"[THRESH_BREAKDOWN] {coin} error: {e}")
        return ""
...
def get_scanner_sleep_interval(base_interval: int = 1800) -> int:
    """
    INTERVAL SCANNER - SEMUA SCANNER PAKAI INI
    NY/London aktif: 1-3 menit | Asia: 5 menit | Dead zone: 10 menit
    """
    try:
        regime = get_market_regime()
        jam = get_wib_hour()

        # 🔥 MARKET AKTIF (NY/London) - SCAN CEPAT
...
def get_taker_volume(coin, minutes=5):
    ...
    try:
        ...
        cutoff_ms = now_ms - (minutes * 60 * 1000)  # FIX: cutoff dalam milidetik yang benar
        for t in trades:
...
def get_cvd_acceleration(coin):
    ...
    try:
        # FIX #6: fetch 1x (1h) lalu slice manual — hindari 3 API calls
        # get_cvd(coin, 1) sudah cache 30s, tapi 0.25 & 0.5 punya key berbeda → 3x fetch
        # Solusi: ambil trades sekali, hitung 3 window dari data yang sama
        import time as _t
        now_ms = int(_t.time() * 1000)
        trades_raw = None
        try:
            trades_raw = api_call_with_retry(info.recent_trades, coin, max_retries=2, delay=0.5)
        except Exception:
            pass

        def _calc_cvd_from_trades(trades, minutes):
            if not trades:
                return 0
        ...
        if trades_raw:
            cvd_15m = _calc_cvd_from_trades(trades_raw, 15)
            cvd_30m = _calc_cvd_from_trades(trades_raw, 30)
            cvd_1h  = _calc_cvd_from_trades(trades_raw, 60)
        else:
...
def funding_divergence(coin):
    ...
    try:
        ...
        if funding > 0.05 and change_1h < 1.0:
            # Estimasi: ~8 jam per 0.01% funding ekstrem (konservatif)
            minutes_to_squeeze = max(60, int((0.1 / max(funding, 0.001)) * 60))
            return "LONG_SQUEEZE", 75, minutes_to_squeeze
        elif funding < -0.05 and change_1h > -1.0:
            minutes_to_squeeze = max(60, int((0.1 / max(abs(funding), 0.001)) * 60))
            return "SHORT_SQUEEZE", 75, minutes_to_squeeze
        return None, 0, 0
    except Exception:
        return None, 0, 0
...
def get_ob_delta_fast(coin):
    ...
    try:
        ...
        # FIX #3: pakai top 5 levels & alpha 0.30 — konsisten dengan get_ob_delta
        # (fast version hanya beda di cache TTL yang lebih pendek: 5s vs 30s)
        with state_lock:
            prev = _ob_cache_v2.get(coin, raw)
            smoothed = 0.30 * raw + 0.70 * prev
...
def entry(message):
    try:
        ...
        if bias in ["LONG", "SHORT"] and score >= 50:
            ...
            # === FILTER PARITY WARNING — konsisten dengan entry alert ===
            # Entry alert: wajib zona + wajib vol spike. Command: tampilkan sebagai warning, tidak skip.
            try:
                _warn_parts = []
                # Zona check: apakah price di dalam OB/FVG di M5 atau M15
                _has_zone = smc_zone is not None
                if not _has_zone:
                    for _tf_r in [m5, m15]:
                        if _tf_r and (_tf_r.get("in_ob") or _tf_r.get("in_fvg")):
                            _has_zone = True
                            break
                if not _has_zone:
                    _warn_parts.append("⚠️ NO ZONE — harga belum di OB/FVG")
                # Vol spike check
                try:
                    _vs, _vs_ok = get_volume_spike_info(coin, mark)
                    if not _vs_ok:
                        _warn_parts.append(f"⚠️ VOL LEMAH — spike {_vs:.1f}x (min 1.5x)")
                except Exception:
                    pass
                if _warn_parts:
                    teks += "\n" + "\n".join(_warn_parts)
                    teks += "\n   ⏳ Alert akan skip kondisi ini — pertimbangkan tunggu"
            except Exception as _e_fw:
                logger.debug(f"[ENTRY_CMD] filter_warning: {_e_fw}")
        ...
        if bias in ["LONG", "SHORT"]:
            try:
                thr_line = get_threshold_breakdown(coin, bias, "entry")
                if thr_line:
                    teks += f"\n{thr_line}"
...
📌 /entry {coin} untuk market order (lebih cepat)."""
        ...
        # Threshold breakdown
        try:
            _thr_smc = get_threshold_breakdown(coin, direction, "smc")
            if _thr_smc:
                teks += f"\n{_thr_smc}"
        except Exception:
            pass
...
def warroom(message):
    try:
        ...
        # === UNIFIED CONFIDENCE — konsisten dengan alert & command lain ===
        try:
            _wr_final_dir = "LONG" if smc["dominant_bias"] == "BULLISH" else "SHORT" if smc["dominant_bias"] == "BEARISH" else "LONG"
            # base_score: pakai confluence score cs_wr_cmd yang dihitung di atas, fallback 50
            try:
                _wr_base = cs_wr_cmd["score"]
            except Exception:
                _wr_base = 50
            _uc_wr_cmd = calculate_unified_confidence(coin, _wr_final_dir, base_score=_wr_base, alert_type="warroom")
            teks += "\n" + format_unified_confidence(_uc_wr_cmd)
        except Exception as _e_uc:
            logger.debug(f"[WARROOM] unified_confidence skip: {_e_uc}")

        # Threshold breakdown
        try:
            _thr_wr_dir = "LONG" if smc["dominant_bias"] == "BULLISH" else "SHORT" if smc["dominant_bias"] == "BEARISH" else "LONG"
            _thr_wr = get_threshold_breakdown(coin, _thr_wr_dir, "warroom")
            if _thr_wr:
                teks += f"\n{_thr_wr}"
        except Exception:
            pass
