...
_AGGRESSIVE_MODE = True   # Default ON: skip session filter, candle jadi bonus, cross-val lebih longgar
_ob_cache_time = {}
    ...
    try:
        ...
        if coin:
            try:
                ...
                candles = info.candles_snapshot(coin, "5m", end_ms - 900_000, end_ms)
                if candles and len(candles) >= 3:
                    vols = [float(c.get('v', 0)) * float(c.get('c', 0)) for c in candles[-3:]]
                    avg_vol_5m = sum(vols) / len(vols) if vols else 0
                    if avg_vol_5m < 500_000:
                        return True
            except Exception:
                pass
        return False
...
SMC_MIN_CONFIDENCE = 60  # Longgar dari 80
SMC_MIN_RR = 1.3        # Longgar dari 2.0
SMC_VOLATILE_MIN_CONFIDENCE = 75  # Naik dari 72
SMC_VOLATILE_MIN_RR = 2.3  # Naik dari 2.2

ENTRY_MIN_SCORE = 55  # Longgar dari 75
ENTRY_MIN_RR = 1.2      # Longgar dari 1.8
ENTRY_NEED_ALIGN = 2

SQUEEZE_MIN_SCORE = 50  # Longgar dari 70
SQUEEZE_MIN_RR = 1.2    # Turun dari 1.5
...
_optimize_running = False  # Guard: cegah concurrent optimization runs
...
def apply_best_params(params, profit_factor):
    ...
    # FIX: Baca old_params dengan lock (thread-safe)
    with state_lock:
        old_params = {
    ...
    # FIX: Terapkan parameter baru dengan lock (thread-safe)
    with state_lock:
...
def _run_optimize_in_background():
    """Worker — dijalankan di background thread agar tidak block scheduler loop."""
    ...
    try:
        ...
        logger.error(f"[OPTIMIZE] Background job error: {e}")
    finally:
        with state_lock:
            _optimize_running = False
        logger.info("[OPTIMIZE] Background optimization thread selesai")


def auto_optimize_job():
    """
    Job yang dijalankan otomatis (tiap minggu).
    FIX: Dijalankan di background thread — tidak block scheduler loop.
    """
    global _optimize_running
    with state_lock:
        if _optimize_running:
            logger.info("[OPTIMIZE] Skipped — optimization masih berjalan")
            return
        _optimize_running = True

    t = threading.Thread(target=_run_optimize_in_background, daemon=True, name="AutoOptimize")
    t.start()
    logger.info("[OPTIMIZE] Background optimization thread started")
...
def get_dynamic_entry_config(coin: str, direction: str) -> dict:
    """Entry threshold super dinamis: Regime + Session + Volatility"""
    regime = get_market_regime()
    ...
    min_rr = ENTRY_MIN_RR
    need_confirm = 2
    killzone_bonus = False

    # Regime
    if regime == "TRENDING_UP" and direction == "LONG":
        min_score = int(min_score * 0.85); min_rr *= 0.9; need_confirm = 1
    elif regime == "TRENDING_DOWN" and direction == "SHORT":
        min_score = int(min_score * 0.85); min_rr *= 0.9; need_confirm = 1
    elif regime in ("TRENDING_UP", "TRENDING_DOWN"):
        min_score = int(min_score * 1.2); min_rr *= 1.2; need_confirm = 3
    elif regime == "VOLATILE":
        min_score = int(min_score * 0.85); min_rr *= 0.9; need_confirm = 2  # agresif saat volatil
    elif regime == "PANIC":
        min_score = int(min_score * 1.5); min_rr *= 1.3; need_confirm = 3
    elif regime == "RANGING":
        min_score = int(min_score * 0.9); need_confirm = 2

    # Session
    if session == "ASIA":
        min_score = int(min_score * 1.1); min_rr *= 1.05
    elif session == "NY":
        min_score = int(min_score * 0.9); killzone_bonus = True

    # Volatility
    if vol_profile == "high":
        min_score = int(min_score * 1.15); min_rr *= 1.1; need_confirm = max(need_confirm, 2)
    elif vol_profile == "low":
        min_score = int(min_score * 0.9); need_confirm = max(1, need_confirm - 1)

    return {
        "min_score": max(40, min(90, min_score)),
        "min_rr": round(max(0.8, min(3.0, min_rr)), 1),
        "need_confirm": max(1, min(3, need_confirm)),
...
def get_dynamic_squeeze_config(coin: str, direction: str) -> dict:
    """Squeeze threshold dinamis: Regime + Volatility"""
    regime = get_market_regime()
    coin_upper = coin.upper()
    if coin_upper in VOLATILITY_PROFILE.get("high", []):
        vol_profile = "high"
    ...
    min_score = SQUEEZE_MIN_SCORE
    min_rr = SQUEEZE_MIN_RR
    target_mult = SQUEEZE_MULT
    cooldown = 1200

    if regime == "VOLATILE":
        min_score = int(min_score * 1.3); min_rr *= 1.1; target_mult *= 0.7; cooldown = 1800
    elif regime == "PANIC":
        min_score = int(min_score * 1.5); min_rr *= 1.2; target_mult *= 0.5; cooldown = 2400
    elif regime == "RANGING":
        min_score = int(min_score * 0.8); target_mult *= 1.3; cooldown = 900
    elif regime in ("TRENDING_UP", "TRENDING_DOWN"):
        min_score = int(min_score * 1.1); cooldown = 1800

    if vol_profile == "high":
        min_score = int(min_score * 1.2); min_rr *= 1.1; target_mult *= 0.8
    elif vol_profile == "low":
        min_score = int(min_score * 0.9); target_mult *= 1.2

    return {
        "min_score": max(35, min(85, min_score)),
        "min_rr": round(max(0.8, min(2.5, min_rr)), 1),
        "target_mult": max(0.2, min(1.2, target_mult)),
        "cooldown": cooldown,
        "regime": regime, "vol_profile": vol_profile,
    }


def get_dynamic_smc_config(coin: str, direction: str) -> dict:
    """SMC threshold dinamis: Regime + Session + Volatility"""
    regime = get_market_regime()
    session = get_session_analysis().get("name", "NY")
    coin_upper = coin.upper()
    if coin_upper in VOLATILITY_PROFILE.get("high", []):
    ...
    if regime == "VOLATILE":
        min_confidence = int(min_confidence * 1.25); min_rr *= 1.15
    elif regime == "PANIC":
        min_confidence = int(min_confidence * 1.5); min_rr *= 1.3
    elif regime == "TRENDING_UP":
        min_confidence = int(min_confidence * (0.85 if direction == "LONG" else 1.15))
    elif regime == "TRENDING_DOWN":
        min_confidence = int(min_confidence * (0.85 if direction == "SHORT" else 1.15))
    elif regime == "RANGING":
        min_confidence = int(min_confidence * 0.9)

    if session == "ASIA":
        min_confidence = int(min_confidence * 1.1); min_rr *= 1.05
    elif session == "NY":
        min_confidence = int(min_confidence * 0.9)

    if vol_profile == "high":
        min_confidence = int(min_confidence * 1.15); min_rr *= 1.1
    elif vol_profile == "low":
        min_confidence = int(min_confidence * 0.9)

    return {
        "min_confidence": max(50, min(95, min_confidence)),
        "min_rr": round(max(1.0, min(3.0, min_rr)), 1),
        "regime": regime, "session": session, "vol_profile": vol_profile,
    }
...
def calculate_predator_score_v2(coin):
    ...
    try:
        ...
        with state_lock:
            oi_prev_entry = OI_HISTORY.get(f"{coin}_predator_v2", None)

        now_ts = time.time()
        if oi_prev_entry is None:
            # Scan pertama — simpan OI + timestamp, skip sinyal (belum ada baseline)
            with state_lock:
                OI_HISTORY[f"{coin}_predator_v2"] = (oi_usd, now_ts)
            return None

        # FIX: oi_prev_entry bisa dict lama (float) atau tuple baru (oi, ts)
        if isinstance(oi_prev_entry, tuple):
            oi_prev, oi_prev_ts = oi_prev_entry
        else:
            oi_prev, oi_prev_ts = oi_prev_entry, now_ts - 3600  # fallback legacy

        # Hitung OI change hanya jika selisih waktu >= 45 menit (FIX: bukan menit lalu)
        time_diff = now_ts - oi_prev_ts
        if time_diff >= 2700:  # >= 45 menit, update baseline dan hitung change
            oi_change_1h = ((oi_usd - oi_prev) / oi_prev * 100) if oi_prev > 0 else 0
            with state_lock:
                OI_HISTORY[f"{coin}_predator_v2"] = (oi_usd, now_ts)
        else:
            # Belum 45 menit — pakai baseline lama, jangan update timestamp
            oi_change_1h = ((oi_usd - oi_prev) / oi_prev * 100) if oi_prev > 0 else 0
...
def get_cvd_delta(coin):
    ...
    try:
        ...
        if not new_trades:
            with state_lock:
                accum_val = _cvd_accum.get(coin, 0)
            return accum_val / 1e6, False
...
def get_zone_distance_dynamic(coin, direction=None, mode="alert"):
    ...
    try:
        if mode == "alert":
            base, max_limit, min_limit = 1.5, 3.5, 1.0
        else:
            base, max_limit, min_limit = 4.0, 8.0, 2.5
        ...
        # ATR fine-tune (blend 50/50 dengan regime result)
        try:
            atr = get_atr(coin, period=14, timeframe="1h") or get_atr(coin, period=14, timeframe="15m")
            if atr:
                price = float(info.all_mids().get(coin, 0))
                if price > 0:
                    atr_pct = (atr / price) * 100
                    atr_dist = max(min_limit, min(max_limit, atr_pct * 2.5))
                    base = (base * mult + atr_dist) / 2
                    return max(min_limit, min(max_limit, base))
        except Exception:
            pass
...
def get_smart_zone_distance(coin, direction, mode="alert"):
    ...
    """
    Hitung threshold jarak trendline untuk bonus.
    mode="alert": tighter (0.15% - 0.5%)
    mode="manual": looser (0.3% - 1.0%)
    """
    try:
        tight_base, medium_base = (0.15, 0.3) if mode == "alert" else (0.3, 0.6)
        ...
        try:
            regime = get_market_regime()
            regime_mult = {"VOLATILE": 1.3, "PANIC": 1.5, "TRENDING_UP": 0.8, "TRENDING_DOWN": 0.8}.get(regime, 1.0)
        except Exception:
            regime_mult = 1.0

        ratio = atr_pct / 0.5
        tight  = max(0.1, min(1.0, tight_base  * vol_mult * regime_mult * ratio))
        medium = max(0.2, min(2.0, medium_base * vol_mult * regime_mult * ratio))
        return tight, medium
    except Exception:
        return (0.15, 0.3) if mode == "alert" else (0.3, 0.6)
...
def get_dynamic_fib_config(coin, mode="alert"):
    ...
    mode="manual": 4H, lookback 30, swing 5
    """
    if mode == "alert":
...
def check_liquidation_for_coin(coin, ctx, mark):
    ...
    try:
        ...
        logger.debug(f"[LIQ_DEBUG] {coin}: price={price_change_pct:.2f}% oi={oi_change_pct:.2f}% vol_spike={volume_spike:.2f}x")

        # Dynamic config berdasarkan regime
        cfg = get_dynamic_liq_config()

        is_price_up   = price_change_pct >  cfg["price_change_pct"]
        is_price_down = price_change_pct < -cfg["price_change_pct"]
        is_oi_drop    = oi_change_pct    < -cfg["oi_change_pct"]
        is_volume_spike = volume_spike   >  cfg["volume_spike"]

        # SHORT SQUEEZE: harga NAIK + (OI drop ATAU vol spike) = shorts kena likuidasi
        if is_price_up and (is_oi_drop or is_volume_spike):
            est_liq = estimate_liquidation_amount(oi_change_usd, price_change_pct)
            if est_liq < cfg["min_liq_usd"] and is_volume_spike:
                est_liq = cur_vol * 0.3
            if est_liq >= cfg["min_liq_usd"]:
                ...
                return {
                    "coin": coin, "type": "SHORT SQUEEZE 🔥", "icon": "🚀",
                    "direction": "🟢 shorts kena likuidasi → harga meledak UP!",
                    "nominal": nominal_str, "price_change": price_change_pct,
                    "price": mark, "volume_spike": volume_spike,
                    "oi_change": oi_change_pct, "funding": funding
                }

        # LONG SQUEEZE: harga TURUN + (OI drop ATAU vol spike) = longs kena likuidasi
        elif is_price_down and (is_oi_drop or is_volume_spike):
            est_liq = estimate_liquidation_amount(oi_change_usd, price_change_pct)
            if est_liq < cfg["min_liq_usd"] and is_volume_spike:
                est_liq = cur_vol * 0.3
            if est_liq >= cfg["min_liq_usd"]:
                ...
                nominal_str = f"${est_liq/1_000_000:.1f}M" if est_liq >= 1_000_000 else f"${est_liq/1_000:.0f}K"
                return {
                    "coin": coin, "type": "LONG SQUEEZE 💀", "icon": "📉",
                    "direction": "🔴 longs kena likuidasi → harga ancur DOWN!",
                    "nominal": nominal_str, "price_change": price_change_pct,
                    "price": mark, "volume_spike": volume_spike,
                    "oi_change": oi_change_pct, "funding": funding
                }
...
def run_liquidation_scanner():
    global _liq_scanner_running, _liq_scanner_enabled
    _liq_scanner_running = True
    logger.info("[LIQ] Scanner started")

    while _liq_scanner_enabled:  # FIX: cek flag agar bisa dihentikan graceful
        try:
...
def get_dynamic_confluence_config(coin: str = None, regime: str = None) -> dict:
    """Semua threshold confluence dihitung real-time dari data pasar."""
    try:
        if regime is None:
            regime = get_market_regime()
    except Exception:
        regime = "RANGING"
    try:
        btc_atr = get_atr("BTC", period=14, timeframe="1h")
        btc_price = float(info.all_mids().get("BTC", 50000))
        market_volatility = (btc_atr / btc_price * 100) if btc_atr and btc_price > 0 else 1.0
    except Exception:
        market_volatility = 1.0
    jam = get_wib_hour()
    if 1 <= jam < 7:
        base_volume = 20_000_000
    elif 7 <= jam < 15:
    ...
    regime_mult = {"PANIC": 2.5, "VOLATILE": 1.8, "TRENDING_UP": 1.2, "TRENDING_DOWN": 1.2, "RANGING": 0.8}.get(regime, 1.0)
    ob_delta_threshold_long  = max(2, int(8 / regime_mult))
    ob_delta_threshold_short = min(-2, int(-8 / regime_mult))
    avg_funding = 0.02
    try:
        data = get_cached_meta()
        fundings = [abs(float(ctx.get("funding") or 0) * 100) for ctx in data[1][:30]]
        if fundings:
            avg_funding = _statistics.mean(fundings)
    except Exception:
        pass
    funding_threshold = max(0.01, avg_funding * 1.5)
    return {
        "min_volume_24h": base_volume,
        "min_oi_change_1h": max(1, int(3 / regime_mult)),
        "max_oi_change_1h": min(50, int(30 * regime_mult)),
        "min_oi_change_4h": max(1, int(4 / regime_mult)),
        "min_funding": -funding_threshold,
        "max_funding": funding_threshold,
        "min_price_change_1h": max(0.1, 0.5 / regime_mult),
        "max_price_change_1h": min(50, int(20 * regime_mult)),
        "min_volume_spike": max(1.0, 1.5 / regime_mult),
        "max_volume_spike": min(50, int(25 * regime_mult)),
        "min_ob_delta_long": ob_delta_threshold_long,
        "min_ob_delta_short": ob_delta_threshold_short,
        "zone_timeframe": "4h" if market_volatility > 1.0 else "1h",
        "fvg_timeframe": "1h" if market_volatility > 1.0 else "15m",
        "scan_interval": max(5, int(15 / regime_mult)),
    }

# Backward compat — akan di-refresh tiap scan loop
CONFLUENCE_CONFIG = {
...
def run_confluence_scanner():
    global _conf_scanner_running, _conf_scanner_enabled
    _conf_scanner_running = True
    logger.info("[CONFLUENCE] Scanner started")

    while _conf_scanner_enabled:  # FIX: cek flag agar bisa dihentikan graceful
        try:
            regime = get_market_regime()
            if regime == "VOLATILE":
            ...
            for coin in coins:
                try:
                    ...
                    # Dynamic config per-scan berdasarkan regime real-time
                    cfg = get_dynamic_confluence_config(coin, regime=regime_conf)

                    if abs(price_change) < cfg["min_price_change_1h"]:
                        continue
                    ...
                    if (is_in_zone and zone_type == "demand") or (is_in_fvg and fvg and fvg['type'] == "bullish"):
                        min_delta_long = cfg["min_ob_delta_long"] * (1.5 if volatile_strict else 1.0)
                        max_fund_long  = cfg["max_funding"] * (0.6 if volatile_strict else 1.0)
                        long_ok = True
...
🔥 R:R = 1:{rr_conf:.1f}"""
                    ...
                    if (is_in_zone and zone_type == "supply") or (is_in_fvg and fvg and fvg['type'] == "bearish"):
                        min_delta_short = cfg["min_ob_delta_short"] * (1.5 if volatile_strict else 1.0)
                        min_fund_short  = cfg["min_funding"] * (0.6 if volatile_strict else 1.0)
                        short_ok = True
...
🔥 R:R = 1:{rr_conf:.1f}"""
            ...
            time.sleep(get_dynamic_confluence_config().get("scan_interval", 10) * 60)
    ...
    try:
        ...
        for coin in top_coins:
            ...
            try:
                ...
                # Spread filter — graduated
                try:
                    spread_pct_ea, is_wide_ea, _ = get_spread_warning(coin)
                    if spread_pct_ea > 0.15:
                        logger.debug(f"[ENTRY_ALERT] {coin} skip spread {spread_pct_ea:.3f}% > 0.15%")
                        continue
                    elif is_wide_ea:
                        # spread 0.05-0.15%: penalty score 10%
                        pass  # diterapkan setelah score dihitung
                except Exception:
                    spread_pct_ea = 0
                    is_wide_ea = False

                # Liquidity vacuum filter — skip jika parah (>40%), penalty jika ringan
                _vac_ea, _vacsev_ea = False, 0.0
                try:
                    _vac_ea, _vacsev_ea, _, _, _ = detect_liquidity_vacuum(coin)
                    if _vac_ea and _vacsev_ea > 40:
                        logger.info(f"[ENTRY_ALERT] {coin} skip vacuum {_vacsev_ea:.0f}% > 40%")
                        continue
                except Exception:
                    pass
                ...
                # Apply spread penalty (0.05-0.15%) ke score
                if is_wide_ea and spread_pct_ea <= 0.15:
                    deriv_score = int(deriv_score * 0.9)
                    logger.debug(f"[ENTRY_ALERT] {coin} spread penalty ×0.9 → score={deriv_score}")

                # Apply vacuum penalty proporsional (severity < 40%)
                if _vac_ea and _vacsev_ea > 0:
                    vac_mult = max(0.7, 1.0 - _vacsev_ea / 100)
                ...
                # 3. Cross scanner validation — smart berdasarkan score
                min_scanners_ea = get_smart_cross_val_min(deriv_score, deriv_bias)
                if not has_cross_validation(coin, deriv_bias, min_scanners=min_scanners_ea):
                    logger.debug(f"[ENTRY_ALERT] {coin} skip — cross validation failed (need {min_scanners_ea} scanners for score {deriv_score})")
                    continue
                ...
                if _candle_conf_ea:
                    ...
                    trendline_tags.append("🕯️CONF×1.15")
                else:
                    deriv_score = int(deriv_score * 0.85)
                    trendline_tags.append("⚠️NOCONF×0.85")
                ...
                try:
                    ...
                    if len(vol_candles_ea) >= 5:
                        ...
                        if vol_spike_ea < get_volume_spike_threshold(coin, mode="alert"):
...
def oi_history_cmd(message):
    ...
    try:
        ...
        # FIX: Tampilkan OI trend delta vs snapshot sebelumnya dari OI_HISTORY
        with state_lock:
            oi_prev_snap = OI_HISTORY.get(coin)
        if oi_prev_snap and oi_prev_snap != oi_val and oi_prev_snap > 0:
            oi_chg = ((oi_val - oi_prev_snap) / oi_prev_snap) * 100
            trend_icon = "📈" if oi_chg > 0 else "📉"
            trend_str = f"{trend_icon} {oi_chg:+.1f}% vs snapshot sebelumnya"
        else:
            trend_str = "📊 Snapshot pertama (belum ada pembanding)"

        txt = f"📊 OI SNAPSHOT • {coin}\n─────────────────────────────────\n⏰ {get_wib()}\n─────────────────────────────────\n💰 Harga : {fmt_price(latest['price'])}\n📊 OI    : ${oi_val:.2f}M\n{bar}\n{trend_str}\n─────────────────────────────────\n"
        if oi_val > 500:
...
def run_scheduler():
    ...
    # FIX: Register schedule jobs sekali di luar while loop
    ...
    while True:
        try:
            ...
            if int(now) % 3600 < 5:  # Setiap ~1 jam
                ...
                # FIX: Prune _signal_pending (> 1 hari) — bungkus lock, diakses dari banyak thread
                with state_lock:
...
@bot.message_handler(commands=['liqstop', 'liqstart'])
def liq_scanner_toggle(message):
    """Toggle liquidation scanner on/off."""
    global _liq_scanner_enabled
    if not is_owner(message):
        return
    if message.text.startswith('/liqstop'):
        _liq_scanner_enabled = False
        bot.reply_to(message, "🔴 Liquidation scanner dihentikan.")
    else:
        _liq_scanner_enabled = True
        # Restart thread kalau sudah tidak running
        if not _liq_scanner_running:
            start_liquidation_scanner()
        bot.reply_to(message, "🟢 Liquidation scanner aktif.")


@bot.message_handler(commands=['confstop', 'confstart'])
def conf_scanner_toggle(message):
    """Toggle confluence scanner on/off."""
    global _conf_scanner_enabled
    if not is_owner(message):
        return
    if message.text.startswith('/confstop'):
        _conf_scanner_enabled = False
        bot.reply_to(message, "🔴 Confluence scanner dihentikan.")
    else:
        _conf_scanner_enabled = True
        if not _conf_scanner_running:
            start_confluence_scanner()
        bot.reply_to(message, "🟢 Confluence scanner aktif.")


# ========== OPTIMIZE COMMANDS ==========

@bot.message_handler(commands=['optimizenow'])
def optimize_now_cmd(message):
    """Trigger manual optimization sekarang juga."""
    if not is_owner(message):
        return
    if check_command_cooldown(message.from_user.id, "optimizenow"):
        bot.reply_to(message, f"⏳ Tunggu {COMMAND_COOLDOWN_SEC}s dulu")
        return
    with state_lock:
        already_running = _optimize_running
        n_signals = len(SIGNAL_OUTCOMES_HISTORY)
    if already_running:
        bot.reply_to(message, "⚙️ Optimization masih berjalan...
⏳ Tunggu selesai dulu")
        return
    if n_signals < 30:
        bot.reply_to(message, f"❌ Belum cukup sinyal untuk optimasi
📊 Punya: {n_signals} | Butuh: 30+")
        return
    bot.reply_to(message,
        f"🔄 Manual optimization dimulai...\n"
        f"📊 Signals: {n_signals}\n"
        f"⏳ Estimasi 2-5 menit\n"
        f"📱 Notifikasi otomatis saat selesai"
    )
    auto_optimize_job()


@bot.message_handler(commands=['optimstatus'])
def optim_status_cmd(message):
    """Lihat status optimizer + parameter aktif."""
    if not is_owner(message):
        return
    if check_command_cooldown(message.from_user.id, "optimstatus"):
        bot.reply_to(message, f"⏳ Tunggu {COMMAND_COOLDOWN_SEC}s dulu")
        return
    try:
        with state_lock:
            is_running = _optimize_running
            ems  = ENTRY_MIN_SCORE
            smcc = SMC_MIN_CONFIDENCE
            sqms = SQUEEZE_MIN_SCORE
            emrr = ENTRY_MIN_RR
            smrr = SMC_MIN_RR
            n_sig = len(SIGNAL_OUTCOMES_HISTORY)

        status_str = "🔄 Sedang berjalan..." if is_running else "✅ Idle"
        ...
        try:
            if os.path.exists(BEST_PARAMS_FILE):
                with open(BEST_PARAMS_FILE, 'r') as f:
                    data = json.load(f)
                applied_at = data.get("applied_at", 0)
                last_pf = data.get("profit_factor", 0)
                if applied_at:
                    last_run = datetime.fromtimestamp(applied_at, tz=WIB).strftime("%d/%m %H:%M WIB")
        except Exception:
            pass

        pf_emoji = "🚀" if last_pf > 2.0 else "📈" if last_pf > 1.5 else "⚠️" if last_pf > 0 else "—"
        ...
        txt = (
            ...
            f"🔧 Status   : {status_str}\n"
            f"📅 Last Run : {last_run}\n"
            f"📊 Last PF  : {pf_emoji} {last_pf:.2f}\n"
            f"📈 Signals  : {n_sig} tersimpan\n"
            f"━━━━━━━━━━━━━━━━━━━━━━\n"
            f"🎯 ACTIVE PARAMS:\n"
            f"  ENTRY_MIN_SCORE    = {ems}\n"
            f"  SMC_MIN_CONFIDENCE = {smcc}\n"
            f"  SQUEEZE_MIN_SCORE  = {sqms}\n"
            f"  ENTRY_MIN_RR       = {emrr}\n"
            f"  SMC_MIN_RR         = {smrr}\n"
            f"━━━━━━━━━━━━━━━━━━━━━━\n"
            f"⏰ Auto-run : Setiap Minggu 00:00 WIB\n"
            f"💡 /optimizenow untuk manual trigger"
        )
        bot.reply_to(message, txt)
    except Exception as e:
...
if __name__ == "__main__":
    ...
    def initialize_adaptive_systems():
        ...
        try:
            ...
            logger.error(f"[INIT] vol_profile error: {e}")
        try:
            learn_session_stats(force=True)
        except Exception as e:
            logger.error(f"[INIT] session_stats error: {e}")
        try:
