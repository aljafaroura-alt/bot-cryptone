...
def get_smc_levels_advanced(coin, direction="LONG"):
    ...
    try:
        ...
        confidence = 55
        if zone_tf == "15m":
            confidence -= 5   # FIX: 15m lebih noise — penalty, bukan bonus
        elif zone_tf == "1h":
            confidence += 10
        elif zone_tf == "4h":
            confidence += 20  # FIX: 4H zona lebih kuat dari 15m, naikkan bonus
    ...
    try:
        ...
        for coin in top_coins:
            ...
            try:
                ...
                if short_score >= 55 and short_score > long_score:
                    # SQUEEZE = SCALPING — tidak butuh SMC confirm penuh
                    # Yang penting: M5 tidak sedang hard BEARISH (yang artinya momentum
                    # turun sudah kencang, squeeze LONG bisa terlambat)
                    # H1 BEARISH boleh — squeeze justru sering terjadi saat funding
                    # extreme meski struktur masih bearish (itulah definisi squeeze)
                    r_sq_m5 = analyze_tf(coin, "5m")
                    time.sleep(0.2)
                    m5_bias = r_sq_m5["bias"] if r_sq_m5 else "NEUTRAL"
                    m5_event = r_sq_m5.get("last_event", "") if r_sq_m5 else ""

                    # Hard reject hanya kalau M5 BEARISH + ada BOS/CHoCH turun
                    # = momentum turun masih fresh, belum waktunya bounce
                    hard_contra = (m5_bias == "BEARISH" and m5_event and "BOS 🔽" in m5_event)
                    if hard_contra:
                        logger.info(f"[SQUEEZE_ALERT] {coin} SHORT SQUEEZE SKIP: M5 BEARISH + BOS turun (momentum contra)")
                    else:
                        at_demand = r_sq_m5 and (r_sq_m5.get("in_ob") or r_sq_m5.get("in_fvg"))
                        score = short_score
                        if at_demand:
                            short_score += 10
                            score = short_score
                        # Bonus kalau M5 sudah mulai bullish (squeeze sudah mulai)
                        if m5_bias == "BULLISH":
                            short_score += 8
                            score = short_score
                        ...
                        if rr >= 1.2:  # Squeeze: min RR 1.2 (lebih rendah dari entry/smc, ini scalping)
                            alerts.append({
                                "coin": coin,
                            ...
                            logger.info(f"[SQUEEZE_ALERT] {coin} SHORT SQUEEZE target={target_pct:.1f}% RR={rr:.1f} M5={m5_bias} zone={at_demand}")

                elif long_score >= 55 and long_score > short_score:
                    # SQUEEZE = SCALPING — cek M5 saja, bukan H1
                    r_sq_m5 = analyze_tf(coin, "5m")
                    time.sleep(0.2)
                    m5_bias = r_sq_m5["bias"] if r_sq_m5 else "NEUTRAL"
                    m5_event = r_sq_m5.get("last_event", "") if r_sq_m5 else ""

                    # Hard reject: M5 BULLISH + BOS naik = momentum naik masih fresh
                    hard_contra = (m5_bias == "BULLISH" and m5_event and "BOS 🔼" in m5_event)
                    if hard_contra:
                        logger.info(f"[SQUEEZE_ALERT] {coin} LONG SQUEEZE SKIP: M5 BULLISH + BOS naik (momentum contra)")
                    else:
                        at_supply = r_sq_m5 and (r_sq_m5.get("in_ob") or r_sq_m5.get("in_fvg"))
                        score = long_score
                        if at_supply:
                            long_score += 10
                            score = long_score
                        ...
                        if rr >= 1.2:
                            ...
                            logger.info(f"[SQUEEZE_ALERT] {coin} LONG SQUEEZE target={target_pct:.1f}% RR={rr:.1f} M5={m5_bias} zone={at_supply}")
        ...
        if alerts:
            ...
            for a in alerts[:3]:
                ...
                smc_tf = a.get("m5_bias", "NEUTRAL")
                at_zone = a.get("at_zone", False)
                m5_emoji = "🟢" if smc_tf == "BULLISH" else "🔴" if smc_tf == "BEARISH" else "⚪"
                zone_tag = " | 📍 DI ZONA ✅" if at_zone else ""
                momentum_line = f"⚡ M5: {m5_emoji} {smc_tf}{zone_tag}"
                    ...
                    try:
                        ...
                        if r_4h and r_4h["bias"] != "NEUTRAL":
                            if direction == "LONG" and r_4h["bias"] == "BEARISH":
                                logger.debug(f"[SMC_ALERT] {coin} {direction} skip — 4H bias BEARISH konflik")
                                continue
                            if direction == "SHORT" and r_4h["bias"] == "BULLISH":
                                logger.debug(f"[SMC_ALERT] {coin} {direction} skip — 4H bias BULLISH konflik")
                                continue
