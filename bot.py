...
def has_cross_validation(coin, direction, min_scanners=2):
    ...
    try:
        ...
        with state_lock:
            records = _cross_scanner.get(key, [])
            recent = [r for r in records if now - r[1] < 3600]  # 1 jam
...
def is_low_quality_session():
    ...
    try:
        ...
        if 1 <= jam < 7:
            return True
        # 08:00-11:00 WIB Asia early (tipis, banyak fakeout)
        if 8 <= jam < 11:
            return True
        return False
    except Exception:
...
def has_candle_confirmation(coin, direction, bars=2):
    """
    Periksa apakah bars candle terakhir searah dengan direction.
    Return True = confirmed (proceed), False = tidak confirmed (skip)
    """
    try:
        candles = get_candles_smc(coin, "5m", limit=bars + 2)
...
SMC_MIN_CONFIDENCE = 80  # Naik dari 66 — lebih ketat
SMC_MIN_RR = 2.0        # Naik dari 1.9 — minimal 2:1
SMC_VOLATILE_MIN_CONFIDENCE = 75  # Naik dari 72
SMC_VOLATILE_MIN_RR = 2.3  # Naik dari 2.2

ENTRY_MIN_SCORE = 75  # Naik dari 60 — lebih selective
ENTRY_MIN_RR = 1.8      # Naik dari 1.5
ENTRY_NEED_ALIGN = 2

SQUEEZE_MIN_SCORE = 70  # Naik dari 55 — lebih selective
...
def get_market_regime():
    ...
    if now - _market_regime_cache["time"] < 900:
        ...
        # FIX #5: UNKNOWN fallback ke RANGING (lebih aman)
        return "RANGING" if regime == "UNKNOWN" else regime

    try:
        end_ms = int(now * 1000)
        start_ms = end_ms - (40 * 4 * 60 * 60 * 1000)  # 40 candle 4h (6.6 hari)
        candles = info.candles_snapshot("BTC", "4h", start_ms, end_ms)

        if len(candles) < 10:
            # FIX #5: Fallback RANGING bukan UNKNOWN
        ...
        # FIX #3: Regime PANIC untuk extreme move — skip semua alert otomatis
        if avg_range > 8.0:
            regime = "PANIC"
        # FIX #2: Turunkan threshold VOLATILE 4.5% → 3.0%
        elif avg_range > 3.0:
            regime = "VOLATILE"
        ...
        cached = _market_regime_cache.get("regime", "UNKNOWN")
        # FIX #5: Jangan return UNKNOWN dari error handler
        return "RANGING" if cached == "UNKNOWN" else cached
...
def get_coin_regime(coin):
    """
    Hitung regime dari 1H candle coin itu sendiri.
    Fallback ke get_market_regime() kalau data kurang atau error.
    Cache 15 menit per coin.
    """
    ...
    try:
        end_ms = int(now * 1000)
        start_ms = end_ms - (24 * 1 * 60 * 60 * 1000)  # 24 candle 1h (24 jam)
        candles = info.candles_snapshot(coin, "1h", start_ms, end_ms)

        if len(candles) < 6:
            # Data kurang → fallback ke BTC regime
            return get_market_regime()

        closes = [float(c['c']) for c in candles[-20:]]  # FIX: Use 20 closes for better trend stability
...
def is_market_chaos(symbol, chaos_pct=1.5):
    ...
    try:
        ...
        # FIX #4: 4x multiplier (was 3x) — cegah false chaos flag di large-cap coin volatile (SOL, ETH)
...
def multi_tf_ob_alignment(coin, direction):
    """Cek OB yang align di multiple timeframe (15m, 1h, 4h)"""
    try:
        aligned = []
        for tf in ["15m", "1h", "4h"]:
            ...
            if ob:
                aligned.append(tf)
        strength = min(45, len(aligned) * 15)
...
def check_entry_alert():
    ...
    try:
        ...
        for coin in top_coins:
            ...
            if coin in _entry_alert_last and now_time - _entry_alert_last[coin] < 3600:  # 60 min cooldown
                stat["cooldown"] += 1
                continue
            ...
            try:
                ...
                try:
                    ...
                    if is_wide_ea:
                        logger.info(f"[ENTRY_ALERT] {coin} skip spread {spread_pct_ea:.3f}%")
                        continue
                except Exception:
                    pass
                ...
                if is_low_quality_session():
                    logger.info(f"[ENTRY_ALERT] {coin} skip — low quality session (dead zone)")
                    continue

                # 2. Konflik sektor dengan narrative
                if is_sector_conflict(coin, deriv_bias):
                    narrative_coin = get_narrative(coin)
                    logger.info(f"[ENTRY_ALERT] {coin} skip — sector conflict (narrative: {narrative_coin})")
                    continue

                # 3. Cross scanner validation (butuh minimal 2, kecuali score >=85)
                min_scanners_ea = 1 if deriv_score >= 85 else 2
                ...
                try:
                    ...
                    if len(vol_candles_ea) >= 5:
                        ...
                        if vol_spike_ea < 1.5:
                            logger.info(f"[ENTRY_ALERT] {coin} skip low vol spike {vol_spike_ea:.1f}x")
                            continue
                except Exception:
                    pass
                ...
                need_align = 3  # minimal 3 dari 3 TF (lebih ketat — win rate +)
                ...
                if sweep_ea.get("is_sweeping"):
                    if sweep_ea["status"] == "SWEEPING":
                        sweep_bonus = 20
                        sweep_tags.append(f"🌊 SWEEP {sweep_ea['sweep_type']} @ {fmt_price(sweep_ea['sweep_level'])}")
                        sweep_tags.append(f"🔲 OB: {fmt_price(sweep_ea['ob_low'])}-{fmt_price(sweep_ea['ob_high'])}")
                ...
                # Ranging regime: hanya izinkan jika ada sweep atau BOS+retest
                if regime_ea == "RANGING" and not (sweep_ea.get("is_sweeping") or bos_valid_ea):
                    logger.info(f"[ENTRY_ALERT] {coin} skip — RANGING + no sweep/BOS")
                    continue
        ...
        for coin in top_coins:
            ...
            try:
                ...
                try:
                    ...
                    vol_spike_sq = 1.0
                if abs(funding) < 0.08 and oi_pct_trigger < 20 and vol_spike_sq < 3.0:
...
💵 Funding: {funding:+.4f}%
🔑 Keyakinan: {confidence}%{hunt_warning}{sweep_warning}{bos_warning_smc}{tl_d1_warning_smc}{htf_warning_smc}{conf_warning_smc}{cvd_warning_smc}{fundiv_warning_smc}{oi_warning_smc}{multitf_warning_smc}{timeext_warning_smc}{poc_warning_smc}
            ...
            for direction in ["LONG", "SHORT"]:
                ...
                try:
                    ...
                    try:
                        ...
                        if is_wide_saa:
                            logger.info(f"[SMC_ALERT] {coin} skip spread {spread_pct_saa:.3f}%")
                            continue
                    except Exception:
                        pass
                    ...
                    if is_sector_conflict(coin, direction):
                        logger.info(f"[SMC_ALERT] {coin} skip — sector conflict")
                        continue
                    ...
                    if strong_conf_saa < 3 and confidence < 85:
    ...
    while True:
        try:
            ...
            if SNIPER_ALL_COIN:
                ...
                # FIX #3: PANIC regime → skip seluruh sniper scan
                ...
                for coin in coins:
                    try:
                        ...
                        if is_market_chaos(coin, cfg['chaos_pct']):
                            continue
                        # FIX #1: Pakai regime per-coin (1H), bukan hanya BTC 4H
                        coin_regime = get_coin_regime(coin)
                        if coin_regime == "PANIC":
                            logger.debug(f"[SNIPER] {coin} skip — coin regime PANIC")
                            continue
                        delta = get_ob_delta(coin)
                        funding = get_funding_pct(ctx)
                        price = float(all_mids.get(coin, mark))
                        wall_bid, _ = get_bid_wall_level(coin)   # FIX #1: konsisten pakai level (top-10 largest)
                        wall_ask, _ = get_ask_wall_level(coin)
                        narrative = get_narrative(coin)
                        change = get_change(ctx)

                        # FIX #2: Volume filter — skip coin dengan likuiditas rendah (slippage risk)
                        ...
                        # FIX #3: SHORT funding harus positif (long bayar short) — min 0.005% supaya ada edge
