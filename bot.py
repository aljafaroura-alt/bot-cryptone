...
_cvd_cache = {}
_cvd_last_tid = {}      # track last processed trade ID per coin
_cvd_accum = {}         # accumulated CVD delta per coin (incremental)
_oi_history_cache = {}
_oi_history_time = {}
_history_cache = {}
_history_cache_time = {}
...
def calculate_predator_score(coin):
    ...
    try:
        ...
        # CVD - pakai incremental delta, bukan kumulatif
        cvd_change, is_first_cvd = get_cvd_delta(coin)
        if is_first_cvd:
            # Scan pertama: skip supaya tidak ada score dari data kosong
            return None
...
def get_cvd_delta(coin):
    ...
    Dipakai predator supaya cvd_change tidak selalu 0.
    Return: (cvd_delta_M, is_first_call)
    """
    try:
        trades = info.recent_trades(coin)
        if not trades:
        ...
        # Sortir ascending by tid (trade terbaru = index 0, jadi reverse)
        def get_tid(t):
            return int(t.get('tid', t.get('hash', 0)))
        ...
        if last_tid is None:
            # First call: simpan cursor, return 0
        ...
        logger.debug(f"[CVD_DELTA] {coin} error: {e}")
        return 0, False
