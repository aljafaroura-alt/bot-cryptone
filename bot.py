            ...
            try:
                ...
                if aligned >= need_align:
                    if (dominant == "BULLISH" and deriv_bias == "LONG") or (dominant == "BEARISH" and deriv_bias == "SHORT"):
                        ...
                        # Zone info — OB/FVG/S&D sebagai score booster dan display
                        zone_tags = []
                        sd_boost = 0
                        for label, r in [("1h", r_h1), ("15m", r_m15), ("5m", r_m5)]:
                        ...
                        # Cek S/D zone di 1H dan 4H — boost score kalau harga dalam zona
                        try:
        ...
        if alerts:
            ...
            for a in alerts[:5]:  # tambah jadi 5 alert (karena lebih banyak sinyal)
                ...
                if in_zone >= 2:
                    ...
                    zone_line = f"📍 Zona: Tidak di OB/FVG/S&D"
...
def _is_ob_mitigated(candles, ob_high, ob_low, ob_idx, bias):
    """
    Cek apakah OB sudah 'mitigated' (ditembus >50% oleh candle setelahnya).
    Bullish OB: mitigated kalau ada candle setelah ob_idx yang close di bawah zona midpoint
    Bearish OB: mitigated kalau ada candle setelah ob_idx yang close di atas zona midpoint
    """
    ...
    for j in range(ob_idx + 2, len(candles) - 1):  # exclude candle terakhir (current)
        ...
        c_close = float(c['c'])
        if bias == "BULLISH":
            # Mitigated kalau harga turun menembus midpoint zona
            if c_close < zone_mid:
                return True
        else:  # BEARISH
            # Mitigated kalau harga naik menembus midpoint zona
            if c_close > zone_mid:
                return True
    return False
    ...
    for i in range(len(candles) - 1, 1, -1):
        ...
        if c3_low > c1_high:
            ...
            if gap_size_pct >= 0.05:
                # Mitigation check: FVG basi kalau ada candle setelah i yang close di dalam gap
                mitigated = False
                gap_mid = (gap_low + gap_high) / 2
                for j in range(i + 1, len(candles) - 1):
                    if float(candles[j]['c']) < gap_mid:
                        mitigated = True
                        break
                if mitigated:
                    continue
        ...
        if c3_high < c1_low:
            if fvg_type and fvg_type != "bearish":
                ...
                gap_size_pct = (gap_high - gap_low) / gap_low * 100 if gap_low > 0 else 0
                if gap_size_pct >= 0.05:
                    # Mitigation check: FVG basi kalau candle setelah i close di dalam gap
                    mitigated = False
                    gap_mid = (gap_low + gap_high) / 2
                    for j in range(i + 1, len(candles) - 1):
                        if float(candles[j]['c']) > gap_mid:
...
def find_sd_zone(candles, bias, max_distance_pct=3.0):
    ...
    for i in range(len(candles) - 2, 4, -1):
        ...
        # Freshness check: candle setelah impulse belum pernah menembus >50% zona
        mitigated = False
        for j in range(i + 1, len(candles) - 1):
            c = candles[j]
            c_close = float(c['c'])
            if bias == "BULLISH" and c_close < zone_mid:
                mitigated = True
                break
            if bias == "BEARISH" and c_close > zone_mid:
                mitigated = True
                break
        if mitigated:
            continue
...
def check_smc_alert():
    ...
    try:
        ...
        if _smc_volatile_mode:
            ...
            MIN_CONFIDENCE = 63  # naik sedikit dari 60 — filter noise lebih
            MIN_RR = 1.9
        ...
        for coin in top_coins:
            ...
            for direction in ["LONG", "SHORT"]:
                ...
                try:
                    ...
                    # H1 bull + M15 bear = pullback ke zona = setup SMC paling clean → BAGUS
                    # Yang di-skip: M15 align tapi harga udah jauh lewat zona (late entry)
                    # Bonus confidence kalau M15 sudah mulai reversal (align dengan H1)
                    try:
                        r_15m = analyze_tf(coin, "15m")
                        if r_15m and r_15m["bias"] != "NEUTRAL":
                            # Bonus kalau M15 mulai align (reversal confirmed)
