        ...
        for coin in top_coins:
            ...
            try:
                ...
                if not r_m15:
                    ...
                    continue
                r_m5 = analyze_tf(coin, "5m")
                time.sleep(0.3)  # FIX: was missing sleep after 5m call
                if not r_m5:
                ...
                try:
                    ...
                    try:
                        ...
                        ind_data = {
                            ...
                            "cvd_strong": False,       # warroom tidak track CVD
                ...
                if aligned >= 2:
                    if (dominant == "BULLISH" and deriv_bias == "LONG") or (dominant == "BEARISH" and deriv_bias == "SHORT"):
                        ...
                        alerts.append({
                            ...
                            "rr": rr,
                            "alignment": aligned,
                            "ob_delta": ob_delta,   # untuk learning
                            "funding": funding,      # untuk learning
                        })
...
def squeeze(message):
    try:
        ...
        if short_score >= 70:
            # SHORT SQUEEZE = harga naik (shorts kena liquidasi)
            # Target: jarak ke short_liq level * multiplier
            raw_pct = (short_liq['price'] / mark - 1) * 100  # positif, ke atas
            target_pct = raw_pct * squeeze_mult
            target_price = mark * (1 + target_pct / 100)
            # SL: di bawah harga sekarang (bukan long_liq yang juga di bawah — itu bukan SL long)
            ...
            teks += f"💡 {advice}"
            squeeze_direction = "LONG"  # ride the squeeze = long
            sl_for_learning, tp_for_learning = sl_price, target_price
        elif long_score >= 70:
            # LONG SQUEEZE = harga turun (longs kena liquidasi)
            raw_pct = (long_liq['price'] / mark - 1) * 100  # negatif, ke bawah
            target_pct = raw_pct * squeeze_mult
            target_price = mark * (1 + target_pct / 100)
            sl_pct = max(0.5, abs(raw_pct) * 0.4)
            sl_price = mark * (1 + sl_pct / 100)
        ...
        # Track untuk learning engine kalau ada setup
        if squeeze_direction:
            try:
                ind_data = {
                    "funding_strong": abs(funding) > 0.05,
                    "ob_strong": False,
                    "wall_strong": big_bid > 500_000 if squeeze_direction == "LONG" else big_ask > 500_000,
                    "cvd_strong": False,
                    "momentum_strong": False,
                }
