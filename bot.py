    ...
    try:
        ...
        for coin in top_coins:
            ...
            try:
                ...
                # Candle confirmation: BONUS bukan mandatory
                _candle_conf_ea = has_candle_confirmation(coin, deriv_bias, bars=1)
                if _candle_conf_ea:
                    trendline_bonus += 20
                    trendline_tags.append("🕯️CONFIRM+20")
                else:
                    trendline_bonus -= 5
                    trendline_tags.append("⚠️NO_CONF")
                ...
                if regime_ea == "RANGING" and not (sweep_ea.get("is_sweeping") or bos_valid_ea):
                    ...
                    # VOLATILE: sweep + confirm jadi BONUS, bukan mandatory
                    # dynamic threshold sudah turun 0.85x untuk VOLATILE
                    if sweep_ea.get("is_sweeping") and has_candle_confirmation(coin, deriv_bias, bars=1):
                        trendline_bonus += 15
                        trendline_tags.append("⚡SWEEP+CONF")
                    elif sweep_ea.get("is_sweeping"):
                        trendline_bonus += 8
                        trendline_tags.append("⚡SWEEP")
                    else:
                        trendline_tags.append("⚠️VOLATILE_NO_SWEEP")
...
def process_short_squeeze(coin, mark, short_score, short_liq, big_bid, big_ask, funding, regime):
    ...
    if rr >= SQUEEZE_MIN_RR:
        ...
        if is_volume_anomaly(coin):
            return None
        # Candle confirmation: bonus/penalti, bukan mandatory (squeeze = scalping)
            ...
            for direction in ["LONG", "SHORT"]:
                ...
                try:
                    ...
                    if is_volume_anomaly(coin):
                        ...
                        continue
                    if has_candle_confirmation(coin, direction, bars=2):
                        confidence = min(99, confidence + 25)
                        zone_type = f"{zone_type} 🕯️CONF" if zone_type else "🕯️CONF"
                    else:
                        confidence = max(0, confidence - 10)
                        zone_type = f"{zone_type} ⚠️NOCONF" if zone_type else "⚠️NOCONF"
