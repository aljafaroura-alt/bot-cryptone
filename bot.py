                ...
                for coin in coins:
                    try:
                        ...
                        if wall >= cfg['wall_min'] and delta >= cfg['delta_min'] and funding <= cfg['funding_max']:
                            ...
                            # === TP: ask wall level, fallback fixed % ===
                            _, ask_wall_px = get_ask_wall_level(coin)
                            if ask_wall_px > 0 and ask_wall_px > price:
                                tp = ask_wall_px * 0.999
                                tp_pct = abs((tp - price) / price * 100)
                                if tp_pct < 0.5 or tp_pct > 10.0:
                            ...
                            # === Minimum R:R 1.5 — kalau ga nyampe, fallback full fixed ===
                            if rr < 1.5:
                                sl_pct = sl_pct_fallback
                                tp_pct = tp_pct_fallback
                                sl = price * (1 - sl_pct / 100)
                            ...
                            alert += f"🐋 Wall: ${wall/1e6:.2f}M\n\n"
                            alert += f"🎯 Entry : {fmt_price(price)}\n"
                            alert += f"🛑 SL    : {fmt_price(sl)} (-{sl_pct:.1f}%)\n"
                            alert += f"✅ TP    : {fmt_price(tp)} (+{tp_pct:.1f}%)\n"
