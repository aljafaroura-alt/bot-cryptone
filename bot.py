...
def get_dynamic_entry_config(coin: str, direction: str) -> dict:
    ...
    if regime == "TRENDING_UP" and direction == "LONG":
        ...
        score_mult, rr_mult = get_dynamic_volatile_multiplier_realtime(coin)
        min_score = int(min_score * score_mult); min_rr *= rr_mult; need_confirm = 2  # agresif saat volatil
    elif regime == "PANIC":
...
def get_zone_distance_dynamic(coin, direction=None, mode="alert"):
    ...
    """
    try:
        # Base dari ATR real-time (bukan hardcode 1.5/4.0)
...
def get_atr_multipliers(coin: str) -> tuple:
    """ATR tinggi → agresif (turunkan threshold, perlebar zone). Returns (score_mult, rr_mult, zone_mult)."""
    try:
        atr = get_atr(coin, period=14, timeframe="1h")
        price = float(info.all_mids().get(coin, 0))
...
def get_liquidity_multiplier(coin: str) -> float:
    """Deep orderbook & tight spread → 1.0-1.1. Tipis & lebar → 0.6-0.9."""
    try:
        total_depth, _, _ = get_orderbook_depth(coin, top_levels=10)
        if total_depth <= 0:
            return 0.8
        l2 = info.l2_snapshot(coin)
        best_bid = float(l2['levels'][0][0]['px'])
        ...
        return max(0.6, min(1.1, depth_score * 0.6 + spread_score * 0.4))
    except Exception:
        return 0.9


def get_volume_confidence_multiplier(coin: str, mark: float) -> float:
    """Volume tinggi & spike → 1.0-1.15. Rendah & flat → 0.7-0.9."""
    try:
        ctx, _ = get_ctx(coin)
        vol_24h = float(ctx.get("dayNtlVlm") or 0)
        jam = get_wib_hour()
        baseline = 5_000_000 if jam < 7 or jam >= 20 else (10_000_000 if jam < 15 else 8_000_000)
...
def get_dynamic_candle_multiplier_realtime(coin: str, direction: str) -> tuple:
    """Candle confirmation multiplier berdasarkan ROC + ATR 5m real-time."""
    try:
        candles = get_candles_smc(coin, "5m", limit=6)
        roc = 0.0
        if candles and len(candles) >= 2:
            roc = (float(candles[-1]['c']) - float(candles[-2]['c'])) / float(candles[-2]['c']) * 100
...
def get_dynamic_volatile_multiplier_realtime(coin: str) -> tuple:
    """VOLATILE regime multiplier dari ATR & volume aktual. Returns (score_mult, rr_mult)."""
    try:
        atr = get_atr(coin, period=14, timeframe="1h")
        price = float(info.all_mids().get(coin, 0))
        if not atr or price <= 0:
        ...
        try:
            ctx, _ = get_ctx(coin)
            if float(ctx.get("dayNtlVlm") or 0) > 100_000_000:
...
def get_dynamic_zone_base_realtime(coin: str, mode: str) -> float:
    """Zone base dari ATR real-time. Bukan hardcode 1.5/4.0."""
    try:
        atr = get_atr(coin, period=14, timeframe="1h")
        price = float(info.all_mids().get(coin, 0))
        if not atr or price <= 0:
        ...
        atr_pct = (atr / price) * 100
        if mode == "alert":
            base, min_val, max_val = atr_pct * 1.2, 0.8, 3.0
        else:
            base, min_val, max_val = atr_pct * 2.5, 1.5, 7.0
        coin_upper = coin.upper()
        if coin_upper in VOLATILITY_PROFILE.get("high", []):
            base *= 1.3
        elif coin_upper in VOLATILITY_PROFILE.get("low", []):
...
def get_dynamic_spread_thresholds_realtime(coin: str) -> tuple:
    """Spread thresholds dinamis dari spread aktual. Returns (skip_thresh, penalty_thresh, penalty_mult)."""
    try:
        l2 = info.l2_snapshot(coin)
        best_bid = float(l2['levels'][0][0]['px'])
        best_ask = float(l2['levels'][1][0]['px'])
        mid = (best_bid + best_ask) / 2
        cur_spread = (best_ask - best_bid) / mid * 100
        if cur_spread < 0.03:
            s, p, m = 0.25, 0.08, 0.95
        elif cur_spread < 0.05:
            s, p, m = 0.2, 0.06, 0.92
        elif cur_spread < 0.1:
            s, p, m = 0.18, 0.05, 0.9
        else:
            s, p, m = 0.15, 0.04, 0.85
        if cur_spread > 0.1:
            s = min(s, 0.12)
        return s, p, m
    except Exception:
...
def get_dynamic_vacuum_thresholds_realtime(coin: str) -> tuple:
    """Vacuum thresholds dari depth history + regime. Returns (skip_sev, penalty_start)."""
    try:
        ...
        if len(recent) > 5:
            ...
            skip_sev     = int(drops[int(len(drops) * 0.8)] * 100)
            penalty_start = int(drops[int(len(drops) * 0.5)] * 100)
        else:
    ...
    try:
        ...
        for coin in top_coins:
            ...
            try:
                ...
                # Spread filter — dinamis berdasarkan spread aktual coin
                spread_pct_ea = 0; is_wide_ea = False
                try:
                    spread_pct_ea, is_wide_ea, _ = get_spread_warning(coin)
                    _skip_thr, _pen_thr, _pen_mult = get_dynamic_spread_thresholds_realtime(coin)
                    if spread_pct_ea > _skip_thr:
                        logger.debug(f"[ENTRY_ALERT] {coin} skip spread {spread_pct_ea:.3f}% > {_skip_thr:.3f}%")
                        continue
                    elif spread_pct_ea > _pen_thr:
                        deriv_score = int(deriv_score * _pen_mult)
                        logger.debug(f"[ENTRY_ALERT] {coin} spread penalty ×{_pen_mult:.2f}")
                except Exception:
                    spread_pct_ea = 0; is_wide_ea = False

                # Liquidity vacuum filter — threshold dinamis dari depth history
                _vac_ea, _vacsev_ea = False, 0.0
                try:
                    _vac_ea, _vacsev_ea, _, _, _ = detect_liquidity_vacuum(coin)
                    if _vac_ea:
                        _vac_skip, _vac_pen_start = get_dynamic_vacuum_thresholds_realtime(coin)
                        if _vacsev_ea > _vac_skip:
                            logger.info(f"[ENTRY_ALERT] {coin} skip vacuum {_vacsev_ea:.0f}% > {_vac_skip:.0f}%")
                            continue
                        elif _vacsev_ea > _vac_pen_start:
                            _vac_pen_mult = 1.0 - (_vacsev_ea - _vac_pen_start) / 100
                            deriv_score = int(deriv_score * max(0.7, _vac_pen_mult))
                            logger.debug(f"[ENTRY_ALERT] {coin} vacuum penalty {_vacsev_ea:.0f}%")
                except Exception:
                    pass
                ...
                # Candle confirmation: multiplier dinamis berdasarkan ROC + ATR
