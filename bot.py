...
def api_call_with_retry(func, *args, max_retries=3, delay=1, **kwargs):
    """Wrapper retry + token bucket + circuit breaker (Patch 10)."""
...
def check_liquidation_for_coin(coin, ctx, mark):
    ...
    try:
        ...
        if len(candles) < 3:
            return None
        # FIX BUG #1: Sort ascending by timestamp — API kadang return descending
        candles = sorted(candles, key=lambda x: int(x.get('t', 0)))

        # Harga change dari candle 1 menit lalu (candles[-1] = terbaru, candles[-2] = 1m lalu)
...
def run_liquidation_scanner():
    ...
    while _liq_scanner_enabled and not _shutdown_event.is_set():  # FIX: cek flag agar bisa dihentikan graceful
        try:
...
def detect_liquidity_hunt(coin, direction="LONG"):
    ...
    try:
        ...
        bull_ob_m5  = find_ob_zone(c_m5,  "BULLISH", max_distance_pct=2.0) if is_bullish_bos else None
        bull_ob_m15 = find_ob_zone(c_m15, "BULLISH", max_distance_pct=2.0) if is_bullish_bos else None
        bear_ob_m5  = find_ob_zone(c_m5,  "BEARISH", max_distance_pct=2.0) if is_bearish_bos else None
        bear_ob_m15 = find_ob_zone(c_m15, "BEARISH", max_distance_pct=2.0) if is_bearish_bos else None
                ...
                try:
                    ...
                    if (is_in_zone and zone_type == "demand") or (is_in_fvg and fvg and fvg['type'] == "bullish"):
                        ...
                        if long_ok:
                            ...
                            if rr_conf >= (1.5 if volatile_strict else 1.2):
    ...
    try:
        ...
        for coin in top_coins:
            ...
            try:
                ...
                if aligned >= need_align and ((dominant == "BULLISH" and deriv_bias == "LONG") or (dominant == "BEARISH" and deriv_bias == "SHORT")):
                    ...
                    mq_wr = max(0.6, mq_wr)  # minimal 0.6x, tidak skip
                    final_score_wr = max(40, min(99, int(deriv_score * mq_wr)))
        ...
        if alerts:
            ...
            for a in alerts[:5]:
                ...
                confluence_line = ""
                try:
                    cs_wr = calculate_smart_confluence_score(a['coin'], a['direction'], a['price'])
                    bar_len_wr = min(10, int(cs_wr['score'] / 10))
                    bar_wr = "🟩" * bar_len_wr + "⬜" * (10 - bar_len_wr)
                    confluence_line = f"\n🧠 CONFLUENCE: {cs_wr['grade']} | {cs_wr['score']}/100\n{bar_wr}"
                    if cs_wr["tags"]:
                        confluence_line += f"\n📌 {', '.join(cs_wr['tags'][:3])}"
                except Exception:
                    pass
    ...
    try:
        ...
        for coin in top_coins:
            ...
            try:
                ...
                boosted_score = min(99, deriv_score + sd_boost + trendline_bonus + cross_bonus_ea + zone_bonus_ea)
                ...
                market_mult_ea = max(0.5, market_mult_ea)  # minimal 0.5x, tidak skip
                final_score_ea = max(35, min(99, int(boosted_score * market_mult_ea) + tf_penalty_ea + conf_penalty_ea))
                logger.info(f"[ENTRY_ALERT] {coin} final={final_score_ea} mq={market_mult_ea:.2f}x tf_pen={tf_penalty_ea} conf_pen={conf_penalty_ea} cross={cross_bonus_ea} zone={zone_bonus_ea}")
        ...
        if alerts:
            ...
            for a in alerts[:5]:
                ...
                # Confluence score
                try:
                    cs_ea = calculate_smart_confluence_score(a['coin'], a['direction'], a['price'])
                    bar_len_ea = min(10, int(cs_ea['score'] / 10))
                    bar_ea = "🟩" * bar_len_ea + "⬜" * (10 - bar_len_ea)
                    conf_teks_ea = f"\n🧠 CONFLUENCE: {cs_ea['grade']} | {cs_ea['score']}/100\n{bar_ea}"
                    if cs_ea["tags"]:
                        conf_teks_ea += f"\n📌 {', '.join(cs_ea['tags'][:3])}"
                    teks += md_escape(conf_teks_ea)
                except Exception:
                    pass
...
💡 /squeeze {a['coin']} | /entry {a['coin']}"""
                ...
                # Confluence score
                try:
                    cs_sq = calculate_smart_confluence_score(a['coin'], squeeze_dir, a['price'])
                    bar_len_sq = min(10, int(cs_sq['score'] / 10))
                    bar_sq = "🟩" * bar_len_sq + "⬜" * (10 - bar_len_sq)
                    teks += f"\n🧠 CONFLUENCE: {cs_sq['grade']} | {cs_sq['score']}/100\n{bar_sq}"
                    if cs_sq["tags"]:
                        teks += f"\n📌 {', '.join(cs_sq['tags'][:3])}"
                except Exception:
                    pass
...
def start(message):
    ...
    teks = f"""
🧬 <b>HYPERLIQUID TERMINAL BOT v4.0</b>
GM/GN {user} 😼
...
🔫 <b>SNIPER &amp; ALERTS</b>
/sniper — Auto sniper ON
/sniperaggro — Mode AGGRO (lebih agresif)
/sniperinsane — Mode INSANE (filter ketat)
/stopsniper — Stop sniper
/sniperstatus — Status sniper saat ini

🚨 <b>AUTO ALERTS (TOGGLE)</b>
/warroomalert on/off — Warroom (min 60)
/entryalert on/off — Entry (min 70)
/squeezealert on/off — Squeeze (min 55)
/smcalert on/off — SMC (min 60%)
/predatortoggle on/off — Predator scan
/copytradealert on/off — Copy alert
/copytracker on/off — Wallet tracker
...
/correlation BTC — Korelasi coin
/mood — Market sentiment
/sentiment — Funding + price sentiment
/delta — OB delta depth
/trap — Stop hunt detection
/vol — Volatility scanner
...
/banditstatus — Bandit UCB1 status
...
🔧 <b>Bot by Cryptone | v4.0 FINAL</b>
"""
...
def master_market_scan(force: bool = False) -> dict:
    ...
    try:
        ...
        top_coins = [c[0] for c in coins_vol[:25]]
...
📌 /entry {coin} untuk market order (lebih cepat)."""
        ...
        # Confluence score
        try:
            cs_smc_cmd = calculate_smart_confluence_score(coin, direction, mark)
            bar_len_sc = min(10, int(cs_smc_cmd['score'] / 10))
            bar_sc = "🟩" * bar_len_sc + "⬜" * (10 - bar_len_sc)
            teks += f"\n🧠 CONFLUENCE: {cs_smc_cmd['score']}/100 {cs_smc_cmd['grade']}"
            teks += f"\n{bar_sc}"
            if cs_smc_cmd["tags"]:
                teks += f"\n📌 {', '.join(cs_smc_cmd['tags'][:4])}"
        except Exception:
            pass
    ...
    try:
        ...
        for coin in top_coins:
            for direction in ["LONG", "SHORT"]:
                ...
                try:
                    ...
                    if market_mult_smc < 0.6:  # FIX SMC #5: dari 0.75 → 0.6 (lebih toleran)
        ...
        if alerts:
            ...
            for a in alerts[:3]:
                ...
                # Confluence score
                try:
                    cs_smc = calculate_smart_confluence_score(a['coin'], a['direction'], a['price'])
                    bar_len_smc = min(10, int(cs_smc['score'] / 10))
                    bar_smc = "🟩" * bar_len_smc + "⬜" * (10 - bar_len_smc)
                    conf_teks_smc = f"\n🧠 CONFLUENCE: {cs_smc['grade']} | {cs_smc['score']}/100\n{bar_smc}"
                    if cs_smc["tags"]:
                        conf_teks_smc += f"\n📌 {', '.join(cs_smc['tags'][:3])}"
                    teks += md_escape(conf_teks_smc)
                except Exception:
                    pass
...
def status_cmd(message):
    ...
    global _liq_scanner_running, _conf_scanner_running, _warroom_alert_running
    chat_id = message.chat.id
    ...
    liq_text = "✅ RUNNING" if _liq_scanner_running else "🔴 STOPPED"
    conf_text = "✅ RUNNING" if _conf_scanner_running else "🔴 STOPPED"
    ...
    if not _predator_enabled:
        ...
        predator_text = "✅ ON (tiap 90 menit)"
    else:
        predator_text = "🟡 IDLE"

    # ========== ALERTS ==========
    warroom_alert_status = "✅ ON (≥60, tiap 30m)" if _warroom_alert_running else "❌ OFF"
    entry_alert_status = "✅ ON (≥70, tiap 20m)" if _entry_alert_running else "❌ OFF"
    ...
    corr_status = f"✅ {corr_coins} coins, {corr_age}m ago" if corr_coins > 0 else "🟡 building..."
...
├ Session   : {get_sesi()}
├ Regime    : {get_market_regime()}
└ Threshold : {threshold_mode} (E:{ENTRY_MIN_SCORE} S:{SMC_MIN_CONFIDENCE} SQ:{SQUEEZE_MIN_SCORE})

🕶️ <b>SNIPER</b>     : {sniper_text}
👽 <b>TEMEN</b>      : {temen_text}
⛔ <b>LIQ SCAN</b>   : {liq_text}
🔍 <b>CONFLUENCE</b> : {conf_text}
💀 <b>DIVERGENCE</b> : {div_text}
💎 <b>CVD</b>        : {cvd_text}
🌐 <b>SMART FLOW</b> : {smart_text}
👹 <b>PREDATOR</b>   : {predator_text}
...
└ Tracker   : {"✅ ON" if _copytrade_tracker_enabled else "🔕 OFF"}

📊 <b>MARKET INTEL</b>
├ Correlation : {corr_status}
├ IA Score    : {ias_mult:.2f}x ({ias.get('reason', 'normal')[:40]})
└ Learning    : {len(SIGNAL_OUTCOMES_HISTORY)} signals
...
def sniper_status_cmd(message):
    ...
    if SNIPER_ALL_COIN and _sniper_auto_state == "auto_on":
        ...
        auto_info = "\n✋ <b>Manual off</b> — Kamu matiin sendiri, auto tidak akan nyala"

    teks = f"""🕶️ <b>SNIPER STATUS</b>
━━━━━━━━━━━━━━━━━━━━━━
Status      : {status}
Mode        : {mode_emoji} {SNIPER_MODE}
━━━━━━━━━━━━━━━━━━━━━━

📡 <b>THRESHOLDS SAAT INI</b>
