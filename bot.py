...
WATCHED_WALLETS = {}        # {address: label} — auto-populated, jangan hardcode
...
def entry(message):
    try:
        ...
        # Ambil orderbook untuk cari wall besar (BID & ASK)
        l2 = info.l2_snapshot(coin)
        bids = l2['levels'][0]
        asks = l2['levels'][1]
        ...
        # Cari BID wall terbesar di 15 level pertama (minimal 200k)
        for b in bids[:15]:
        ...
        # Cari ASK wall terbesar di 15 level pertama (minimal 200k)
        for a in asks[:15]:
        ...
        # Hitung score dengan regime bonus
        ...
        # Tampilkan setup berdasarkan bias dan score
...
def warroom(message):
    try:
        ...
        # Ambil wall besar (sama persis dengan di /entry)
        l2 = info.l2_snapshot(coin)
        bids = l2['levels'][0]
        asks = l2['levels'][1]
        bid_wall_usd = 0
        ...
        # Hitung score (sudah pakai regime bonus)
        ...
        ob_display = max(-75, min(75, ob_delta))
...
def load_wallet_state():
    """Load last known positions dan watched wallets dari file"""
    global _wallet_last_positions, WATCHED_WALLETS
    try:
        if os.path.exists(WALLET_TRACKER_FILE):
            ...
            logger.info(f"[WALLET] Loaded {len(WATCHED_WALLETS)} wallets, {len(_wallet_last_positions)} position snapshots")
    except Exception as e:
        logger.error(f"[WALLET] Load error: {e}")


def save_wallet_state():
    """Persist positions dan watched wallets ke file"""
    try:
        with state_lock:
            data = {
...
def fetch_leaderboard_wallets(limit: int = 20) -> list:
    ...
    try:
        ...
        if r.status_code != 200:
            ...
            return []
        data = r.json()
        # Format: list of {ethAddress, accountValue, windowPerformances: [{period, pnl, roi, vlm}]}
        traders = []
        for entry in data:
            addr = entry.get("ethAddress", "")
            if not addr or len(addr) < 10:
                continue
            # Ambil pnl dari window "week" (lebih reliable dari "day")
            perfs = entry.get("windowPerformances", [])
            pnl = 0
            for wp in perfs:
                if wp.get("period") == "week":
    ...
    try:
        ...
        for coin in hot_coins:
            try:
                ...
                trades = info.recent_trades(coin)
                for t in trades[:50]:
                    addr = t.get("users", [None])[0] if t.get("users") else None
...
def auto_discover_wallets():
    ...
    # Trim ke max
    trimmed = dict(list(new_wallets.items())[:WALLET_MAX_TRACK])

    with state_lock:
        old_count = len(WATCHED_WALLETS)
        WATCHED_WALLETS = trimmed
        _wallet_discovery_last = time.time()

    logger.info(f"[WALLET] Discovery done: {old_count} → {len(WATCHED_WALLETS)} wallets tracked")

    if len(trimmed) > 0:
        names = ", ".join(list(trimmed.values())[:5])
        send_to_both(
            f"🔍 WALLET DISCOVERY UPDATE\n"
            f"⏰ {get_wib()}\n"
            f"━━━━━━━━━━━━━━━━━\n"
            f"📡 Tracking {len(trimmed)} smart money wallets\n"
            f"🏆 Top: {names}{'...' if len(trimmed) > 5 else ''}"
        )
