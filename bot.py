...
def fetch_high_oi_wallets(limit: int = 10) -> list:
    ...
    try:
        ...
        # FIX: Kalau OI_HISTORY belum populate, fallback ke coins liquid yang diketahui
        if not hot_coins:
            hot_coins = ["BTC", "ETH", "SOL", "BNB", "XRP"]
            logger.info("[WALLET] OI_HISTORY kosong, fallback ke default coins untuk hi-OI scan")

        found_wallets = {}
        mids = info.all_mids()
        # FIX: pakai COPYTRADE_SIZE_FILTER sesuai mode, bukan hardcode $50K
        ...
        for coin in hot_coins:
            try:
                ...
                for t in trades[:50]:
                    ...
                    price = float(mids.get(coin, 0))
                    notional = sz * price
                    # FIX: akumulasi lintas coins (bukan skip kalau wallet udah ada)
                    if notional >= trade_filter:
