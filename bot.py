...
def get_smc_levels_advanced(coin, direction="LONG"):
    ...
    try:
        ...
        for tf_candles, tf_name in [(candles_15m, "15m"), (candles_1h, "1h"), (candles_4h, "4h")]:
            if not tf_candles:
                continue
            ob = find_ob_zone(tf_candles, direction, max_distance_pct=2.5)
            if ob:
                zone = ob
                zone_type = f"OB ({tf_name})"
                zone_tf = tf_name
                break
            fvg = find_fvg_smc(tf_candles, max_distance_pct=2.5)
            if fvg:
        ...
        if ctx:
            ob_delta = get_ob_delta(coin)
            funding = get_funding_pct(ctx)
            if direction == "LONG" and ob_delta > 10:
                confidence += 10
            elif direction == "SHORT" and ob_delta < -10:
                confidence += 10
        ...
        try:
            _atr_sl_pct, _ = get_adaptive_sltp(coin, current_price, direction)
    ...
    try:
        ...
        for coin in top_coins:
            ...
            try:
                ...
                # FIX: Borderline score (<75) wajib konfirmasi price di zona OB/FVG
                # Cegah entry di tengah nowhere hanya karena funding/OB delta kebetulan tinggi
                if deriv_score < 75 and in_zone_count == 0:
    ...
    try:
        ...
        for coin in top_coins:
            ...
            try:
                ...
                # OB DELTA (FIX: tambah price action ke squeeze scoring, bukan cuma funding+liq+wall)
                ...
                if short_score >= 55 and short_score > long_score:
                    # FIX: SMC Filter — validasi struktur harga sebelum fire alert
                    r_sq_h1 = analyze_tf(coin, "1h")
                    time.sleep(0.2)
                    r_sq_m15 = analyze_tf(coin, "15m")
                    time.sleep(0.2)

                    # Hard reject: 1h BEARISH = structure contra SHORT SQUEEZE (arah LONG)
                    if r_sq_h1 and r_sq_h1["bias"] == "BEARISH":
                        logger.info(f"[SQUEEZE_ALERT] {coin} SHORT SQUEEZE SKIP: 1h BEARISH (SMC contra)")
                    else:
                        smc_bullish = sum(1 for r in [r_sq_h1, r_sq_m15] if r and r["bias"] == "BULLISH")
                        if smc_bullish == 0:
                            logger.info(f"[SQUEEZE_ALERT] {coin} SHORT SQUEEZE SKIP: 0 TF BULLISH (no SMC confirm)")
                        else:
                            ...
                            if rr >= 1.2:
                                ...
                                logger.info(f"[SQUEEZE_ALERT] {coin} SHORT SQUEEZE target={target_pct:.1f}% RR={rr:.1f} SMC={smc_bullish}TF zone={at_demand}")
                    ...
                    # Hard reject: 1h BULLISH = structure contra LONG SQUEEZE (arah SHORT)
                    if r_sq_h1 and r_sq_h1["bias"] == "BULLISH":
                        logger.info(f"[SQUEEZE_ALERT] {coin} LONG SQUEEZE SKIP: 1h BULLISH (SMC contra)")
                    else:
                        smc_bearish = sum(1 for r in [r_sq_h1, r_sq_m15] if r and r["bias"] == "BEARISH")
                        if smc_bearish == 0:
                            logger.info(f"[SQUEEZE_ALERT] {coin} LONG SQUEEZE SKIP: 0 TF BEARISH (no SMC confirm)")
                        else:
                            ...
                            if rr >= 1.2:
                                ...
                                logger.info(f"[SQUEEZE_ALERT] {coin} LONG SQUEEZE target={target_pct:.1f}% RR={rr:.1f} SMC={smc_bearish}TF zone={at_supply}")
        ...
        if alerts:
            ...
            for a in alerts[:3]:
                ...
                # Fix #2: Tampilkan SMC context dari smc_tf dan at_zone
                smc_tf = a.get("smc_tf", 0)
                at_zone = a.get("at_zone", False)
                if smc_tf >= 2:
                    smc_line = f"📐 SMC: {smc_tf}/2 TF konfirmasi" + (" ✅ DI ZONA" if at_zone else "")
                elif smc_tf == 1:
                    smc_line = f"📐 SMC: {smc_tf}/2 TF konfirmasi" + (" ✅ DI ZONA" if at_zone else " ⚠️ partial")
                else:
                    smc_line = f"📐 SMC: scoring only (no TF confirm)"
...
def find_fvg_smc(candles, max_distance_pct=2.0):
    ...
    for i in range(len(candles) - 1, 1, -1):
        ...
        if c3_low > c1_high:
            ...
            if gap_size_pct < 0.05:  # FVG terlalu kecil, skip noise
                continue
            ...
            for direction in ["LONG", "SHORT"]:
                ...
                try:
                    ...
                    if direction == "LONG" and structure_bias == "BEARISH":
                        logger.debug(f"[SMC_ALERT] {coin} LONG skip — structure bias BEARISH (konflik)")
                        continue
                    if direction == "SHORT" and structure_bias == "BULLISH":
                        logger.debug(f"[SMC_ALERT] {coin} SHORT skip — structure bias BULLISH (konflik)")
                        continue
                    ...
                    if direction == "LONG" and funding_contra_long:
                        logger.debug(f"[SMC_ALERT] {coin} LONG skip — funding {funding:+.4f}% & OB delta {ob_delta_smc:+.0f}% contra")
                        continue
                    if direction == "SHORT" and funding_contra_short:
                        logger.debug(f"[SMC_ALERT] {coin} SHORT skip — funding {funding:+.4f}% & OB delta {ob_delta_smc:+.0f}% contra")
                        continue
                    # ─────────────────────────────────────────────────────
...
def squeeze(message):
    try:
        ...
        # SMC Filter — konsisten dengan check_squeeze_alert
        # Ambil struktur H1 + M15 sebelum fire setup
        r_sq_h1 = analyze_tf(coin, "1h")
        r_sq_m15 = analyze_tf(coin, "15m")
        h1_bias_sq = r_sq_h1["bias"] if r_sq_h1 else "NEUTRAL"
        m15_bias_sq = r_sq_m15["bias"] if r_sq_m15 else "NEUTRAL"
        at_zone_sq = any(r and (r.get("in_ob") or r.get("in_fvg")) for r in [r_sq_h1, r_sq_m15])

        # Tampilkan info SMC di output
        smc_tf_long = sum(1 for b in [h1_bias_sq, m15_bias_sq] if b == "BULLISH")
        smc_tf_short = sum(1 for b in [h1_bias_sq, m15_bias_sq] if b == "BEARISH")
        teks += f"🧠 SMC H1: {h1_bias_sq} | M15: {m15_bias_sq}"
        teks += " ✅ DI ZONA\n" if at_zone_sq else "\n"
        teks += "─────────────────────────────────\n"

        if short_score >= 55 and short_score > long_score:
            # Hard reject: H1 BEARISH = contra SHORT SQUEEZE (arah LONG)
            if h1_bias_sq == "BEARISH":
                teks += f"⛔ SHORT SQUEEZE BLOCKED\n"
                teks += f"1H BEARISH — struktur contra LONG\n"
                teks += f"Tunggu struktur balik BULLISH/NEUTRAL\n"
            elif smc_tf_long == 0:
                teks += f"⚠️ SHORT SQUEEZE LEMAH\n"
                teks += f"Score {short_score} tapi 0 TF BULLISH — kurang konfirmasi SMC\n"
                teks += f"Hati-hati, bisa fakeout\n"
            else:
                raw_pct = (short_liq['price'] / mark - 1) * 100
                raw_pct = min(2.5, raw_pct)
                target_pct = raw_pct * squeeze_mult
                ...
                sl_price = mark * (1 - sl_pct / 100)
                rr = target_pct / sl_pct if sl_pct > 0 else 0
                zone_bonus = " ✅ DI ZONA" if at_zone_sq else ""
                teks += f"🚨 SHORT SQUEEZE | SMC {smc_tf_long}/2 TF{zone_bonus}\n"
                teks += f"🎯 Target : {fmt_price(target_price)} (+{target_pct:.1f}%)\n"
                teks += f"⛔ SL     : {fmt_price(sl_price)} (-{sl_pct:.1f}%)\n"
                teks += f"⚓ R:R    : 1:{rr:.1f}\n"
                teks += f"💡 {advice}\n"
                teks += f"⚡ EKSEKUSI SEKARANG\n"
                squeeze_direction = "LONG"
                sl_for_learning, tp_for_learning = sl_price, target_price

        elif long_score >= 55 and long_score > short_score:
            # Hard reject: H1 BULLISH = contra LONG SQUEEZE (arah SHORT)
            if h1_bias_sq == "BULLISH":
                teks += f"⛔ LONG SQUEEZE BLOCKED\n"
                teks += f"1H BULLISH — struktur contra SHORT\n"
                teks += f"Tunggu struktur balik BEARISH/NEUTRAL\n"
            elif smc_tf_short == 0:
                teks += f"⚠️ LONG SQUEEZE LEMAH\n"
                teks += f"Score {long_score} tapi 0 TF BEARISH — kurang konfirmasi SMC\n"
                teks += f"Hati-hati, bisa fakeout\n"
            else:
                raw_pct = (mark / long_liq['price'] - 1) * 100
                raw_pct = min(2.5, raw_pct)
                ...
                rr = target_pct / sl_pct if sl_pct > 0 else 0
                zone_bonus = " ✅ DI ZONA" if at_zone_sq else ""
                teks += f"🚨 LONG SQUEEZE | SMC {smc_tf_short}/2 TF{zone_bonus}\n"
                teks += f"🎯 Target : {fmt_price(target_price)} (-{target_pct:.1f}%)\n"
                teks += f"⛔ SL     : {fmt_price(sl_price)} (+{sl_pct:.1f}%)\n"
                teks += f"⚓ R:R    : 1:{rr:.1f}\n"
                teks += f"💡 {advice}\n"
