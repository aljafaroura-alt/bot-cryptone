...
_iceberg_depth_history = {}  # {coin: [(ts, bid_depths, ask_depths)]}
_ob_cache_time = {}
...
def get_dynamic_entry_config(coin: str, direction: str) -> dict:
    ...
    if regime == "TRENDING_UP" and direction == "LONG":
        ...
        min_score = int(min_score * 1.2); min_rr *= 1.1; need_confirm = 3
    elif regime == "PANIC":
...
def get_dynamic_trendline_threshold(coin, direction=None, mode="alert"):
    """
    ...
    try:
        ...
        if alerts:
            ...
            for a in alerts[:3]:
                ...
                try:
                    pos_sz_ww = get_adaptive_position_size(a['coin'], a['direction'], a['price'], a.get('sl', a['price'] * 0.98))
                    genius_line += f"\n💰 Sizing: ${pos_sz_ww['position_usd']:,.0f} | Risk {pos_sz_ww['risk_pct']:.1f}% {pos_sz_ww['risk_label']}"
                except Exception:
                    pass
                ...
                try:
                    ...
                    try:
                        ...
                        ind_data = {
                            "funding_strong": abs(a.get("ob_delta", 0)) > 0.02,
...
def predict_price_ensemble(coin, direction):
    """Ensemble prediction dari indikator existing"""
    score = 0
    reasons = []
    try:
        rsi = get_rsi(coin)
    ...
    try:
        macd = get_macd(coin)
        if macd and direction == "LONG" and macd["trend"] == "BULLISH":
            score += 20; reasons.append("MACD bullish")
        elif macd and direction == "SHORT" and macd["trend"] == "BEARISH":
            score += 20; reasons.append("MACD bearish")
    except Exception: pass
    try:
        imb, _, _ = get_order_flow_imbalance(coin)
        if direction == "LONG" and imb > 15:
            score += 20; reasons.append(f"Flow +{imb:.0f}%")
        elif direction == "SHORT" and imb < -15:
            score += 20; reasons.append(f"Flow {imb:.0f}%")
    except Exception: pass
    try:
        _, is_accel, accel_dir = get_cvd_acceleration(coin)
        if is_accel and ((direction == "LONG" and accel_dir == "BULLISH") or
                         (direction == "SHORT" and accel_dir == "BEARISH")):
            score += 15; reasons.append("CVD accel")
    except Exception: pass
    try:
        oi_pct, is_oi, oi_dir = oi_impulse(coin)
        if is_oi and ((direction == "LONG" and oi_dir == "LONG") or
                      (direction == "SHORT" and oi_dir == "SHORT")):
            score += 15; reasons.append(f"OI +{oi_pct:.0f}%")
    except Exception: pass
    try:
        regime = get_market_regime()
        if regime == "TRENDING_UP" and direction == "LONG":
            score = int(score * 1.15); reasons.append("trending up")
        elif regime == "TRENDING_DOWN" and direction == "SHORT":
            score = int(score * 1.15); reasons.append("trending down")
    except Exception: pass
    confidence = min(92, score)
    return {"direction": direction, "confidence": confidence, "reasons": reasons[:3], "is_strong": confidence >= 65}


# ============================================================
# ADAPTIVE POSITION SIZING
# ============================================================
def get_adaptive_position_size(coin, direction, entry_price, sl_price, account_equity=10000):
    """Rekomendasi ukuran posisi adaptif dari fungsi existing"""
    try:
        atr = get_atr(coin)
        atr_pct = (atr / entry_price) * 100 if atr and entry_price > 0 else 1.0
    except Exception:
        atr_pct = 1.0
    try:
        regime = get_market_regime()
    ...
    try:
        ...
        is_vacuum, vac_sev = False, 0
    try:
        mq, _ = get_market_quality_multiplier(coin, direction, entry_price, alert_type="entry")
    except Exception:
        mq = 1.0

    base_risk = 2.0
    regime_mult = {"TRENDING_UP": 1.4, "TRENDING_DOWN": 1.4, "VOLATILE": 0.6, "RANGING": 0.9, "PANIC": 0.4}.get(regime, 1.0)
    session_mult = 0.6 if session == "ASIA" else 1.0 if session == "LONDON" else 1.2
    vol_mult = max(0.4, min(1.3, 1.2 / (atr_pct + 0.3)))
    vacuum_mult = 0.5 if is_vacuum and vac_sev > 30 else 0.8 if is_vacuum else 1.0
    mq_mult = max(0.7, min(1.3, mq))

    final_risk = max(0.5, min(5.0, base_risk * regime_mult * session_mult * vol_mult * vacuum_mult * mq_mult))
    sl_dist = abs(entry_price - sl_price) if sl_price and entry_price else entry_price * 0.02
    risk_amount = account_equity * (final_risk / 100)
    position_size = risk_amount / sl_dist if sl_dist > 0 else 0

    risk_label = "🔥 AGGRESSIVE" if final_risk >= 3.0 else "⚡ NORMAL" if final_risk >= 1.8 else "🛡️ CONSERVATIVE"
    return {
        "risk_pct": round(final_risk, 1),
        "position_size": round(position_size, 4),
        "position_usd": round(position_size * entry_price, 0),
        "risk_label": risk_label,
    }


# ============================================================
# AGGRESSIVE ICEBERG DETECTION
# ============================================================
def detect_iceberg_adaptive(coin):
    """Deteksi iceberg order dari depth changes"""
    global _iceberg_depth_history
    now = time.time()
    signals = []
    try:
        l2 = api_call_with_retry(info.l2_snapshot, coin)
        bids = l2['levels'][0][:10]
        asks = l2['levels'][1][:10]
        bid_depths = [float(b['sz']) * float(b['px']) for b in bids]
        ask_depths = [float(a['sz']) * float(a['px']) for a in asks]
        with state_lock:
            if coin not in _iceberg_depth_history:
                _iceberg_depth_history[coin] = []
            _iceberg_depth_history[coin].append((now, bid_depths, ask_depths))
            if len(_iceberg_depth_history[coin]) > 12:
                _iceberg_depth_history[coin] = _iceberg_depth_history[coin][-12:]
            history = list(_iceberg_depth_history[coin][-6:])
        if len(history) < 4:
            return signals
        regime = get_market_regime()
        threshold = 2 if regime == "VOLATILE" else 3
        for level in range(min(3, len(bid_depths))):
            bid_changes, ask_changes = [], []
            for i in range(1, len(history)):
                pb = history[i-1][1][level] if level < len(history[i-1][1]) else 0
                cb = history[i][1][level] if level < len(history[i][1]) else 0
                pa = history[i-1][2][level] if level < len(history[i-1][2]) else 0
                ca = history[i][2][level] if level < len(history[i][2]) else 0
                if pb > 0: bid_changes.append(cb / pb)
                if pa > 0: ask_changes.append(ca / pa)
            bid_refills = sum(1 for j in range(1, len(bid_changes)) if bid_changes[j] > 1.2 and bid_changes[j-1] < 0.8)
            ask_refills = sum(1 for j in range(1, len(ask_changes)) if ask_changes[j] > 1.2 and ask_changes[j-1] < 0.8)
            if bid_refills >= threshold:
                signals.append(f"🧊 Iceberg BID L{level+1} — whale accumulating")
            if ask_refills >= threshold:
                signals.append(f"🧊 Iceberg ASK L{level+1} — whale distributing")
        return signals
    except Exception as e:
        logger.debug(f"[ICEBERG] {coin}: {e}")
        return []
    ...
    try:
        ...
        for coin in top_coins:
            ...
            try:
                ...
                # ML ensemble boost
                try:
                    ml_pred = predict_price_ensemble(coin, deriv_bias)
                    if ml_pred["is_strong"]:
                        final_score_ea = min(99, final_score_ea + 10)
                        trendline_tags.append(f"🧠ML+{ml_pred['confidence']:.0f}%")
                except Exception:
                    ml_pred = {"is_strong": False, "confidence": 0, "reasons": []}
        ...
        if alerts:
            ...
            for a in alerts[:3]:
                ...
                # Iceberg detection
                try:
                    ice_sigs = detect_iceberg_adaptive(a['coin'])
                    if ice_sigs:
                        genius_line += "\n" + "  ".join(ice_sigs[:2])
                except Exception:
                    pass
                ...
                # Position sizing
                try:
                    pos_sz = get_adaptive_position_size(a['coin'], a['direction'], a['price'], a['sl'])
                    pos_line = f"\n💰 Sizing: ${pos_sz['position_usd']:,.0f} | Risk {pos_sz['risk_pct']:.1f}% {pos_sz['risk_label']}"
                except Exception:
                    pos_line = ""
...
def process_short_squeeze(coin, mark, short_score, short_liq, big_bid, big_ask, funding, regime):
    ...
    try:
        candles_1h = get_candles_smc(coin, "1h", limit=50)
        if candles_1h:
            dyn_dist = get_zone_distance_dynamic(coin, direction="LONG", mode="alert") * 1.3
...
def process_long_squeeze(coin, mark, long_score, long_liq, big_bid, big_ask, funding, regime):
    ...
    try:
        candles_1h = get_candles_smc(coin, "1h", limit=50)
        if candles_1h:
            dyn_dist = get_zone_distance_dynamic(coin, direction="SHORT", mode="alert") * 1.3
