# scanners_entry.py - PART 1: IMPORT, INIT, DAN TF ANALYSIS
# ENTRY ALERT SCANNER - Bagian 1 dari 2

import time
import logging

from config import (_entry_alert_last, _sweep_pending, _fakeout_pending, MAX_CONFIDENCE_BY_SOURCE,
                    _AGGRESSIVE_MODE, state_lock)
...
def check_entry_alert():
    ...
    try:
        ...
        # Filter coin berdasarkan volume
        coins = []
        ...
        for coin in top_coins:
            ...
            try:
                ...
                # ========== SCORE CALCULATION ==========
                long_score, short_score = calculate_scores_smart(ob_delta, funding, bid_wall, ask_wall, 
                                                                  short_liq_size, long_liq_size, coin=coin)
                ...
                # ========== TIMEFRAME ANALYSIS (dari master cache) ==========
                ...
                # ========== LANJUT KE PART 2 ==========
                # (Lanjut ke file scanners_entry_part2.py)
        ...
        if alerts:
            ...
            for a in alerts[:5]:
                ...
                teks = (
                    ...
                    f"📊 {md_escape(a['alignment'])}/{md_escape(a.get('tf_total', 3))} TF align\n"
...
# scanners_entry.py - PART 2: SWEEP, HTF, DIVERGENCE, SL/TP, DAN FINAL SCORE
# ENTRY ALERT SCANNER - Bagian 2 dari 2
# (Ini adalah lanjutan dari for loop di Part 1, tepat setelah zone_bonus)

                # ========== SWEEP BONUS (LANJUTAN DARI PART 1) ==========
                sweep_bonus = 0
                ...
                # ========== SCORE AGGREGATION ==========
                boosted_score = min(MAX_CONFIDENCE_BY_SOURCE["entry"], 
                                    deriv_score + sd_boost + trendline_bonus + cross_bonus + zone_bonus)
                ...
                alerts.append({
                    ...
                    "trendline_tags": trendline_tags, "vol_spike": vol_spike,
                })
        ...
        # ========== KIRIM ALERT (sudah ada di Part 1) ==========
        # (Kode pengiriman alert sudah ditulis di Part 1 setelah for loop)
