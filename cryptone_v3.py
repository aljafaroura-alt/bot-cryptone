...
class HyperliquidAdapter:
    ...
    def __init__(self, config: Config):
        ...
        self._rest_min_interval: float = 0.12  # ~8 req/s ceiling, well under HL's limit
    ...
    async def _rest_post(self, path: str, payload: dict, *, max_retries: int = 3):
        """Shared throttled POST for every REST candleSnapshot/funding/
        l2Book call. Two things layered on top of a plain aiohttp POST:

        1. Pacing — self._rest_lock + self._rest_min_interval enforce a
           minimum gap between *any* two outgoing REST calls from this
           client, so a burst of 700 backfill requests (232 symbols x 3
           timeframes) goes out at a steady ~8 req/s instead of all at
           once.
        2. 429 retry — HTTP 429 specifically gets exponential backoff
           retries (unlike other failures, which still fail soft
        ...
        for attempt in range(max_retries + 1):
            async with self._rest_lock:
                wait = self._rest_min_interval - (time.monotonic() - self._rest_last_call)
                if wait > 0:
                ...
                async with self._session.post(
                    ...
                    if resp.status == 429:
                        if attempt < max_retries:
                            ...
                            logger.debug(f"_rest_post({path}): HTTP 429, retry {attempt + 1}/{max_retries} in {backoff:.1f}s")
