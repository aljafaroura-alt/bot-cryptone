...
class TelegramAdapter:
    ...
    async def send_events_batched(self, events: list):
        """Send a scan's worth of events as few messages as possible:
        one compact monospace table for all MEDIUM/WATCHING-tier events,
        plus one full-detail message per HIGH/CRITICAL event so urgent
        signals aren't buried in the table or delayed waiting on it.
        """
        if not events:
            return
        ...
        for e in urgent_tier:
            await self._send_text(TelegramFormatter.format_event(e))
...
class CryptoneV3:
    ...
    async def _flush_scan_events(self):
        ...
        if events:
            await self.telegram.send_events_batched(events)
