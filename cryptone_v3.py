...
class Config:
    ...
    def from_env(cls) -> "Config":
        ...
        return cls(
            hyperliquid_api=os.environ.get("HYPERLIQUID_API", "https://api.hyperliquid.xyz"),
            hyperliquid_ws=os.environ.get("HYPERLIQUID_WS", "wss://api.hyperliquid.xyz/ws"),
...
class WSConnection:
    ...
    async def connect(self) -> bool:
        ...
        if not WEBSOCKETS_AVAILABLE:
            logger.warning(
                "⚠️  'websockets' package not installed — WSConnection running in "
                "SIMULATION MODE. Run `pip install websockets` for real Hyperliquid WS."
            )
            self.connected = True
    ...
    async def _connect_with_reconnect(self):
        ...
        while self._running:
            try:
                ...
                logger.info(f"✅ WS Connected (ID: {self.connection_id})")
                attempts = 0
            ...
            if self._running:
                ...
                logger.info(f"Reconnecting in {delay:.0f}s (attempt {attempts})")
...
class DiscoveryEngine:
    ...
    async def discover(self, snapshots: Dict[str, MarketData]) -> List[DiscoveryScore]:
        ...
        logger.info(f"🔍 Discovery: {len(scores)} candidates → Top {len(top)}")
        for s in top[:5]:
...
class HyperliquidAdapter:
    ...
    async def __aenter__(self):
        if AIOHTTP_AVAILABLE:
            ...
            logger.info("🔌 HyperliquidAdapter connected (REST mode via aiohttp)")
        else:
            logger.warning(
                "⚠️  'aiohttp' package not installed — HyperliquidAdapter running in "
                "SIMULATION MODE. Run `pip install aiohttp` for real Hyperliquid REST data."
            )
    ...
    async def snapshot_all(self) -> Dict[str, MarketData]:
        ...
        try:
            ...
            logger.error(f"REST snapshot failed: {e} — falling back to last known snapshot")
    ...
    def _transition_state(self, cand: Candidate, evidence: Dict) -> Candidate:
        ...
        if current == PrimaryState.WATCHING:
            ...
            if sum(basic) >= self.min_evidence:
                ...
                logger.debug(f"{cand.symbol}: WATCHING → ACTIVE")
            ...
            if conviction_score > 0.6:
                ...
                logger.debug(f"{cand.symbol}: ACTIVE → HIGH_CONVICTION")
            elif conviction_score < 0.2:
                cand.state = PrimaryState.DORMANT
                logger.debug(f"{cand.symbol}: ACTIVE → DORMANT")
            ...
            if thesis_score > 0.5 and not evidence['thesis_broken']:
                ...
                logger.info(f"🔥 {cand.symbol}: HIGH_CONVICTION → THESIS")
            elif evidence['thesis_broken']:
                cand.state = PrimaryState.DORMANT
                logger.warning(f"{cand.symbol}: THESIS_BROKEN")
...
class TelegramAdapter:
    ...
    async def __aenter__(self):
        ...
        if not AIOHTTP_AVAILABLE:
            logger.warning("⚠️  'aiohttp' not installed — Telegram alerts will be logged, not sent.")
            return self

        if not (self.config.telegram_token and self.config.telegram_chat_id):
            logger.warning("⚠️  telegram_enabled=True but token/chat_id missing — alerts will be logged, not sent.")
            return self
        ...
        try:
            async with self._session.get(f"{self._api_base}/getMe", timeout=aiohttp.ClientTimeout(total=10)) as resp:
                ...
                if data.get("ok"):
                    ...
                    logger.info(f"📨 TelegramAdapter connected as @{bot_name}")
                else:
                    logger.error(f"📨 Telegram getMe failed: {data}")
        except Exception as e:
            logger.error(f"📨 Telegram connection check failed: {e}")
    ...
    async def send_event(self, event: Event):
        ...
        if not self._is_configured() or not self._session:
            logger.info(f"📨 [TELEGRAM not configured] Would send:\n{text}")
            return
...
class CryptoneV3:
    ...
    async def initialize(self):
        """Initialize system"""
        logger.info("🚀 Initializing Cryptone v3.0...")
        logger.info("   🔍 Dynamic Discovery: ENABLED")
        logger.info(f"   📊 Max candidates: {self.config.discovery.max_candidates}")
        logger.info(f"   🎯 Anchors: {self.config.anchors.symbols}")
    ...
    async def _on_ws_reconnect(self):
        ...
        logger.info("🔁 WS reconnected — refreshing REST snapshot for recovery")
        try:
            await self.hyperliquid.snapshot_all()
        except Exception as e:
    ...
    async def scan(self):
        ...
        if not discovered:
            logger.debug("No interesting candidates found")
            return

        top_candidates = discovered[:self.config.discovery.max_candidates]

        logger.info(f"🔍 Discovery: Top {len(top_candidates)} candidates")
        for i, score in enumerate(top_candidates[:5]):
            logger.info(f"   #{i+1} {score.symbol}: score={score.total_score:.3f} "
        ...
        if active:
            logger.info(f"📊 Active: {len(active)} | High conviction/Thesis: {len(high_conviction)}")
            for c in active[:3]:
                logger.info(f"   {c.symbol}: {c.state.value} Q:{c.quality:.2f}")
    ...
    async def _handle_event(self, event: Event):
        ...
        logger.info(f"📢 {event.priority.value}: {event.symbol} - {event.event_type}")
    ...
    async def run(self, max_duration_seconds: Optional[float] = None):
        ...
        logger.info("🔍 Starting dynamic market surveillance...")
        logger.info(f"   Discovery scan: {self.config.discovery.scan_interval}s")
        logger.info(f"   Max candidates: {self.config.discovery.max_candidates}")
        logger.info("   Universe: DYNAMIC (all Hyperliquid markets)")
        if max_duration_seconds:
            logger.info(f"   ⏱️  Max run duration: {max_duration_seconds:.0f}s (CI job limit guard)")
            ...
            try:
                if max_duration_seconds is not None:
                    ...
                    if elapsed_total >= max_duration_seconds:
                        logger.info(f"⏱️  Max duration reached ({elapsed_total:.0f}s) — shutting down gracefully")
                        break
