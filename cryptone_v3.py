...
class TimeframeConfig:
    ...
    Contract (per war-room review):
      macro_context   (1H)  — higher-timeframe structure/trend; the bar
                               PreMove's CONTEXT_ALIGNED support leg and
...
class ContextEngine:
    ...
    def get_context(self, symbol: str, timeframe: Optional[str] = None) -> Dict:
        """Get context from true OHLC candles. Defaults to setup_context
    ...
    def get_all_contexts(self, symbol: str) -> Dict:
        ...
        PreMoveEngine's CONTEXT_ALIGNED support leg, OpportunityEngine's
        direction read) that expect a single primary trend field.
