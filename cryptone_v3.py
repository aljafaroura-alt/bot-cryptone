...
class StateMachine:
    ...
    def _propose_transition(self, cand: Candidate, current: "PrimaryState", evidence: Dict) -> "PrimaryState":
        ...
        if current == PrimaryState.WATCHING:
            ...
            if evidence['thesis_broken'] or divergent or evidence.get('direction_flip'):
                next_state = PrimaryState.DORMANT
...
class TelegramFormatter:
    ...
    def format_event(event: Event) -> str:
        ...
        lines = [
            f"{priority_emoji} *{event.symbol} PERP* — {event.state}{dir_badge}",
        ]
        ...
        stats = f"*{event.state}* · {event.direction.upper() if event.direction else 'UNDETERMINED'} · Q{event.quality:.2f}"
        lines.append(stats)
        ...
        # one read before scrolling into the raw STRUCTURE/PRE-MOVE/
        # REVERSAL sections below.
        ...
        # from MicrostructureEngine.build_expectation_narrative. Only
        # shown when there's actually an expectation open or just
        # resolved this snapshot (None otherwise) — this is meant to be
        # a rare, meaningful callout, not a line on every alert.
        ...
        # see build_target_map. Shown right after Micro Expectation since
        # it's the natural follow-on question ("if this confirms, what's
        # actually between here and there"). Only rendered when path_state
        # isn't UNKNOWN — an unknown read has nothing useful to show, same
        # "don't show a line just to show something" posture as the
        # Micro Expectation block above.
        tgt = event.target_map
        if tgt and tgt.get("path_state") != "UNKNOWN":
            state_emoji = {
        ...
        # Already gated at the source (generate_event's caller only
        # attaches this when sample_count >= footprint_min_samples), so
        # here it's just "show it if present" — no second threshold
        # check duplicated in the formatter. Independent of Target Map
        # above (own divider) so it still renders even on events with no
        # target_map (e.g. WATCHING-tier, direction not yet set).
    ...
    def format_event_caption(event: Event) -> str:
        ...
        if event.direction == "long":
            ...
            dir_badge = "  ⚠️ NO DIRECTION"
        lines = [f"{priority_emoji} *{event.symbol} PERP* — {event.state}{dir_badge}"]
            ...
            for side_clusters, side_name in (
                ...
                for c in walls:
                    result.zones.append(RadarZone(
                        ...
                        label=f"{side_name.upper()} WALL ({abs(c.pct_from_mid):.2f}%, {c.relative_depth:.1f}x avg)",
                    ))
                for c in gaps:
                    result.zones.append(RadarZone(
                        kind="liquidity_gap",
