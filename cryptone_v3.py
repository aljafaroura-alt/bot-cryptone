...
# Belief-level strings used by EngineBelief.level (STRONG/MODERATE/WEAK)
# — a fourth confidence vocabulary alongside float/LOW-MEDIUM-HIGH,
# found during the Gap 1 audit. Mapped to representative floats so
# normalize_confidence() can place it on the same 0-1 scale.
_BELIEF_LEVEL_TO_FLOAT = {"STRONG": 0.85, "MODERATE": 0.6, "WEAK": 0.3}
_CATEGORY_TO_FLOAT = {"LOW": 0.25, "MEDIUM": 0.55, "HIGH": 0.85}
_FLOAT_TO_CATEGORY_BOUNDARIES = (
    (0.7, "HIGH"),
    (0.4, "MEDIUM"),
)
...
def normalize_confidence(value: object) -> Optional[float]:
    """Single normalization point for Gap 1: every confidence-shaped
    value in this file is one of (a) float 0.0-1.0
    (PreMoveSignal.confidence, ReversalSignal.confidence,
    macro_bias_confidence, ...), (b) "LOW"/"MEDIUM"/"HIGH" string
    (AIContextResult.risk, ExternalIntel.confidence), or (c)
    "STRONG"/"MODERATE"/"WEAK" (EngineBelief.level). Returns a plain
    0.0-1.0 float regardless of which vocabulary it came in as, or
    None if the value can't be interpreted at all (never guesses).
    Callers needing a bucket back out (e.g. for a badge) should use
    confidence_to_category() on the result, not re-derive their own
    thresholds — keeps the CRITICAL/HIGH/NORMAL/LOW importance cutoffs
    consistent across every engine feeding MarketIntelligenceBuilder.
    """
    if value is None:
        return None
    ...
    s = value.value if isinstance(value, Enum) else str(value)
    s = s.upper().strip()
    if s in _CATEGORY_TO_FLOAT:
        return _CATEGORY_TO_FLOAT[s]
    if s in _BELIEF_LEVEL_TO_FLOAT:
        return _BELIEF_LEVEL_TO_FLOAT[s]
    try:
...
def confidence_to_category(confidence: Optional[float]) -> Optional[str]:
    """Inverse direction of normalize_confidence, for callers that want
    a LOW/MEDIUM/HIGH badge from a normalized float rather than a raw
    number in the text. Boundaries chosen to match the existing
    AIContextResult self-rating convention already in use elsewhere in
    the file, so a Cryptone-derived HIGH and an AI-self-rated HIGH mean
    the same thing once both pass through here."""
    if confidence is None:
        return None
    for threshold, label in _FLOAT_TO_CATEGORY_BOUNDARIES:
...
class TelegramFormatter:
    ...
    def _narrative_clauses(event: "Event") -> List[Tuple[str, str]]:
        ...
        # Full structured mode: show up to 2 items per list (not just
        # the first) so a CONFLICT with multiple contradictions, or a
        # COMPLEMENTARY read with several blind spots, isn't silently
        # truncated to one line — this clause feeds the always-shown
        # "AI Collaboration" card in format_event now, not a
        # space-constrained caption, so there's room for it.
        collab = event.collaboration
        if collab:
            relation = collab.get("relation")
            # P3.8-0 Gap 3 fix: CollaborationResult.thesis (the one-line
            # human synthesis CollaborationEngine already builds) was
            # only ever reaching to_dict() and never getting read back
            # out — decided to keep it, not rewrite it, since it's the
            # "story" lead-in the specific clauses below (contradictions/
            # blind_spots/support) then back up as evidence. Skipped for
            # INSUFFICIENT since _thesis_line's fallback text there
            # ("No usable external context this scan") is filler, not
            # a story worth a line.
            thesis_line = collab.get("thesis")
            if thesis_line and relation != "INSUFFICIENT":
                clauses.append(("ai", TelegramFormatter._escape_md(str(thesis_line)[:200])))
            if relation == "CONFLICT":
                items = (collab.get("contradictions") or [])[:2]
                for text in items:
                    text = TelegramFormatter._escape_md(str(text)[:200])
                    clauses.append(("ai", f"⚠️ AI context conflict: {text}"))
            elif relation == "COMPLEMENTARY":
                items = (collab.get("blind_spots") or collab.get("missing_context") or [])[:2]
                for text in items:
                    text = TelegramFormatter._escape_md(str(text)[:200])
                    clauses.append(("ai", f"AI adds context Cryptone doesn't see: {text}"))
            elif relation == "ALIGNED":
                items = (collab.get("supporting_context") or [])[:2]
                for text in items:
                    text = TelegramFormatter._escape_md(str(text)[:200])
        ...
        # Full structured mode: up to 3 findings, not just the first.
        intel = event.external_intel
        if intel and intel.get("evidence_type") not in (None, "NONE"):
            findings = (intel.get("findings") or [])[:3]
