...
class TelegramFormatter:
    ...
    def format_validation_digest(
        summary: List["StageValidation"],
        lead_time: List["LeadTimeSummary"],
        follow_through: List[Dict],
        horizons_minutes: List[int],
    ) -> Optional[str]:
        """P5 (Validation Visibility digest) — once-daily answer to the
        war-room's actual closing question: 'does Cryptone know something
        before the market shows it obviously, or is it just following?'
        Renders three sections from data that was already being computed
        correctly every scan by ValidationTracker but had zero visible
        output (summary()/summary_by_regime()/follow_through_summary()
        had no caller at all; lead_time_summary() only reached a
        logger.info line). No new computation here — pure formatting
        over already-correct data, same posture as format_batch_watch.

        Returns None (not an empty string) when there's nothing resolved
        yet, so the caller can skip sending entirely rather than push an
        empty digest — same "nothing to report" posture
        _check_economic_calendar already uses for a no-events day.

        Layout, one fenced block per section so all three keep fixed-
        width column alignment on Telegram (no native table support):
          HIT RATE   — per stage, at the shortest configured horizon
                       only (not all horizons) to keep the digest to one
                       screenful; full per-horizon detail is in
                       ValidationTracker.summary() for anyone reading
                       state.json/logs directly, not something this
                       digest tries to replace.
          FOLLOW-THRU — moved-and-stayed-moved rate, only for stages
                       with a qualifying sample (follow_through_summary
                       already excludes stages with none).
          LEAD TIME   — median minutes-before-move and false-anticipation
                       rate, i.e. the actual "did we know early" number.
        """
        if not summary and not lead_time and not follow_through:
            return None

        shortest_h = min(horizons_minutes) if horizons_minutes else None
        lines = ["📈 *Validation Digest* — does Cryptone know early?", ""]

        # --- HIT RATE (shortest horizon only) ---
        hit_rows = [
            s for s in summary
            if s.horizon_minutes == shortest_h and s.sample_count > 0
        ]
        if hit_rows:
            col_stage = max(len("STAGE"), max(len(s.stage) for s in hit_rows))
            header = f"{'STAGE':<{col_stage}} {'N':>4} {'HIT%':>6} {'AVG%':>7}"
            sep = "─" * len(header)
            table = [header, sep]
            for s in hit_rows:
                hit_pct = f"{s.hit_rate * 100:.0f}" if s.hit_rate is not None else "-"
                avg_pct = f"{s.avg_move_pct:+.2f}" if s.avg_move_pct is not None else "-"
                table.append(f"{s.stage:<{col_stage}} {s.sample_count:>4} {hit_pct:>6} {avg_pct:>7}")
            lines.append(f"*Hit rate @ {shortest_h}m*")
            lines.append("```")
            lines.append("\n".join(table))
            lines.append("```")
        ...
        # --- FOLLOW-THROUGH ---
        if follow_through:
            col_stage = max(len("STAGE"), max(len(r["stage"]) for r in follow_through))
            header = f"{'STAGE':<{col_stage}} {'N':>4} {'HOLD%':>6}"
            sep = "─" * len(header)
            table = [header, sep]
            for r in follow_through:
                table.append(
                    f"{r['stage']:<{col_stage}} {r['sample_count']:>4} "
                    f"{r['follow_through_rate'] * 100:>5.0f}%"
                )
            lines.append("*Follow-through (moved & stayed moved)*")
            lines.append("```")
            lines.append("\n".join(table))
            lines.append("```")

        # --- LEAD TIME (the core question) ---
        lt_rows = [s for s in lead_time if s.resolved_count > 0]
        if lt_rows:
            col_stage = max(len("STAGE"), max(len(s.stage) for s in lt_rows))
            header = f"{'STAGE':<{col_stage}} {'N':>4} {'LED':>4} {'MED_LEAD':>8} {'FALSE%':>7}"
            sep = "─" * len(header)
            table = [header, sep]
            for s in lt_rows:
                med = f"{s.median_lead_time_minutes:.0f}m" if s.median_lead_time_minutes is not None else "-"
                false_pct = (
                    f"{s.false_anticipation_rate * 100:.0f}"
                    if s.false_anticipation_rate is not None else "-"
                )
                table.append(
                    f"{s.stage:<{col_stage}} {s.resolved_count:>4} {s.led_count:>4} "
                    f"{med:>8} {false_pct:>7}"
                )
            lines.append("*Lead time (led = crossed threshold before horizon)*")
            lines.append("```")
            lines.append("\n".join(table))
            lines.append("```")

        if len(lines) <= 2:
            # Every section had zero qualifying rows (all sample_count==0
            # or resolved_count==0) even though summary/lead_time weren't
            # both fully empty lists — same "nothing to report" case as
            # the top-level None return, just caught after the loop.
            return None
...
class RadarZone:
    ...
    kind: str            # 'compression' | 'structural_high' | 'structural_low'
                          ...
                          # | 'liquidity_pulled' | 'invalidation' | 'event_origin'
    price_low: float
    price_high: float
    label: str
...
class CryptoneV3:
    ...
    async def initialize(self):
        ...
        # lazily) keeps its lifecycle symmetric with hyperliquid/telegram
        # above and guarantees __aexit__ always has a session to close.
        await self.macro_sentiment.__aenter__()
        await self.news_provider.__aenter__()
        await self.logo_provider.__aenter__()
        await self.economic_calendar.__aenter__()
    ...
    async def _check_validation_digest(self):
        """P5 (Validation Visibility) — once-daily send of
        ValidationTracker's summary/follow-through/lead-time to Telegram.
        Same dedup-by-UTC-date shape as _check_economic_calendar above,
        deliberately copied rather than sharing state with it (separate
        enabled flag + separate sent-date, see ValidationConfig), so one
        digest can be toggled without affecting the other. Purely a read
        + format + send — does not touch ValidationTracker's recording
        logic, which keeps running every scan regardless of this."""
        vc = self.config.validation
        if not vc.enabled or not vc.daily_digest_enabled:
            return

        try:
            now = utc_now()
            if now.hour < vc.daily_digest_hour_utc:
                return
            if self.validation_tracker.daily_digest_already_sent_today():
                return

            text = TelegramFormatter.format_validation_digest(
                self.validation_tracker.summary(),
                self.validation_tracker.lead_time_summary(),
                self.validation_tracker.follow_through_summary(),
                vc.horizons_minutes,
            )
            if text:
                await self.telegram._send_text(text)
                # Success-gated like the calendar digest would be if
                # _send_text returned a status — it currently doesn't
                # (see TelegramAdapter._send_text), so this mirrors the
                # "not configured" no-op case: a misconfigured Telegram
                # just logs the digest instead of sending, same as every
                # other alert path, rather than blocking retries forever.
                self.validation_tracker.mark_daily_digest_sent()
            else:
                # Nothing resolved yet (e.g. bot restarted recently, or
                # still within max_horizon_minutes of the first signal)
                # — mark sent anyway so this doesn't retry every scan
                # until midnight, same "empty day" posture as the
                # calendar digest.
                self.validation_tracker.mark_daily_digest_sent()
        except Exception as e:
        ...
        for candidate in scored_candidates:
            ...
            if candidate.quality > self.opportunity.config.notification_min_quality and candidate.is_active:
                ...
                event = self.event_engine.generate_event(
                    ...
                    expectation_narrative=self.micro.build_expectation_narrative(symbol),
                )
                if event:
