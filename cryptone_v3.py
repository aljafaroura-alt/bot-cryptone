...
class ContextEngine:
    ...
    def _classify_window(self, window: List[Candle], use_span: bool = False) -> Dict:
    ...
    def _get_multi_horizon_context(self, symbol: str, timeframe: Optional[str] = None) -> Dict:
        ...
        for n in cc.horizon_bars:
            if len(candles) < max(cc.min_bars_required, n):
                ...
                continue
            window = candles[-n:]
            verdict = self._classify_window(window, use_span=True)
            verdict['bars'] = n
            horizons[n] = verdict
        ...
        if cc.horizon_discovery_enabled and reconciled != 'INSUFFICIENT_DATA':
            discovery = self._discover_persistence_horizon(candles, cc.horizon_discovery_step)
    ...
    def _discover_persistence_horizon(self, candles: List[Candle], step: int) -> Dict:
        ...
        macro_verdict = self._classify_window(candles[-max_n:], use_span=True)
        macro_trend = macro_verdict['trend']

        onset = None
        n = cc.min_bars_required
        while n <= max_n:
            verdict = self._classify_window(candles[-n:], use_span=True)
...
class TelegramFormatter:
    ...
    def format_batch_watch(events: list) -> str:
        ...
        for e in events:
            ...
            # needed. Extreme readings still get flagged, but by the
            # existing z-score-based CROWDED_LONG/CROWDED_SHORT evidence
            # (already computed per-symbol against that symbol's own
            # funding history) rather than a new one-size-fits-all cutoff.
            if e.funding_rate is not None:
                funding = f"{e.funding_rate:+.4f}"
                if "CROWDED_SHORT" in e.evidence or "CROWDED_LONG" in e.evidence:
                    funding += "⚠️"
            else:
                funding = "-"
