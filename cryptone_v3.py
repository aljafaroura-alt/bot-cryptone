...
class TelegramFormatter:
    ...
    def format_validation_digest(
        ...
        P7 (closes the lateness gap): stage_summaries/lead_time_summaries
        now carry CONFIRMED as up to two rows (path='normal' and
        path='early_confirm', from ValidationTracker.summary()/
        lead_time_summary() calling _summarize/_lead_time_summarize with
        split_confirmed_by_path=True). This renders BOTH sub-rows under
        one 'Confirmed' header when both have samples, so the digest can
        actually show whether the leading-legs path fires earlier/
        cleaner than the old path — instead of blending them back into
        one number, which would hide exactly the thing this split exists
        to reveal. Today (no early-confirm outcomes resolved yet) this
        renders identically to before: one 'normal' row under CONFIRMED,
        no visible change.
        """
        if not stage_summaries:
            return None

        # One row per (stage, path) — pick the shortest horizon with any
        # samples for each, same "read it the way a trader would" reasoning
        # as SymbolFootprint.symbol_footprint. path='' for every stage
        # except a split CONFIRMED, where it's 'normal'/'early_confirm'.
        by_stage_path: Dict[Tuple[str, str], "StageValidation"] = {}
        for s in stage_summaries:
            if s.sample_count <= 0:
                continue
            key = (s.stage, s.path)
            existing = by_stage_path.get(key)
            if existing is None or s.horizon_minutes < existing.horizon_minutes:
                by_stage_path[key] = s
        if not by_stage_path:
            return None

        lead_by_stage_path = {(ls.stage, ls.path): ls for ls in lead_time_summaries}
        displacement_summary = displacement_summary or {}
        stage_order = ["EARLY", "BUILDING", "ACTIVATING", "CONFIRMED", "LATE"]
        # Render order within a stage: '' (unsplit stages carry their one
        # row here), then 'normal', then 'early_confirm' — old-path-first
        # keeps the familiar reading order stable for anyone used to the
        # pre-split digest.
        path_order = ["", "normal", "early_confirm"]
        path_label = {"normal": "normal path", "early_confirm": "early-confirm, leading legs"}

        def _render_row(sv: "StageValidation", label_suffix: str) -> List[str]:
            out = []
            ...
            header = f"*{sv.stage.capitalize()}{label_suffix}*{marker} — n={sv.sample_count} ({sv.horizon_minutes}m horizon)"
            out.append(header)
            out.append(f"  hit rate: {hit_str}  ·  meaningful move: {meaningful_str}")
            lt = lead_by_stage_path.get((sv.stage, sv.path))
            if lt is not None and lt.resolved_count > 0:
                if lt.median_lead_time_minutes is not None:
                    ...
                    out.append(f"  led 0/{lt.resolved_count} — no confirmed anticipation yet")
            ...
            # "there was nothing left to lead." Displacement is only
            # tracked per-stage (not per-path) since it's recorded at the
            # FIRST time an episode is seen, before the path split even
            # applies — shown once under the stage's first rendered row.
            if not label_suffix:
                disp = displacement_summary.get(sv.stage)
        ...
        for stage in stage_order:
            rows_this_stage = [
                (path, by_stage_path[(stage, path)])
                for path in path_order if (stage, path) in by_stage_path
            ]
            if not rows_this_stage:
                continue
            for i, (path, sv) in enumerate(rows_this_stage):
                any_thin = any_thin or sv.sample_count < min_trustworthy_samples
                label_suffix = f" ({path_label[path]})" if path else ""
                lines.extend(_render_row(sv, label_suffix))
                # Displacement note (label_suffix-gated above) only fires
                # on the '' row; for a split CONFIRMED (no '' row exists)
                # show it once, under the first sub-row instead.
                if path and i == 0:
