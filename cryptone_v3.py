...
class TelegramFormatter:
    ...
    def format_event_caption(event: Event) -> str:
        """Short caption for sendPhoto. Full structured mode (per Jafar):
        send_event() now ALWAYS follows this photo with the complete
        format_event() breakdown as its own message, so this caption's
        only job is the at-a-glance read while the chart loads — symbol/
        direction/state/priority, the price-since-detection line, and
        top evidence flags. The narrative/AI sections used to be
        squeezed in here under Telegram's 1024-char caption limit; they
        now live unclipped in the follow-up message instead of being
        truncated to fit.
        """
        priority_emoji = {
            EventPriority.LOW: "ℹ️",
        ...
        # P3.8-0 Gap 4 fix: same VERDICT rating as format_event, compacted
        # to a single emoji badge on the header line since the caption
        # has no room for a labeled line — full "VERDICT: BEST (confirmed)"
        # text follows in the format_event breakdown right after.
        verdict_badge = ""
        if event.thesis_rating:
            verdict_emoji = {"BEST": "🟢", "WATCH": "🟡", "WAIT": "⚪", "IGNORE": "🔴"}
            verdict_badge = f"  {verdict_emoji.get(event.thesis_rating, '')}{event.thesis_rating}"

        lines = [f"{priority_emoji} *{event.symbol} PERP — {TelegramFormatter._humanize(event.state)}{dir_badge}*  Q{event.quality:.2f}{verdict_badge}"]
        ...
        lines.append("👇 full breakdown below")
...
class TelegramAdapter:
    ...
    async def send_event(self, event: Event, chart_bytes: Optional[bytes] = None):
        """Full structured mode (per Jafar): a chart shows price action,
        but it doesn't show regime, target map, footprint, AI
        collaboration, or external intel — so those can't be dropped
        just because a chart was attached. Chart (if any) still goes
        out first with a short caption for the at-a-glance read, then
        the FULL format_event() breakdown always follows as its own
        message — not just as a caption-failure fallback anymore.
        """
        ...
        text = NarrativeRenderer.format_event(event)
