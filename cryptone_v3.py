...
class EvidenceBuilder:
    ...
    def __init__(self, evidence_config: Optional["EvidenceThresholdConfig"] = None):
...
class StateMachine:
    ...
    def _transition_state(self, cand: Candidate, evidence: Dict) -> tuple:
        ...
        if current == PrimaryState.WATCHING:
            ...
            if thesis_score > self.sc.thesis_enter and not evidence['thesis_broken']:
...
class CryptoneV3:
    ...
    def __init__(self, config: Optional[Config] = None):
        ...
        self.evidence_builder = EvidenceBuilder(self.config.evidence)
    ...
    async def run(self, max_duration_seconds: Optional[float] = None):
        ...
        while self.running:
            try:
                ...
                retry_s = self.config.runtime.error_retry_seconds
                logger.error(f"scan #{self.scan_count} crashed: {e} — retrying in {retry_s:.0f}s")
