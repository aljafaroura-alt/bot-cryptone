...
class LiquidityCluster:
    ...
    # P4.1 (Liquidity Persistence): how many CONSECUTIVE scans this same
    # price bucket has classified the same way (THICK stays THICK,
    # THIN stays THIN) — see MicrostructureEngine._liq_cluster_history.
    # 1 means "first time seen this way, this scan" (fresh — could be a
    # spoof that gets pulled next scan); higher means the level has
    # actually held. This is the time dimension the original snapshot-only
    # LiquidityMap was missing (see war-room review: "wall could be
    # pulled a second later, spoof risk indistinguishable from a
    # genuinely defended level").
    persistence_snapshots: int = 1

    # Derived label from persistence_snapshots vs
    # LiquidityMapConfig.persistence_forming_seconds /
    # persistence_defended_seconds (time+depth adaptive) — kept as a field (not computed by
    # callers) so every consumer (narrative, chart, target map) reads
    # the exact same tier boundary instead of each re-deriving it.
    #   FRESH    — just appeared, no track record yet, treat cautiously
    #   FORMING  — held a few scans, building evidence
    #   DEFENDED — held long enough to call it real, not noise/spoof
    confidence: str = "FRESH"

    # True when this cluster is THICK (a wall) on a side whose
    # aggregate depth is CURRENTLY in an open TESTED/PULLED episode
    # (MicrostructureEngine._liq_test_state / _liq_test_state_ask) —
    # i.e. the side-level flow state machine is showing this side is
    # actively being sold/bought into right now, which is exactly the
    # condition under which a resting wall is most likely to get pulled
    # rather than defended. None when there's no test-state signal to
    # cross-check against (state machine has no baseline yet for this
    # symbol) — distinct from False (checked, and no test is open).
...
class LiquidityMap:
    ...
    def walls_above(self, min_confidence: Optional[str] = None) -> List[LiquidityCluster]:
        """THICK ask clusters, nearest-to-price first — candidate
        resistance/targets on the upside.

        min_confidence (None/'FORMING'/'DEFENDED') filters out clusters
        below that persistence tier — see LiquidityCluster.confidence.
        Default None returns everything, same behavior as before P4.1,
        so existing callers that haven't opted into the filter are
        unaffected; callers that want the "was this actually tested,
        not just glimpsed once" read pass min_confidence='FORMING' or
        stricter."""
        tiers = {"FRESH": 0, "FORMING": 1, "DEFENDED": 2}
        floor = tiers.get(min_confidence, -1)
        return sorted(
            (c for c in self.ask_clusters if c.kind == "THICK" and tiers.get(c.confidence, 0) >= floor),
            key=lambda c: c.pct_from_mid,
        )

    def walls_below(self, min_confidence: Optional[str] = None) -> List[LiquidityCluster]:
        """THICK bid clusters, nearest-to-price first — candidate
        support/targets on the downside. See walls_above for
        min_confidence."""
        tiers = {"FRESH": 0, "FORMING": 1, "DEFENDED": 2}
        floor = tiers.get(min_confidence, -1)
        return sorted(
            (c for c in self.bid_clusters if c.kind == "THICK" and tiers.get(c.confidence, 0) >= floor),
            key=lambda c: -c.pct_from_mid,
        )
    ...
    def path_description(self, direction: str) -> str:
        ...
        P4.1 (Liquidity Persistence): the wall reported is the nearest
        wall with confidence >= FORMING (held long enough per the
        adaptive persistence_forming_seconds window),
        not simply the nearest THICK cluster regardless of age. This is
        the fix for the exact failure mode the war-room review flagged:
        without this filter, a wall that appeared one scan ago (possible
        spoof, never tested) would get announced identically to one
        that's held for 30 — "resistance wall immediately" reads the
        same confident way either time even though one is far more
        likely to vanish before price gets there. A FRESH-only wall
        still gets mentioned, but honestly labeled as such rather than
        silently promoted to the same footing as a defended one. When a
        THICK cluster is at_risk (its side is currently under an open
        TESTED/PULLED episode), that's called out too — that's the
        "was real, may be getting pulled right now" case, distinct from
        both FRESH and a quiet DEFENDED wall.
        """
        if not self.is_valid:
            return "liquidity path unknown — orderbook not available"

        walls_all = self.walls_above() if direction == "up" else self.walls_below()
        walls_tested = self.walls_above(min_confidence="FORMING") if direction == "up" else self.walls_below(min_confidence="FORMING")
        ...
        # Prefer the nearest wall that's actually held for at least one
        # more scan than the moment it appeared; if none has, fall back
        # to the nearest raw THICK cluster but label it FRESH so the
        # reader knows it's an unconfirmed read, not a defended level.
        nearest = walls_tested[0] if walls_tested else walls_all[0]
        ...
        conf_note = {
            "FRESH": " — just appeared, unconfirmed",
            "FORMING": " — forming, a few scans held",
            "DEFENDED": " — defended, held multiple scans",
        }.get(nearest.confidence, "")
        risk_note = ""
        if nearest.at_risk:
            risk_note = "; side currently under active test — may not hold"
        ...
        return (
            ...
            f"{thin_note}{conf_note}{risk_note}"
        )
...
class TargetMapReading:
    ...
    # P4.1 (Liquidity Persistence): confidence tier and raw scan count of
    # whichever wall nearest_wall_price/pct refer to — carried as their
    # own fields (not just embedded in description text) so any
    # downstream consumer (validation digest, chart color, a future
    # scoring gate) can read the number directly instead of parsing the
    # sentence. None when no wall is behind this reading at all (OPEN/
    # STRUCTURAL_ONLY/UNKNOWN states).
    nearest_wall_confidence: Optional[str] = None  # 'FRESH' | 'FORMING' | 'DEFENDED'
    nearest_wall_persistence_snapshots: Optional[int] = None
    nearest_wall_at_risk: Optional[bool] = None
    ...
    def to_dict(self) -> Dict[str, object]:
        ...
        return {
            ...
            "nearest_wall_confidence": self.nearest_wall_confidence,
            "nearest_wall_persistence_snapshots": self.nearest_wall_persistence_snapshots,
            "nearest_wall_at_risk": self.nearest_wall_at_risk,
        }
...
def build_target_map(
    ...
    reviewer's proposal objected to replacing):
      ...
      PARTIALLY_DEFENDED   — a wall exists but there's thin depth before
                              ...
                              description text already), OR the nearest
                              wall is close but still FRESH (P4.1:
                              unconfirmed, held for under
                              the adaptive persistence_forming_seconds window) — a wall
                              that just appeared isn't "DEFENDED" in the
                              sense a trader means by that word, it's a
                              claim that hasn't been tested yet
      DEFENDED             — a wall sits close/immediately ahead AND has
                              confidence >= FORMING (held at least
                              long enough per the adaptive persistence_forming_seconds
                              window) — the P4.1 fix for the exact
                              "golden-signal-looking but actually a
                              spoof" gap the war-room review raised
    ...
    if liquidity_map is not None and liquidity_map.is_valid:
        ...
        if not walls:
            ...
            nearest_persistence = None
            nearest_at_risk = None
        else:
            # P4.1: pick the same nearest-CONFIRMED-wall path_description
            # itself uses (min_confidence='FORMING'), falling back to the
            # raw nearest THICK cluster only if nothing has cleared that
            # bar yet — keeps nearest_wall_price/pct in this reading
            # pointed at the same wall the description text refers to,
            # rather than the two silently disagreeing.
            walls_tested = liquidity_map.walls_above(min_confidence="FORMING") if lm_dir == "up" else liquidity_map.walls_below(min_confidence="FORMING")
            nearest = walls_tested[0] if walls_tested else walls[0]
            nearest_pct = abs(nearest.pct_from_mid)
            nearest_price = nearest.price_high if lm_dir == "up" else nearest.price_low
            nearest_confidence = nearest.confidence
            nearest_persistence = nearest.persistence_snapshots
            nearest_at_risk = nearest.at_risk
            if nearest.confidence == "FRESH":
                # Close in price but not yet proven in time — same
                # "not fully trustworthy yet" bucket as thin-before-wall,
                # rather than being labeled DEFENDED on price distance
                # alone.
        ...
        return TargetMapReading(
            ...
            nearest_wall_confidence=nearest_confidence,
            nearest_wall_persistence_snapshots=nearest_persistence,
            nearest_wall_at_risk=nearest_at_risk,
        )
...
class RadarZone:
    ...
    # P4.1 (Liquidity Persistence): set only on liquidity_wall/
    # liquidity_gap zones, mirrors LiquidityCluster.confidence
    # ('FRESH'/'FORMING'/'DEFENDED'). ChartRenderer uses this to draw a
    # FRESH wall visually distinct from a DEFENDED one (see
    # render_radar_chart) instead of every wall rendering with identical
    # weight regardless of whether it's actually been tested.
    confidence: Optional[str] = None
    # True when the wall's side has an open TESTED/PULLED episode right
    # now (LiquidityCluster.at_risk) — independent of confidence, since
    # a long-DEFENDED wall can still be actively under test this scan.
    at_risk: Optional[bool] = None
...
class RadarZones:
    ...
    candles: the zone overlays themselves plus the header/footer text
    context and WS/BOOK/REST freshness — composed once per HIGH/CRITICAL
    ...
    P3.8-D: thesis_rating/market_agreement_status/macro_trend/
    compression_state/setup_stage — previously five separate text
    fields threaded through from compute_radar_zones's own kwargs —
    are gone. The chart now reads the exact same MarketIntelligence
    Event.market_intelligence already carries (built once in
    EventEngine.generate_event, see that method's docstring), pulling
    the equivalent values off Insight.evidence tags (thesis_rating=X
    on .thesis, macro_trend=X/setup_stage=X/compression_state=X on
    .structure) instead of a second, chart-only computation of the
    same facts. Forward-ref string because MarketIntelligence is
    defined later in the file (P3.8-A), same reason Event.
    market_intelligence uses one.
    """
...
def compute_radar_zones(
    ...
    MicrostructureEngine's liquidity_flow/LiquidityMap) — there is no
    "Claude thinks price will do X" annotation anywhere in this layer,
    per the ChartConfig comment.

    market_intelligence (P3.8-D) is passed straight through to the
    returned RadarZones unmodified — no zone geometry is derived from
    it here, it's just threaded so render_radar_chart's header/badge
    text can read the same MarketIntelligence Event carries, instead
    of this function computing a second, chart-only copy of thesis_
    rating/market_agreement/macro_trend/setup_stage/compression_state
    the way it used to.
    ...
    if liquidity_map is not None and liquidity_map.is_valid:
        # P4.1: prefer the nearest wall that's cleared FORMING (held at
        # the adaptive persistence_forming_seconds window) over the raw nearest
        # THICK cluster — same "don't chart a one-scan-old blip with the
        # same visual weight as a defended level" fix as
        # LiquidityMap.path_description/build_target_map above. Falls
        # back to the raw nearest wall (still charted, just tagged
        # FRESH) only when nothing on that side has any track record
        # yet, so a genuinely brand-new session isn't left with an empty
        # chart while history accumulates.
        above_tested = liquidity_map.walls_above(min_confidence="FORMING")
        below_tested = liquidity_map.walls_below(min_confidence="FORMING")
        above_pick = (above_tested or liquidity_map.walls_above())[:1]
        below_pick = (below_tested or liquidity_map.walls_below())[:1]
        for cluster in above_pick + below_pick:
            side_word = "ASK WALL" if cluster.side == "ask" else "BID WALL"
            risk_tag = " \u26a0" if cluster.at_risk else ""  # ⚠ under active test
            zones.append(RadarZone(
                kind="liquidity_wall", price_low=cluster.price_low, price_high=cluster.price_high,
                label=f"{side_word} {abs(cluster.pct_from_mid):.2f}% \u00b7 {cluster.relative_depth:.1f}x \u00b7 {cluster.confidence}{risk_tag}",
                confidence=cluster.confidence, at_risk=cluster.at_risk,
            ))
        ...
        if thin_clusters:
            ...
            zones.append(RadarZone(
                ...
                confidence=c.confidence, at_risk=c.at_risk,
            ))
...
class Candidate:
    ...
    # P3.8-0 Gap 8 fix: True when PreMove and Reversal named opposite
    # directions this scan. Ephemeral like the two fields above —
    # recomputed each scan, not carried across restarts.
    pre_move_reversal_conflict: bool = False
...
class Event:
    ...
    # P3.8-0 Gap 4 fix: rate_thesis()'s BEST/WATCH/WAIT/IGNORE verdict and
    # the MarketAgreement status behind it were already wired into the
    # chart's Market Agreement badge (now read from MarketIntelligence.
    # thesis.evidence's thesis_rating=X tag, see P3.8-D / compute_radar_
    # zones's market_intelligence param) but never carried onto Event, so
    # format_event's Telegram text had no way to know what the chart
    # badge next to it was already saying.
    # Same "carry the value, format at render time" posture as every
    # other Optional field on this dataclass — plain str, not the enum,
    # to keep Event trivially serializable.
    ...
    # P3.8-C (Narrative Renderer wiring): MarketIntelligenceBuilder's
    # output for this event's candidate, built once at generate_event()
    # time (Candidate is in scope there) and carried whole — not
    # flattened to a dict like collaboration/external_intel/
    # narrative_watch above, because unlike those this never needs to
    # cross a JSON boundary; it's read straight back out by
    # NarrativeRenderer at send time. Forward-ref string because
    # MarketIntelligence is defined later in the file (P3.8-A), same
    # reason market_breadth above uses one for "MarketBreadthReading".
    # None whenever build() itself isn't reached (shouldn't normally
    # happen — build() has no failure branches — but kept Optional for
    # the same backfill-safety posture as every other engine-read field
    # on this dataclass, and so old code paths that construct an Event
    # by hand elsewhere still work unchanged).
    market_intelligence: Optional["MarketIntelligence"] = None
...
class AnchorConfig:
    ...
    enabled: bool = True
    # True unless the operator pinned anchors explicitly via
    # ANCHOR_SYMBOLS. When True, CryptoneV3.initialize() replaces this
    # placeholder list with the top-N symbols by OI+volume from the
    # first live snapshot — so "which coins are majors" tracks the
    # actual market instead of a literal someone typed in 2025.
    auto: bool = True
...
class LiquidityMapConfig:
    ...
    # P3.8-1 (Adaptive Liquidity Persistence): FRESH/FORMING/DEFENDED is
    # now decided on WALL-CLOCK elapsed time a bucket has held its
    # classification, not raw scan count — the original count-based
    # design (persistence_forming/defended_snapshots) baked in an
    # assumption about scan_interval that would silently go stale the
    # moment discovery.scan_interval changed (concern #1 from the
    # laporan). Time-based windows are cadence-agnostic by construction:
    # whether Cryptone scans every 10s or every 60s, "held for 2
    # minutes" means the same thing either way.
    persistence_forming_seconds: float = 60.0
    persistence_defended_seconds: float = 300.0

    # Depth-adaptive discount: the elapsed-time bar above is the price
    # for a BORDERLINE wall (relative_depth just over thick_multiplier).
    # A wall sitting far above that bar is already strong evidence on
    # its own — this is the same relative_depth number ChartRenderer
    # uses to decide dash-width/hatch styling (see the P4.1 chart
    # section), reused here rather than introducing a second, unrelated
    # notion of "how thick is this wall". A cluster at/above
    # persistence_depth_strength_ref gets its required window shrunk to
    # persistence_depth_min_discount of the base; anything at or below
    # thick_multiplier gets no discount (1.0x); linear in between.
    persistence_depth_strength_ref: float = 3.0
    persistence_depth_min_discount: float = 0.5

    # Hard floor regardless of elapsed time or depth discount — guards
    # against a pathologically fast scan_interval (e.g. testing/backfill
    # runs) letting a wall reach DEFENDED off two back-to-back scans a
    # few seconds apart just because the wall-clock window technically
    # elapsed. A streak still needs at least this many consecutive scans.
    persistence_min_snapshots_floor: int = 2

    # Ceiling on how many distinct (side, bucket_idx) keys
    # _liq_cluster_history tracks per symbol — bounds memory for
    # symbols whose mid price (and therefore bucket indexing) drifts a
    # lot over time. Oldest-untouched keys are pruned first each scan
    # (see get_liquidity_map's history bookkeeping).
    max_history_buckets_per_symbol: int = 200
...
class RegimeEngine:
    ...
    def export_state(self) -> dict:
        """P3.8-0 Gap 1 fix: checkpoint per-symbol vol_history so the
        volatility percentile axis doesn't reset to UNKNOWN_VOL and
        require vol_history_min_samples fresh scans after every
        restart. Same shape convention as every other export_state
        here: plain-JSON-able dict, deque -> list."""
        return {sym: list(hist) for sym, hist in self._vol_history.items()}

    def load_state(self, state: dict) -> None:
        """Tolerant of a missing/old-format state (empty dict/None) —
        same fail-soft posture as every other load_state here; a
        symbol just re-warms its percentile from zero, same as today's
        cold-start behavior, rather than raising."""
        if not state:
            return
        maxlen = self.rc.vol_history_maxlen
        for sym, hist in state.items():
            self._vol_history[sym] = deque(hist, maxlen=maxlen)
...
class WebSocketConfig:
    ...
    # P3.8-0 Gap 13 fix: ping_interval/ping_timeout only catch a
    # connection where the SERVER stops responding to pings — a TCP
    # connection that's gone half-open (socket still reports open, but
    # no data flows either direction) can sit indefinitely without
    # tripping either. This is a second, independent staleness check:
    # if no message of ANY kind (not just trades — subscriptionResponse/
    # pong count too) has arrived in this many seconds, force-close and
    # let the existing reconnect loop take over. Set well above
    # ping_interval + ping_timeout so it only fires for genuinely
    # half-open connections, not slow-but-alive ones.
    watchdog_stale_seconds: float = 90.0
...
class RuntimeConfig:
    ...
    # P3.8-0 audit fix (Gap 18): with no ceiling, a permanent error (bad
    # API key, expired credential, endpoint moved) just retries forever
    # at retry_backoff_max forever, burning the runner/quota with no one
    # noticing. 0 disables the cap (old behavior) for anyone who wants
    # that back deliberately.
    max_consecutive_scan_failures: int = 20
...
class Config:
    ...
    def from_env(cls) -> "Config":
        ...
        # If the operator didn't pin ANCHOR_SYMBOLS explicitly, treat the
        # literal above as a placeholder only — initialize() will replace
        # it with a live OI+volume ranking once the first snapshot is in.
        anchors_auto = "ANCHOR_SYMBOLS" not in os.environ
        ...
        return cls(
            ...
            anchors=AnchorConfig(symbols=anchor_symbols, auto=anchors_auto),
...
# P3.8-0 Gap 16 fix: opt-in structured (JSON) logging, one line per
# record, for anyone piping this into ELK/Datadog/CloudWatch etc.
# Deliberately NOT the default — the colored _CleanFormatter above is
# what makes a live terminal session actually readable during
# development, and this file has no evidence anyone's tailing it into
# a log pipeline today. Opt in with CRYPTONE_JSON_LOGS=1; everyone else
# gets identical output to before this fix, nothing changes by default.
...
if os.environ.get("CRYPTONE_JSON_LOGS", "").strip() in ("1", "true", "True"):
...
class WSConnection:
    ...
    async def _connect_with_reconnect(self):
        ...
        while self._running:
            try:
                ...
                # P3.8-0 Gap 13 fix: run the staleness watchdog
                # concurrently with the message loop. Whichever finishes
                # first (message_loop exiting on its own, or the
                # watchdog force-closing a half-open socket) cancels the
                # other — this is what actually forces _message_loop's
                # `async for` to raise and fall through to the existing
                ...
                message_task = asyncio.create_task(self._message_loop())
                    ...
                    {watchdog_task, message_task}, return_when=asyncio.FIRST_COMPLETED,
                )
                for t in pending:
                    t.cancel()
                for t in done:
    ...
    async def _staleness_watchdog(self):
        """P3.8-0 Gap 13 fix: periodically checks last_message_at (set on
        EVERY message, including pong/subscriptionResponse — not just
        trades) and force-closes the socket if nothing has arrived in
        watchdog_stale_seconds. websockets' own ping_interval/ping_timeout
        already handles a server that stops answering pings; this covers
        the other half-open case — a TCP connection that still reports
        as open while carrying no data in either direction, which
        wouldn't otherwise trip until some much longer OS-level TCP
        timeout. Runs until cancelled (by the message loop finishing, or
        by disconnect()).
        """
        while True:
            await asyncio.sleep(self.wc.watchdog_check_interval)
            if self.last_message_at is None:
                continue  # hasn't received a first message yet — not "stale", just new
            age = (utc_now() - self.last_message_at).total_seconds()
            if age > self.wc.watchdog_stale_seconds:
                logger.warning(
                    f"WS watchdog: no message in {age:.0f}s (limit "
                    f"{self.wc.watchdog_stale_seconds:.0f}s) — connection looks "
                    f"half-open, forcing reconnect"
                )
                if self.websocket:
...
class MicrostructureEngine:
    def __init__(
        ...
        # P4.1 (Liquidity Persistence): per-symbol, per-(side, bucket_idx)
        # consecutive-scan counters feeding LiquidityCluster.persistence_
        # snapshots/confidence. This is intentionally the SAME kind of
        # in-progress, cheap-to-rebuild tracker as _liq_test_state above
        # (see export_state's docstring for why those aren't persisted
        # across restarts) — a cold start just means every wall reads
        # FRESH for a handful of scans until the history repopulates,
        # not a lasting signal-quality cost like the EMA baselines are.
        # Value dict per key: {'kind': 'THICK'/'THIN', 'count': int,
        # 'last_seen_scan': int} — last_seen_scan lets stale keys (a
        # bucket that stopped appearing because price moved away) get
        # pruned instead of accumulating forever.
        self._liq_cluster_history: Dict[str, Dict[Tuple[str, int], Dict]] = defaultdict(dict)
        self._liq_scan_counter: Dict[str, int] = defaultdict(int)

        # P3.8-1 (Wall Mention Instrumentation): cumulative counter of
        # how many times a TargetMapReading's nearest_wall_confidence
        # actually shaped an outbound reading — tag='watching' for the
        # liquidity-only read generate_event() attaches to every
        # candidate, tag='chart' for the structural-enriched read that
        # replaces it for HIGH/CRITICAL events that get charted (see
        # both build_target_map call sites). This is deliberately
        # observation-only — nothing here changes what fires or gates
        # an alert (additive-only: StateMachine/scoring is untouched) —
        # it exists so laporan concern #2 ("does the FRESH filter
        # actually cut convincing-but-fragile notifs, or need more
        # tuning at the gating layer") can be checked against real
        # numbers over a few days of running instead of guessed at.
        # Persisted via export_state/load_state below so the count
        # survives the ~5.8h self-relaunch cycle instead of resetting
        # every run, since the whole point is a multi-day trend.
        self._liq_wall_mention_stats: Dict[str, Dict[str, int]] = {
            "watching": defaultdict(int),
    ...
    @staticmethod
    def _dt_to_iso(d: dict) -> dict:
        """Datetime values -> isoformat strings, one level deep — enough
        for the flat dicts used by _expectation_history entries."""
        out = {}
        for k, v in d.items():
    ...
    @staticmethod
    def _dt_from_iso(d: dict, keys: Tuple[str, ...]) -> dict:
        out = dict(d)
        for k in keys:
            if out.get(k):
                try:
    ...
    def export_state(self) -> dict:
        """P3.8-0 Gap 1 fix. Deliberately narrow scope: only the two
        pieces of state here that are actually slow to rebuild and
        matter for signal quality —
          - liquidity EMA baselines (_liq_baseline_bid/ask): take many
            snapshots to converge, discounting liquidity-behavior
            evidence (TESTED/REPLENISHED/PULLED) until they do
          - resolved expectation history (_expectation_history): P4
            lead-time/hit-rate validation data, genuinely lost forever
            if not saved, not just re-derivable from live data
        Deliberately NOT persisted: trades/orderbooks/window_trades
        (seconds-old live buffers, correctly empty on a fresh process
        anyway), _liq_test_state/_liq_test_state_ask/_expectations
        (in-progress trackers that resolve within a handful of
        snapshots — restarting mid-test just means one test result is
        lost, not a lasting cold-start cost like the baselines are).
        """
        return {
            "liq_baseline_bid": dict(self._liq_baseline_bid),
            "liq_baseline_ask": dict(self._liq_baseline_ask),
            "expectation_history": {
                sym: [self._dt_to_iso(e) for e in hist]
                for sym, hist in self._expectation_history.items()
            },
            # P3.8-1: cumulative, small (3 keys x 2 tags), cheap to
            # persist every checkpoint alongside everything else here.
            "liq_wall_mention_stats": {
                tag: dict(counts) for tag, counts in self._liq_wall_mention_stats.items()
            },
        }

    def record_wall_mention(self, confidence: Optional[str], tag: str = "watching") -> None:
        """P3.8-1: call this once per TargetMapReading that actually got
        used (attached to a candidate/event), passing its
        nearest_wall_confidence. None (no wall on that side, or no valid
        liquidity map) is recorded under its own "none" bucket rather
        than silently dropped, so the denominator in wall_mention_summary
        reflects every reading, not just the ones with a wall."""
        bucket = self._liq_wall_mention_stats.setdefault(tag, defaultdict(int))
        bucket[confidence or "none"] += 1

    def wall_mention_summary(self) -> str:
        """P3.8-1: human-readable cumulative mix, e.g.
        'chart: FRESH 12% FORMING 31% DEFENDED 57% (n=210) | watching: ...'
        Meant for a once-per-checkpoint log line (see CryptoneV3.save_state)
        — this is the thing to eyeball over a few days to answer laporan
        concern #2 for real, instead of guessing."""
        parts = []
        for tag in ("chart", "watching"):
            counts = self._liq_wall_mention_stats.get(tag) or {}
            total = sum(counts.values())
            if total == 0:
                continue
            bits = " ".join(
                f"{label} {counts.get(label, 0) / total * 100:.0f}%"
                for label in ("FRESH", "FORMING", "DEFENDED", "none")
                if counts.get(label, 0) > 0
            )
            parts.append(f"{tag}: {bits} (n={total})")
        return " | ".join(parts)

    def load_state(self, state: dict) -> None:
        if not state:
            return
        self._liq_baseline_bid.update(state.get("liq_baseline_bid") or {})
        self._liq_baseline_ask.update(state.get("liq_baseline_ask") or {})
        for tag, counts in (state.get("liq_wall_mention_stats") or {}).items():
            bucket = self._liq_wall_mention_stats.setdefault(tag, defaultdict(int))
            for label, n in (counts or {}).items():
                bucket[label] += n
        maxlen = self.mc.expectation_history_maxlen
        for sym, hist in (state.get("expectation_history") or {}).items():
            self._expectation_history[sym] = deque(
                (self._dt_from_iso(e, ("opened_at", "resolved_at")) for e in hist),
                maxlen=maxlen,
            )
    ...
    def cleanup(self, stale_seconds: Optional[int] = None):
        ...
        for sym in stale_symbols:
            ...
            # P4.1 (Liquidity Persistence) state
            self._liq_cluster_history.pop(sym, None)
            self._liq_scan_counter.pop(sym, None)
    ...
    def _adaptive_persistence_windows(self, relative_depth: float) -> Tuple[float, float]:
        """P3.8-1 (Adaptive Liquidity Persistence): turns the fixed
        persistence_forming/defended_seconds base windows into a
        per-cluster required window, discounted by how strong the wall
        already looks. relative_depth is the exact number ChartRenderer
        uses to pick wall dash-width/hatch (see the P4.1 chart section)
        — reusing it here means "how convincing is this wall" is
        computed once and read the same way by the confidence label and
        by the chart, instead of two independently-tuned notions of wall
        strength drifting apart over time.

        A cluster right at thick_multiplier (the bare minimum to count
        as THICK at all) gets no discount — it still has to sit through
        the full base window before earning FORMING/DEFENDED, same as
        the original design. A cluster at or above
        persistence_depth_strength_ref is already strong enough evidence
        that it only needs persistence_depth_min_discount of the base
        window. Linear in between; relative_depth below thick_multiplier
        shouldn't reach this function (not classified THICK), but is
        clamped to 1.0x discount defensively anyway.
        """
        lmc = self.lmc
        span = lmc.persistence_depth_strength_ref - lmc.thick_multiplier
        if span <= 0:
            discount = 1.0
        else:
            t = (relative_depth - lmc.thick_multiplier) / span
            t = max(0.0, min(1.0, t))
            discount = 1.0 - t * (1.0 - lmc.persistence_depth_min_discount)
        return (
            lmc.persistence_forming_seconds * discount,
            lmc.persistence_defended_seconds * discount,
        )
    ...
    def get_liquidity_map(self, symbol: str) -> "LiquidityMap":
        ...
        # P4.1 (Liquidity Persistence): advance this symbol's scan
        # counter once per get_liquidity_map call (not per side) so
        # bid/ask history entries touched in the same call share one
        # "last_seen_scan" clock — see the pruning step below.
        self._liq_scan_counter[symbol] += 1
        scan_no = self._liq_scan_counter[symbol]
        history = self._liq_cluster_history[symbol]
        ...
        def _bucket_side(levels: List["OrderBookLevel"], side: str) -> Tuple[List[LiquidityCluster], bool]:
            ...
            # P4.1 (Liquidity Persistence): update each individual
            # bucket idx's history BEFORE merging changes their grouping
            # — persistence tracks the raw (side, idx) key, which is
            # stable scan-to-scan regardless of how neighboring buckets
            # happen to merge this particular scan (a merge span
            # shifting by one bucket shouldn't reset an otherwise-held
            # level's track record).
            for idx_low, idx_high, kind in raw:
                for idx in range(idx_low, idx_high + 1):
                    key = (side, idx)
                    entry = history.get(key)
                    # A streak only continues if this bucket classified
                    # the SAME way on the IMMEDIATELY PRECEDING scan
                    # (scan_no - 1). A gap of even one scan — the bucket
                    # briefly stopped being THICK/THIN, or price moved
                    # off it and back — means whatever was there before
                    # is not evidence this new appearance is defended;
                    # count restarts at 1. Without this, a wall that
                    # flickers on/off could accumulate persistence as if
                    # it had held continuously, which would silently
                    # defeat the whole point of this counter (telling
                    # flicker/spoof apart from something actually resting).
                    if (
                        entry is not None
                        and entry["kind"] == kind
                        and entry["last_seen_scan"] == scan_no - 1
                    ):
                        entry["count"] += 1
                        # first_seen_ts is NOT touched on a continuing
                        # streak — it's the anchor the elapsed-time check
                        # below measures from, so it should only move
                        # when the streak actually restarts.
                    else:
                        entry = {"kind": kind, "count": 1, "first_seen_ts": now}
                    entry["last_seen_scan"] = scan_no
                    history[key] = entry
            ...
            for idx_low, idx_high, kind in raw:
                ...
                # Cluster-level persistence = the WEAKEST (minimum) of its
                # constituent buckets' streaks, not an average — a span
                # where one bucket just flipped THICK this scan while its
                # neighbor has held for 10 hasn't collectively "proven
                # itself" yet; reporting the min keeps confidence honest
                # rather than letting one long-lived bucket launder a
                # freshly-appeared neighbor's lack of history.
                persistence = min(
                    history[(side, i)]["count"] for i in range(idx_low, idx_high + 1)
                )
                # P3.8-1: elapsed wall-clock time of the WEAKEST
                # (most-recently-restarted) constituent bucket — same
                # weakest-link logic as the count-based persistence
                # above, just measured in seconds instead of scans so
                # the result doesn't depend on how often Cryptone
                # happens to scan (see LiquidityMapConfig.persistence_
                # forming_seconds docstring).
                elapsed_seconds = min(
                    (now - history[(side, i)]["first_seen_ts"]).total_seconds()
                    for i in range(idx_low, idx_high + 1)
                )
                forming_win, defended_win = self._adaptive_persistence_windows(relative)
                floor = lmc.persistence_min_snapshots_floor
                if persistence >= floor and elapsed_seconds >= defended_win:
                    confidence = "DEFENDED"
                elif persistence >= floor and elapsed_seconds >= forming_win:
                    confidence = "FORMING"
                else:
                    confidence = "FRESH"
                ...
                clusters.append(LiquidityCluster(
                    ...
                    persistence_snapshots=persistence, confidence=confidence,
        ...
        # P4.1 (Liquidity Persistence): cross-check THICK walls against
        # the side-level flow state machine (_classify_liquidity_side's
        # TESTED/REPLENISHED/PULLED, see _liq_test_state/_liq_test_state_
        # ask) — this is the second, independent confidence input
        # alongside per-bucket age: a wall can look old (high
        # persistence_snapshots) yet still sit on a side whose aggregate
        # depth the flow engine currently shows being actively sold/
        # bought into. at_risk=True on a THICK cluster means "the
        # side-level signal says this side is under active pressure right
        # now" — a caller combining this with a low persistence_snapshots
        # count has the strongest reason to treat the wall as spoof-risk;
        # a DEFENDED cluster with at_risk=True is the "was real, is now
        # being tested" case, which is its own distinct, useful read.
        # at_risk stays None (not False) when the state machine has no
        # baseline yet for this symbol/side — "unknown" is not the same
        # claim as "checked, nothing open".
        bid_test_open = symbol in self._liq_test_state
        ask_test_open = symbol in self._liq_test_state_ask
        bid_has_baseline = symbol in self._liq_baseline_bid
        ask_has_baseline = symbol in self._liq_baseline_ask
        for c in bid_clusters:
            if c.kind == "THICK":
                c.at_risk = bid_test_open if bid_has_baseline else None
        for c in ask_clusters:
            if c.kind == "THICK":
                c.at_risk = ask_test_open if ask_has_baseline else None

        # P4.1 pruning: drop (side, idx) history keys not touched this
        # scan or the prior one (mid can drift, shifting which idx a
        # given price maps to — a key that stops appearing is a stale
        # bucket, not a level that's still resting) and, as a hard
        # ceiling, oldest-touched keys beyond max_history_buckets_per_
        # symbol so a symbol whose mid drifts constantly can't grow this
        # dict unbounded between cleanup() passes.
        stale_keys = [k for k, v in history.items() if scan_no - v["last_seen_scan"] > 1]
        for k in stale_keys:
            del history[k]
        if len(history) > lmc.max_history_buckets_per_symbol:
            overflow = len(history) - lmc.max_history_buckets_per_symbol
            oldest = sorted(history.items(), key=lambda kv: kv[1]["last_seen_scan"])[:overflow]
            for k, _ in oldest:
                del history[k]
...
class MarketBreadthProvider:
    ...
    # P3.8-0 Gap 12 fix: CoinGecko free tier is ~50 req/min — fine for
    # one call per cache_seconds window, but a fleet of bots or a lot
    # of restarts in quick succession can share that budget thin.
    # CoinCap's /v2/assets has no global-summary endpoint, so this
    # approximates: BTC's own marketCapUsd / sum of the top N assets'
    # marketCapUsd for dominance, no 24h-change field available this
    # way (that field stays None on the fallback path — partial data,
    # not fabricated data, same posture as every other Optional field
    # in this file).
    COINCAP_ENDPOINT = "https://api.coincap.io/v2/assets?limit=100"
        ...
        try:
            async with self._session.get(
                ...
                if resp.status != 200:
                    ...
                    logger.warning(f"MarketBreadthProvider: {self._last_error}, trying CoinCap fallback")
                    return await self._fetch_coincap_fallback(now)
                raw = await resp.json()
            ...
            logger.warning(f"MarketBreadthProvider fetch failed: {self._last_error}, trying CoinCap fallback")
            return await self._fetch_coincap_fallback(now)

    async def _fetch_coincap_fallback(self, now: datetime) -> Optional["MarketBreadthReading"]:
        """P3.8-0 Gap 12 fix: CoinGecko failed this call — try CoinCap
        before giving up rather than going dark until the next cache
        window. Dominance is approximated as BTC's marketCapUsd over
        the sum of the top 100 assets by rank (CoinCap has no single
        global-summary endpoint the way CoinGecko does), so it can
        differ slightly from CoinGecko's own denominator (full market
        vs top-100) — close enough for a directional "rotating in/out
        of BTC" read, which is all this reading is used for downstream.
        No 24h-change field is available this way, so
        market_cap_change_24h_pct stays None on this path — partial
        data, same contract as CoinGecko's own optional fields above,
        never fabricated.
        """
        try:
            async with self._session.get(
                self.COINCAP_ENDPOINT, timeout=aiohttp.ClientTimeout(total=8)
            ) as resp:
                if resp.status != 200:
                    self._last_error = f"coincap fallback HTTP {resp.status}"
                    logger.warning(f"MarketBreadthProvider: {self._last_error} — no reading this cycle")
                    return None
                raw = await resp.json()

            assets = raw.get("data", []) if isinstance(raw, dict) else []
            if not assets:
                self._last_error = "coincap fallback: empty asset list"
                return None
            ...
            for a in assets:
                try:
                    mcap = float(a.get("marketCapUsd") or 0)
            ...
            if total_mcap <= 0 or btc_mcap is None:
                self._last_error = "coincap fallback: could not derive BTC dominance"
                logger.warning(f"MarketBreadthProvider: {self._last_error}")
                return None

            reading = MarketBreadthReading(
                btc_dominance_pct=100.0 * btc_mcap / total_mcap,
                total_market_cap_usd=None,  # top-100 sum isn't a real total-market figure
                market_cap_change_24h_pct=None,
                fetched_at=now,
            )
            self._cached_reading = reading
            self._cached_at = now
            logger.info("MarketBreadthProvider: served from CoinCap fallback this cycle")
            return reading
        except Exception as e:
            self._last_error = f"coincap fallback {type(e).__name__}: {e}"
            logger.warning(f"MarketBreadthProvider: fallback also failed: {self._last_error}")
            return None
...
class EconomicCalendarProvider:
    ...
    async def _get_cached_events(self) -> List[EconomicEvent]:
        ...
        if events:
            ...
            # P3.8-0 Gap 11 fix: first-ever fetch failed outright and
            # there's no prior cache to fall back to — _fetch_all
            # already logged the underlying HTTP/parse error, but that
            # alone doesn't tell an operator whether the bot is quietly
            # degraded (serving stale-but-still-useful cache) or fully
            # blind (zero economic events, no fallback source). This
            # makes the "fully blind" case explicit rather than a
            # silent empty list.
            logger.warning(
                "EconomicCalendarProvider: no cached events available and "
                "first fetch failed — economic calendar reminders/digest "
                "will be empty until the next successful fetch"
            )
            return []
        else:
            logger.info(
                f"EconomicCalendarProvider: fetch failed, serving cached "
                f"events from {self._cached_at.isoformat() if self._cached_at else 'unknown'} "
                f"(may be stale)"
            )
        return self._cached_events
...
class MacroBiasEngine:
    ...
    def __init__(self, config: Config, symbol_bias_cooldown_seconds: float = 300.0):
        ...
        # P3.8-0 Gap 10 fix: compute() below used to await
        # compute_symbol_bias one symbol at a time — up to 10 fully
        # sequential news/RSS round-trips per scan, serialized for no
        # reason (the original audit assumed the opposite problem —
        # too much concurrency — but the actual code had none at all).
        # Bounded to 3 concurrent lookups: fast enough to matter for
        # scan cadence, low enough to stay well under NewsProvider's/
        # RSS's own rate limits.
        self._symbol_bias_semaphore = asyncio.Semaphore(3)
    ...
    async def compute(
        ...
        # P3.8-0 Gap 10 fix: cache check stays synchronous/sequential
        # (cheap, no I/O) so cooldown hits never touch the semaphore;
        # only symbols that actually need a fresh fetch go through
        # bounded-concurrency compute_symbol_bias calls.
        to_fetch: List[str] = []
        cached_results: Dict[str, Tuple[Optional[str], float, Optional[str]]] = {}
        for symbol in candidate_symbols[:max_news_lookups]:
            ...
            if cached is not None and (now - cached[0]).total_seconds() < self.symbol_bias_cooldown_seconds:
                ...
                cached_results[symbol] = (direction, confidence, reason)
            else:
                to_fetch.append(symbol)

        async def _bounded_fetch(symbol: str):
            async with self._symbol_bias_semaphore:
                return symbol, await self.compute_symbol_bias(symbol, news, rss, llm)

        if to_fetch:
            fetched = await asyncio.gather(*(_bounded_fetch(s) for s in to_fetch))
            for symbol, (direction, confidence, reason) in fetched:
                self._symbol_bias_cache[symbol] = (now, direction, confidence, reason)
                cached_results[symbol] = (direction, confidence, reason)

        for symbol in candidate_symbols[:max_news_lookups]:
            direction, confidence, reason = cached_results.get(symbol, (None, 0.0, None))
...
class CompressionFeature:
    ...
    def export_state(self) -> dict:
        """P3.8-0 Gap 1 fix: same shape/reasoning as RegimeEngine's —
        without this, lifecycle_state's percentile read cold-starts
        every restart even though the underlying candle history it's
        classifying is unaffected."""
        return {sym: list(hist) for sym, hist in self._ratio_history.items()}

    def load_state(self, state: dict) -> None:
        if not state:
            return
        maxlen = self.cc.adaptive_history_maxlen
        for sym, hist in state.items():
            self._ratio_history[sym] = deque(hist, maxlen=maxlen)
...
class CorrelationEngine:
    ...
    def export_state(self) -> dict:
        """P3.8-0 Gap 1 fix: without this, every decouple episode in
        progress at shutdown is forgotten on restart — min_decouple_seconds
        starts counting from zero again even for a symbol that was
        already most of the way to confirming a decouple. Tuple keys
        aren't JSON-safe, so they're joined with a separator that can't
        appear in a symbol/anchor name (both are uppercase tickers);
        started_at (datetime) -> isoformat."""
        out = {}
        for (symbol, anchor), info in self._decouple_state.items():
            out[f"{symbol}::{anchor}"] = {
        ...
        for key, info in state.items():
            ...
            if info.get("started_at"):
                try:
                    ...
                    started_at = None
            self._decouple_state[(symbol, anchor)] = {
...
class PreMoveEngine:
    ...
    def export_state(self) -> dict:
        """P3.8-0 Gap 1 fix: same shape as RegimeEngine/CompressionFeature.
        Without this, the per-symbol early-confirm leading-count
        threshold resets to early_confirm_default_leading_floor every
        restart until early_confirm_min_history fresh scans accumulate
        again — a real cost for a symbol that had already learned its
        own threshold."""
        return {sym: list(hist) for sym, hist in self._leading_count_history.items()}

    def load_state(self, state: dict) -> None:
        if not state:
            return
        maxlen = self.pc.early_confirm_history_maxlen
        for sym, hist in state.items():
            self._leading_count_history[sym] = deque(hist, maxlen=maxlen)
...
# =====================================================================
# P3.8-0 Gap 1 & 2: shared vocabulary for anything reading across
# engines (MarketIntelligenceBuilder and friends). Additive-only —
# no existing engine's internal state/decision logic is touched here.
# Individual engines keep emitting their own native vocab (float
# confidence, "LOW"/"MEDIUM"/"HIGH" strings, INSUFFICIENT_DATA,
# UNKNOWN_UNKNOWN, ...); these two helpers translate at the boundary
# instead of forcing a rewrite of 29 call sites in one risky pass.
# =====================================================================

class DataState(str, Enum):
    """Canonical replacement for the 6 "we don't have enough to say
    anything yet" spellings found scattered across the file:
    INSUFFICIENT_DATA (17x), INSUFFICIENT_SWINGS (4x), the regime
    engine's UNKNOWN_UNKNOWN, ExternalIntel.evidence_type == "NONE",
    CollaborationRelation.INSUFFICIENT, and CheapScreening's own
    spread_baseline_ready boolean. Anything building a cross-engine
    Insight.state should land here instead of re-deriving "should I
    show this" logic per engine.
    """
    READY = "READY"              # engine has enough to make a real read
    WARMING_UP = "WARMING_UP"    # actively collecting, not yet enough
    INSUFFICIENT = "INSUFFICIENT"  # tried, genuinely came back empty
    UNKNOWN = "UNKNOWN"          # engine couldn't classify (e.g. regime UNKNOWN_UNKNOWN)
    ABSENT = "ABSENT"            # feature/engine not enabled or not applicable this scan

    @classmethod
    def from_legacy(cls, value: object) -> "DataState":
        """Best-effort translation of one of the 6 legacy spellings
        into the canonical enum. Falls back to READY for anything that
        looks like a real value (not a sentinel) rather than guessing
        wrong toward INSUFFICIENT — a caller with an actual reading
        should never get silently downgraded to "no data"."""
        if value is None:
            return cls.ABSENT
        s = value.value if isinstance(value, Enum) else str(value)
        s = s.upper()
        if s in ("INSUFFICIENT_DATA", "INSUFFICIENT_SWINGS", "INSUFFICIENT"):
            return cls.INSUFFICIENT
        if s == "UNKNOWN_UNKNOWN" or s == "UNKNOWN":
            return cls.UNKNOWN
        if s in ("NONE", "ABSENT", "DISABLED", "NOT_APPLICABLE"):
            return cls.ABSENT
        if s in ("WARMING_UP", "WARMUP", "BUILDING"):
            return cls.WARMING_UP
        return cls.READY
...
class CheapScreeningEngine:
    ...
    def export_state(self) -> dict:
        """Checkpoint the per-symbol spread history so a fresh process
        (next GH Actions relaunch, crash recovery, etc.) doesn't throw
        away an already-warm LIQUIDITY baseline and re-pay the ~5-sample
        cold start on every single restart. Same posture as
        BaselineEngine.export_state() — plain data, no fabrication."""
        return {
            symbol: list(hist)
            for symbol, hist in self._spread_history.items()
            if hist
        }
    ...
    def load_state(self, state: dict) -> None:
        ...
        for symbol, values in state.items():
            try:
                ...
                for v in values[-50:]:  # respect maxlen=50
                    hist.append(float(v))
            except Exception:
                continue
...
# =====================================================================
# P3.8-A — INTELLIGENCE SCHEMA
#
# The intermediate language Candidate -> MarketIntelligenceBuilder (P3.8-B,
# next) -> {NarrativeRenderer, ChartRenderer} (P3.8-C/D) both read, instead
# of each formatter reaching into Candidate's raw engine fields directly.
#
# Scope discipline per the roadmap: this phase adds STRUCTURE only.
# Nothing here changes what any engine computes, how the state machine
# transitions, what AI is allowed to say, or what gets sent today —
# format_event/format_event_caption/render_radar_chart are all
# completely untouched by this section. MarketIntelligence isn't wired
# to anything yet; building the translator (Candidate -> this schema)
# is P3.8-B, and switching the renderers over to read from it is
# P3.8-C/D. Additive-only, same posture as every P3.8-0 plumbing fix.
# =====================================================================

class InsightImportance(str, Enum):
    """Presentation priority — deliberately NOT a trading score. This
    never feeds back into direction/confidence/state-machine decisions;
    it only controls how much room an Insight takes in the rendered
    output and whether it's shown at all when space/attention is
    limited. A CONFLICT or REVERSAL_CONFIRMED earns CRITICAL not
    because it's "more true" than a NORMAL funding reading, but
    because a trader needs to see it first.
    """
    CRITICAL = "CRITICAL"  # e.g. PREMOVE_REVERSAL_CONFLICT, thesis CONTRADICTED, liquidity collapse
    HIGH = "HIGH"          # e.g. THESIS_BEST, new external catalyst, reversal confirmed
    NORMAL = "NORMAL"      # the default — most Insights land here
    LOW = "LOW"            # e.g. funding/spread/correlation reading within normal range


class InsightType(str, Enum):
    """The eight narrative sections from the roadmap doc's WHAT/WHY/
    CONTEXT/WHAT-CHANGED/EXPECTATION/RISK/VERDICT structure, plus
    STRUCTURE (market structure — the piece the doc's chart section
    calls out separately from WHAT/situation) and EVIDENCE (the
    supporting-detail list, rendered differently from the eight prose
    sections — bullet clauses, not paragraphs).
    One InsightType per MarketIntelligence field of the same name,
    kept as its own enum (rather than just using the field name as a
    string) so a renderer can group/order/style by type without
    string-matching field names.
    """
    SITUATION = "SITUATION"      # WHAT — the headline read: what's happening right now
    STRUCTURE = "STRUCTURE"      # market structure: swing highs/lows, liquidity zones, regime
    THESIS = "THESIS"            # WHY — the belief + market-agreement synthesis (rate_thesis's territory)
    CONTEXT = "CONTEXT"          # external intelligence: news, calendar, macro bias — AI's contribution
    EXTERNAL = "EXTERNAL"        # WHAT CHANGED — new footprint since last scan (state transitions, new evidence)
    EXPECTATION = "EXPECTATION"  # what happens next if the thesis holds
    RISK = "RISK"                # what would invalidate the thesis
    RESOLUTION = "RESOLUTION"    # VERDICT — the final call (BEST/WATCH/WAIT/IGNORE + one line why)


@dataclass
class EvidenceInsight:
    """One item in MarketIntelligence.evidence — the bullet-level
    supporting detail (today's evidence_labels list, e.g.
    "THESIS_BEST", "PREMOVE_REVERSAL_CONFLICT") given the same
    title/narrative/importance/state shape as a full Insight instead
    of being a bare string, so a renderer can filter/sort/style
    evidence the same way it does the eight main sections — without
    forcing every one-line fact into a full paragraph the way a
    top-level Insight would.
    """
    label: str                              # short machine-stable tag, e.g. "THESIS_BEST" (matches today's evidence_labels vocabulary)
    narrative: str                          # one line, human-readable: "Thesis rated BEST — strong belief, market confirms"
    importance: InsightImportance = InsightImportance.NORMAL
...
@dataclass
class Insight:
    """One of MarketIntelligence's eight narrative sections. Per the
    roadmap doc's explicit framing ("engine menghasilkan makna, bukan
    cuma angka") — this carries a human-readable narrative sentence,
    not just a raw field value a renderer would have to phrase itself.

    state uses the canonical DataState enum from P3.8-0 (Gap 2) rather
    than a ninth ad-hoc vocabulary — this is exactly the reuse the
    plumbing pass was for. A renderer's "show vs skip" decision (the
    doc's point #5: ABSENT should mean "don't print AI: NONE") is a
    single `insight.state == DataState.ABSENT` check, not a per-section
    special case.

    evidence here is a list of short string tags (not full
    EvidenceInsight objects — those live at the MarketIntelligence
    level) for the specific facts THIS section's narrative was built
    from, e.g. THESIS's evidence might be ["engine_belief=STRONG",
    "market_agreement=CONFIRMED"]. Lets a renderer show "why" on demand
    (tooltip/expand) without it being baked into the prose narrative.
    """
    type: InsightType
    title: str                              # short label, e.g. "Situation", "Thesis", "Risk"
    narrative: str                          # the human-readable sentence(s) — "" when state is ABSENT/INSUFFICIENT and there's nothing to say
    importance: InsightImportance = InsightImportance.NORMAL
    state: DataState = DataState.READY
    evidence: List[str] = field(default_factory=list)

    def has_content(self) -> bool:
        """A renderer's canonical check for whether this section is
        worth printing at all — the doc's #5 point made structural:
        ABSENT/INSUFFICIENT sections with no narrative should be
        skippable in one call, not a per-renderer if-chain repeated in
        both NarrativeRenderer and ChartRenderer."""
        return bool(self.narrative) and self.state not in (DataState.ABSENT, DataState.INSUFFICIENT)
...
class MarketIntelligence:
    """The intermediate language itself — per the doc's section 2,
    semantic sections rather than a flat dashboard-shaped dataclass.
    Field order matches the doc's WHAT/WHY/CONTEXT/WHAT-CHANGED/
    EXPECTATION/RISK/VERDICT narrative order (structure inserted after
    situation, matching the chart section's Header/Zones/Narrative
    split) — a renderer iterating fields in declaration order gets the
    right narrative order for free.

    One instance per symbol per scan, built fresh each time (P3.8-B's
    job) — same "recomputed each scan, not carried across restarts"
    posture as Candidate's own ephemeral fields (engine_belief,
    market_agreement, thesis_rating, pre_move_reversal_conflict).
    """
    symbol: str

    situation: Insight      # WHAT
    structure: Insight      # market structure (swings, zones, regime)
    thesis: Insight          # WHY
    context: Insight        # CONTEXT — external/AI intelligence
    external: Insight       # WHAT CHANGED
    expectation: Insight    # EXPECTATION
    risk: Insight            # RISK
    resolution: Insight     # VERDICT

    evidence: List[EvidenceInsight] = field(default_factory=list)

    generated_at: datetime = field(default_factory=utc_now)

    def sections(self) -> List[Insight]:
        """The eight narrative Insights in doc-specified render order —
        the one place that order is defined, so NarrativeRenderer and
        ChartRenderer can't silently drift apart on section ordering."""
        return [
            self.situation, self.structure, self.thesis, self.context,
            self.external, self.expectation, self.risk, self.resolution,
        ]

    def visible_sections(self) -> List[Insight]:
        """sections() filtered to has_content() — the doc's point #5
        made concrete: a renderer asking "what do I actually have to
        say" gets it in one call instead of re-deriving the ABSENT/
        INSUFFICIENT skip logic itself."""
        return [s for s in self.sections() if s.has_content()]

    def to_dict(self) -> dict:
        return {
            "symbol": self.symbol,
            "sections": {s.type.value: s.to_dict() for s in self.sections()},
...
# =====================================================================
# P3.8-B — INTELLIGENCE BUILDER
#
# Candidate -> MarketIntelligence. Pure translation: reads existing
# Candidate fields (pre_move, reversal, regime, engine_belief,
# market_agreement, thesis_rating, collaboration_result, external_intel,
# pre_move_reversal_conflict, episode_stage, ...) and phrases them as
# the eight narrative sections + evidence list. Per the roadmap's
# explicit P3.8-E regression contract: engine behavior, state machine,
# decisions, and AI authority are all UNCHANGED by this class — it
# only reads Candidate, never writes back to it, and nothing calls
# this yet (format_event/render_radar_chart are untouched — that
# wiring is P3.8-C/D).
# =====================================================================

class MarketIntelligenceBuilder:
    """Stateless translator — every method here is a pure function of
    its Candidate argument (plus, where noted, static class data).
    Split into one _build_x per Insight so each section's phrasing
    logic can be read/tested/adjusted independently, matching
    MarketIntelligence.sections()'s field-per-concern shape."""

    # ---- thesis_rating -> presentation importance -----------------
    # Deliberately its own mapping, not reused from anywhere in the
    # ThesisRating/rate_thesis code above — that table encodes trading
    # meaning (belief x market agreement), this one encodes attention
    # priority. IGNORE gets CRITICAL not because IGNORE is "bad" but
    # because a market actively contradicting a belief is exactly the
    # kind of thing a trader needs to see FIRST, per the roadmap doc's
    # #7 example ("CONFLICT... bisa naik ke atas").
    _THESIS_IMPORTANCE = {
        ThesisRating.BEST: InsightImportance.HIGH,
        ThesisRating.WATCH: InsightImportance.NORMAL,
        ThesisRating.WAIT: InsightImportance.LOW,
        ThesisRating.IGNORE: InsightImportance.CRITICAL,
    }
    ...
    def build(
        ...
        *,
        ...
        """All keyword-only params beyond candidate are per-scan-cycle
        data that lives on MarketData/scan context, not on Candidate
        itself (funding_rate, target_map, symbol_footprint, macro
        sentiment/bias, news, compression) — same reason they were
        already separate kwargs on EventEngine.generate_event rather
        than Candidate fields. Builder stays a pure function; it just
        now takes a couple more explicit inputs instead of silently
        having no way to see them. Every one is Optional/default-None
        so build() called with just a candidate (existing tests, any
        future caller that doesn't have this scan-cycle context) still
        works unchanged.

        compression is NOT read off candidate.pre_move.compression
        even though that field exists — pre_move is None for plenty of
        candidates (reversal-only, dormant) while a compression reading
        is fetched unconditionally every scan cycle regardless of
        pre_move firing. Reading through pre_move would silently go
        missing exactly when a candidate has no pre_move, which is a
        real regression risk for a value ChartRenderer's header line
        depends on (P3.8-D). Passed explicitly instead, sourced from
        the same compression_reading variable the chart itself reads.

        market_structure_ctx (macro_trend/setup_stage) does NOT need
        an equivalent kwarg — unlike compression, it's already stored
        unconditionally on candidate.market_structure by the scan loop
        (candidate.market_structure = context, set every scan before
        generate_event() runs), so _build_structure reads it straight
        off the candidate like every other field here.
        """
        return MarketIntelligence(
            symbol=candidate.symbol,
            situation=cls._build_situation(candidate),
            structure=cls._build_structure(candidate, compression=compression),
            thesis=cls._build_thesis(candidate),
            ...
            external=cls._build_external(candidate),
            expectation=cls._build_expectation(candidate, target_map=target_map),
            risk=cls._build_risk(candidate, funding_rate=funding_rate),
            resolution=cls._build_resolution(candidate),
            evidence=cls._build_evidence(candidate),
        )

    # ---- WHAT -------------------------------------------------------
    @staticmethod
    def _build_situation(c: Candidate) -> Insight:
        """The headline read. Prefers PreMove (positioning building)
        over Reversal (exhaustion of an existing move) when both are
        present and non-conflicting — PreMove describes what's
        forming, which is usually the more actionable "what's
        happening" framing; Reversal's read shows up in risk/context
        instead when it disagrees (see pre_move_reversal_conflict
        handling in _build_risk)."""
        pm, rev = c.pre_move, c.reversal
        if pm is not None and pm.direction:
            narrative = (
                f"{c.symbol} is showing {pm.stage.lower()}-stage {pm.direction} "
                f"positioning."
            )
            importance = (
                InsightImportance.HIGH if pm.stage in ("ACTIVATING", "CONFIRMED")
                else InsightImportance.NORMAL
            )
            return Insight(
                type=InsightType.SITUATION, title="Situation", narrative=narrative,
                importance=importance, state=DataState.READY,
                evidence=list(pm.evidence),
            )
        if rev is not None and rev.direction:
            narrative = f"{c.symbol} is showing a {rev.play.replace('_', ' ').lower()} setup ({rev.stage.lower()})."
            importance = InsightImportance.HIGH if rev.stage == "CONFIRMED" else InsightImportance.NORMAL
            return Insight(
                type=InsightType.SITUATION, title="Situation", narrative=narrative,
                importance=importance, state=DataState.READY,
                evidence=list(rev.evidence),
            )
        # Neither engine has a directional read this scan — still say
        # something (candidate.state is always known), rather than
        # leaving the headline section blank.
        return Insight(
            type=InsightType.SITUATION, title="Situation",
            narrative=f"{c.symbol} is being tracked ({c.state.value.lower()}), no directional positioning read yet.",
            importance=InsightImportance.LOW,
            state=DataState.WARMING_UP if c.is_fresh else DataState.INSUFFICIENT,
        )

    # ---- market structure --------------------------------------------
    @staticmethod
    def _build_structure(c: Candidate, *, compression: Optional["CompressionReading"] = None) -> Insight:
        """market_structure_ctx (macro_trend/setup_stage) and compression
        (compression_state) are folded into evidence tags here — NOT
        into the narrative sentence itself, which still describes only
        c.regime same as before. Two deliberate reasons: (1) regime and
        the 1H/15M context read are computed by different engines
        (RegimeEngine vs ContextEngine) and can legitimately disagree
        slightly on wording — mashing them into one prose sentence
        would read like a single engine's contradiction rather than
        two independent reads; (2) this is P3.8-D's actual purpose —
        the chart's 1H/5M/15M header line reads these exact tags
        instead of the RadarZones fields it used to carry separately,
        so the values MUST match tag-for-tag what the chart used to
        show, not a rephrased approximation.
        """
        r = c.regime
        evidence: List[str] = []
        market_structure_ctx = c.market_structure or {}
        macro_trend = market_structure_ctx.get("trend")
        setup_stage = market_structure_ctx.get("15m_structure") or market_structure_ctx.get("15m_trend")
        compression_state = None
        if compression is not None:
            compression_state = "DISPLACED" if compression.is_displaced else (
                "COMPRESSED" if compression.is_compressed else "NEUTRAL"
            )
        if macro_trend:
            evidence.append(f"macro_trend={macro_trend}")
        if setup_stage:
            evidence.append(f"setup_stage={setup_stage}")
        if compression_state:
            evidence.append(f"compression_state={compression_state}")

        if r is None:
            return Insight(
                type=InsightType.STRUCTURE, title="Structure", narrative="",
                state=DataState.ABSENT, evidence=evidence,
            )
        state = DataState.from_legacy(r.trend_state)
        if state == DataState.UNKNOWN:
            return Insight(
                type=InsightType.STRUCTURE, title="Structure",
                narrative="Structure not yet classified — insufficient history to read trend/volatility state.",
                importance=InsightImportance.LOW, state=DataState.UNKNOWN, evidence=evidence,
            )
        narrative = f"Regime: {r.regime.replace('_', ' ').title()}."
        if r.structure_label:
            narrative += f" Structure: {r.structure_label.replace('_', ' ')}."
        if r.vol_percentile is not None and r.vol_samples > 0:
            narrative += f" Volatility at the {r.vol_percentile * 100:.0f}th percentile of its own history."
        return Insight(
            type=InsightType.STRUCTURE, title="Structure", narrative=narrative,
            importance=InsightImportance.NORMAL, state=DataState.READY, evidence=evidence,
        )

    # ---- WHY ----------------------------------------------------------
    @staticmethod
    def _build_thesis(c: Candidate) -> Insight:
        if c.thesis_rating is None or c.engine_belief is None or c.market_agreement is None:
            return Insight(type=InsightType.THESIS, title="Thesis", narrative="", state=DataState.ABSENT)
        belief, agreement, rating = c.engine_belief, c.market_agreement, c.thesis_rating
        narrative = (
            f"Engine belief is {belief.level.lower()}; market is "
            f"{agreement.status.lower()}"
        )
        if agreement.status == "CONTRADICTED" and agreement.contradicting_signal:
            narrative += f" — {agreement.contradicting_signal.replace('_', ' ')} disagrees with the setup"
        narrative += f". Thesis rated {rating.value}."
        # thesis_rating=X added (P3.8-D): this is the exact tag
        # ChartRenderer's Market Agreement badge now reads for its
        # color lookup (rating_style dict keyed on BEST/WATCH/WAIT/
        # IGNORE) — same value RadarZones.thesis_rating used to carry
        # as a dedicated field before this consolidation.
        evidence = [
            f"thesis_rating={rating.value}",
            f"engine_belief={belief.level}",
            f"market_agreement={agreement.status}",
        ]
        if agreement.contradicting_signal:
            evidence.append(f"contradicting_signal={agreement.contradicting_signal}")
        return Insight(
            type=InsightType.THESIS, title="Thesis", narrative=narrative,
            importance=MarketIntelligenceBuilder._THESIS_IMPORTANCE.get(rating, InsightImportance.NORMAL),
            state=DataState.READY, evidence=evidence,
        )

    # ---- CONTEXT (external / AI / macro / history) ---------------------
    @staticmethod
    def _build_context(
        c: Candidate,
        *,
        ...
        """Per the roadmap's #4/#5: AI shows up only when it adds
        something, never as a bare score, and ABSENT means skip —
        never print a literal "AI: NONE". Widened beyond the original
        collaboration/external_intel-only version to also fold in
        symbol_footprint (historical track record), macro sentiment
        (Fear & Greed), macro bias (AI news-driven directional read),
        and the raw news headline — all four were previously
        format_event-only cards with no MarketIntelligence equivalent,
        which meant NarrativeRenderer silently dropped them even
        though the underlying engines still computed them. Same "only
        speak when there's something worth saying" posture as the
        original two clauses.
        """
        collab = c.collaboration_result
        intel = c.external_intel
        parts: List[str] = []
        evidence: List[str] = []
        has_ai_signal = False

        if collab is not None and collab.relation != CollaborationRelation.INSUFFICIENT and collab.thesis:
            parts.append(collab.thesis)
            evidence.append(f"collaboration={collab.relation.value}")

        if intel is not None and intel.evidence_type != "NONE" and intel.findings:
            parts.append(intel.relevance or "; ".join(intel.findings[:2]))
            evidence.append(f"external_intel_confidence={intel.confidence}")
            has_ai_signal = True

        # News headline — only when there's an actual headline; source
        # is a citation detail folded into the same sentence rather
        # than a separate line, matching how AI bias below cites its
        # own reasoning inline instead of as a footnote.
        if news_headline:
            news_part = f'News: "{news_headline}"'
            if news_source:
                news_part += f" ({news_source})"
            parts.append(news_part)
            evidence.append("news_headline=present")

        # AI news-driven directional bias — distinct from collaboration
        # above (that's engine<->AI synthesis on THIS candidate; this
        # is AI's independent read of news flow). Only shown once it
        # cleared its own confidence gate upstream (direction is None
        # otherwise), so no threshold re-litigated here.
        if macro_bias_direction:
            bias_part = f"AI news bias leans {macro_bias_direction}"
            if macro_bias_confidence is not None:
                bias_part += f" ({macro_bias_confidence:.0%} confidence)"
            if macro_bias_reason:
                bias_part += f" — {macro_bias_reason}"
            parts.append(bias_part)
            evidence.append(f"macro_bias={macro_bias_direction}")
            has_ai_signal = True

        # Macro sentiment (Fear & Greed) — backdrop, not a signal about
        # this symbol specifically, so it reads as context rather than
        # thesis support. Label already human-readable from the
        # provider (e.g. "Extreme Fear").
        if macro_sentiment_label:
            sentiment_part = f"Macro backdrop: {macro_sentiment_label}"
            if macro_sentiment_value is not None:
                sentiment_part += f" ({macro_sentiment_value})"
            parts.append(sentiment_part)
            evidence.append(f"macro_sentiment={macro_sentiment_label}")

        # Historical footprint — only surfaced when the underlying
        # ValidationTracker/SymbolFootprint.describe() judged the
        # sample meaningful enough to have a description at all (same
        # gate format_event's old "Track record" card used); Builder
        # doesn't re-derive that threshold, just trusts the caller
        # already applied it upstream (footprint_min_samples).
        if symbol_footprint and symbol_footprint.get("description"):
            parts.append(f"Track record: {symbol_footprint['description']}")
            evidence.append("symbol_footprint=present")

        if not parts:
            return Insight(type=InsightType.CONTEXT, title="Context", narrative="", state=DataState.ABSENT)

        importance = InsightImportance.HIGH if has_ai_signal else InsightImportance.NORMAL
        # Each clause above is a separate observation (news, AI bias,
        # macro backdrop, track record) rather than one continuous
        # thought the way SITUATION/THESIS's sentences build on each
        # other — joined with ". " (each clause period-terminated) so
        # they read as a short list of distinct facts, not one run-on
        # sentence mashing four unrelated subjects together.
        narrative = ". ".join(p.rstrip(".") for p in parts) + "."
        return Insight(
            type=InsightType.CONTEXT, title="Context", narrative=narrative,
            importance=importance, state=DataState.READY, evidence=evidence,
        )

    # ---- WHAT CHANGED ---------------------------------------------------
    @staticmethod
    def _build_external(c: Candidate) -> Insight:
        """Best-effort "what's new" read using only what's on Candidate
        itself (no access to last scan's snapshot from this pure
        function) — episode_stage vs episode_notified_stage mismatch
        is the one reliable signal available here for "something just
        changed enough to matter." A true diff against the prior scan
        belongs in P3.8-C's renderer, which does have access to
        StateMachine history; this section stays conservative rather
        than fabricate a "what changed" narrative from a single
        snapshot."""
        if c.episode_id is None:
            return Insight(type=InsightType.EXTERNAL, title="What Changed", narrative="", state=DataState.ABSENT)
        if c.episode_stage and c.episode_stage != c.episode_notified_stage:
            return Insight(
                type=InsightType.EXTERNAL, title="What Changed",
                narrative=f"Episode entered {c.episode_stage.replace('_', ' ').lower()} stage.",
                importance=InsightImportance.HIGH, state=DataState.READY,
            )
        return Insight(
            type=InsightType.EXTERNAL, title="What Changed", narrative="",
            state=DataState.INSUFFICIENT,  # episode exists but nothing new to report this scan
        )

    # ---- EXPECTATION ------------------------------------------------------
    @staticmethod
    def _build_expectation(c: Candidate, *, target_map: Optional[Dict[str, object]] = None) -> Insight:
        """target_map folded in as a second clause on top of the
        pre_move/reversal projection — "if the thesis holds, here's
        the path" is exactly what EXPECTATION means, and path_state
        (OPEN/DEFENDED/PARTIALLY_DEFENDED/STRUCTURAL_ONLY) was
        previously its own format_event card with no MarketIntelligence
        home. UNKNOWN path_state is treated as absent, same as the old
        card's `!= "UNKNOWN"` gate."""
        pm, rev = c.pre_move, c.reversal
        target_clause = None
        if target_map and target_map.get("path_state") not in (None, "UNKNOWN"):
            target_clause = f"Path: {target_map.get('description')}"

        if pm is not None and pm.direction:
            narrative = (
                f"If {pm.stage.lower()}-stage positioning matures toward "
                f"CONFIRMED, {pm.direction} continuation becomes more plausible."
            )
            if target_clause:
                narrative += f" {target_clause}."
            return Insight(
                type=InsightType.EXPECTATION, title="Expectation", narrative=narrative,
                importance=InsightImportance.NORMAL, state=DataState.READY,
            )
        if rev is not None and rev.direction:
            narrative = f"If the {rev.play.replace('_', ' ').lower()} confirms, expect a {rev.direction} move."
            if target_clause:
                narrative += f" {target_clause}."
            return Insight(
                type=InsightType.EXPECTATION, title="Expectation", narrative=narrative,
        ...
        # No directional projection from PreMove/Reversal, but the
        # target map may still have something to say about the path on
        # its own (e.g. a structural wall sitting nearby regardless of
        # which engine is driving the thesis).
        if target_clause:
            return Insight(
                type=InsightType.EXPECTATION, title="Expectation", narrative=f"{target_clause}.",
                importance=InsightImportance.NORMAL, state=DataState.READY,
            )
        return Insight(type=InsightType.EXPECTATION, title="Expectation", narrative="", state=DataState.ABSENT)

    # ---- RISK -------------------------------------------------------------
    @staticmethod
    def _build_risk(c: Candidate, *, funding_rate: Optional[float] = None) -> Insight:
        parts: List[str] = []
        evidence: List[str] = []
        importance = InsightImportance.NORMAL

        if c.pre_move_reversal_conflict:
            parts.append(
                "PreMove and Reversal are naming opposite directions this "
                "scan — two engines disagree on where this goes next."
            )
            evidence.append("pre_move_reversal_conflict=true")
            importance = InsightImportance.CRITICAL

        if c.market_agreement is not None and c.market_agreement.status == "CONTRADICTED":
            sig = c.market_agreement.contradicting_signal or "market behavior"
            parts.append(f"Market is actively contradicting the belief via {sig.replace('_', ' ')}.")
            importance = InsightImportance.CRITICAL

        # Funding crowding — reuses EngineBelief.crowded_expansion/
        # exhaustion_risk (already-computed flags, same ones _build_
        # thesis's belief.level draws on) rather than re-deriving a
        # threshold from the raw funding_rate number here. crowded
        # alone is a mild warning (late entrants risk); crowded WITH
        # absorption fighting the aggressive side (exhaustion_risk) is
        # the sharper "late-long/late-short risk" read the old
        # format_event card gave — same distinction, same source.
        belief = c.engine_belief
        if belief is not None and belief.crowded_expansion:
            crowd_dir = c.direction or "position"
            funding_str = f" ({funding_rate:.4f})" if funding_rate is not None else ""
            if belief.exhaustion_risk:
                parts.append(
                    f"Funding is crowded {crowd_dir}{funding_str} and absorption is "
                    f"fighting the aggressive side — late-{crowd_dir} exhaustion risk."
                )
                importance = InsightImportance.CRITICAL
            else:
                parts.append(f"Funding is crowded {crowd_dir}{funding_str} — late-{crowd_dir} risk.")
            evidence.append("crowded_expansion=true")

        invalidation = None
        if c.pre_move is not None and c.pre_move.invalidation_condition:
            invalidation = c.pre_move.invalidation_condition
        elif c.reversal is not None and c.reversal.invalidation_condition:
            invalidation = c.reversal.invalidation_condition
        if invalidation:
            parts.append(f"Invalidated if: {invalidation}")

        if not parts:
            return Insight(type=InsightType.RISK, title="Risk", narrative="", state=DataState.ABSENT)
        return Insight(
            type=InsightType.RISK, title="Risk", narrative=" ".join(parts),
            importance=importance, state=DataState.READY, evidence=evidence,
        )

    # ---- VERDICT ------------------------------------------------------------
    @staticmethod
    def _build_resolution(c: Candidate) -> Insight:
        if c.thesis_rating is None:
            return Insight(
                type=InsightType.RESOLUTION, title="Verdict",
                narrative=f"No thesis rating yet — {c.state.value.lower()}.",
                importance=InsightImportance.LOW, state=DataState.INSUFFICIENT,
            )
        direction_txt = f" {c.direction.upper()}" if c.direction else ""
        narrative = f"{c.thesis_rating.value}{direction_txt}"
        if c.market_agreement is not None:
            narrative += f" ({c.market_agreement.status.lower()})"
        return Insight(
            type=InsightType.RESOLUTION, title="Verdict", narrative=narrative,
            importance=MarketIntelligenceBuilder._THESIS_IMPORTANCE.get(c.thesis_rating, InsightImportance.NORMAL),
            state=DataState.READY,
        )

    # ---- evidence list --------------------------------------------------
    @staticmethod
    def _build_evidence(c: Candidate) -> List[EvidenceInsight]:
        out: List[EvidenceInsight] = []

        if c.thesis_rating is not None:
            out.append(EvidenceInsight(
                label=f"THESIS_{c.thesis_rating.value}",
                narrative=f"Thesis rated {c.thesis_rating.value}",
                importance=MarketIntelligenceBuilder._THESIS_IMPORTANCE.get(c.thesis_rating, InsightImportance.NORMAL),
            ))
        if c.pre_move_reversal_conflict:
            out.append(EvidenceInsight(
                label="PREMOVE_REVERSAL_CONFLICT",
                narrative="PreMove and Reversal disagree on direction",
                importance=InsightImportance.CRITICAL,
            ))
        if c.pre_move is not None:
            for label in c.pre_move.evidence:
                out.append(EvidenceInsight(label=label, narrative=label.replace("_", " ").title()))
        if c.reversal is not None:
            for label in c.reversal.evidence:
                out.append(EvidenceInsight(label=label, narrative=label.replace("_", " ").title()))
        if c.collaboration_result is not None:
            for contra in c.collaboration_result.contradictions:
                out.append(EvidenceInsight(
                    label="AI_CONTRADICTION", narrative=contra, importance=InsightImportance.HIGH,
                ))
            for blind in c.collaboration_result.blind_spots:
                out.append(EvidenceInsight(
                    label="AI_BLIND_SPOT", narrative=blind, importance=InsightImportance.NORMAL,
                ))
        return out
...
class EventEngine:
    ...
    def generate_event(
        ...
        event = Event(
            ...
            macro_bias_reason=macro_bias_reason,
            thesis_rating=(
                candidate.thesis_rating.value
                if candidate.thesis_rating is not None else None
            ),
            market_agreement_status=(
                candidate.market_agreement.status
                if candidate.market_agreement is not None else None
            ),
            ...
            # P3.8-C: build once here, where the real Candidate is
            # still in scope — this is the only place in the codebase
            # that constructs an Event from a Candidate, so it's the
            # one correct spot to attach the translation. Pure read of
            # candidate, matching MarketIntelligenceBuilder's contract
            # (P3.8-B: never writes back to Candidate).
            market_intelligence=MarketIntelligenceBuilder.build(
                candidate,
                funding_rate=funding_rate,
                target_map=target_map,
                symbol_footprint=symbol_footprint,
...
# =====================================================================
# P3.8-C — NARRATIVE RENDERER
#
# MarketIntelligence -> Telegram text. Full replace of the old
# format_event's field-dump posture (per the roadmap doc's point #1:
# "jangan formatter membaca Candidate mentah lagi"). This class reads
# ONLY Event.market_intelligence (the MarketIntelligenceBuilder output
# built once in EventEngine.generate_event) plus a small set of
# identity/transport fields that are genuinely not "content" — symbol,
# priority, timestamp, direction badge. Everything a trader used to
# get as a separate labeled card (structure, pre-move, target map,
# coverage lines, evidence dump) is now inside one of the eight
# Insight sections, phrased as prose by the Builder — this class's
# only job is to lay those eight sections out and skip the ones with
# nothing to say (Insight.has_content()), never to re-derive meaning
# from raw candidate/event fields itself.
#
# P3.8-E regression contract: this changes ONLY how Cryptone explains
# itself. No engine, no state machine, no decision logic is touched by
# this class existing.
# =====================================================================

class NarrativeRenderer:
    """Stateless — every method is a pure function of a MarketIntelligence
    (plus, for format_event, the couple of Event fields that are
    identity/transport rather than analytical content)."""

    # Same rationale as TelegramFormatter._MD_SPECIAL_CHARS: Telegram's
    # legacy Markdown only recognizes these four as entity delimiters,
    # but still hard-errors on any one left unbalanced. Insight
    # narratives are Builder-authored prose (candidate/engine labels,
    # not arbitrary external text) so this is mostly a safety net —
    # kept anyway since AI-sourced CONTEXT narrative can contain a
    # news headline's punctuation.
    _MD_SPECIAL_CHARS = ("_", "*", "`", "[")
    ...
    # One emoji per InsightType, used as the section's line-lead
    # instead of a "*Label:*" prefix — the doc's chart mockup (#9)
    # leads each block with a symbol, not a word, and the eight
    # sections are frequent/uniform enough on every alert that a text
    # label on each would read as more chrome than content.
    _SECTION_EMOJI = {
        InsightType.SITUATION: "📍",
        InsightType.STRUCTURE: "🔀",
        InsightType.THESIS: "🧭",
        InsightType.CONTEXT: "🧠",
        InsightType.EXTERNAL: "🆕",
    ...
    @staticmethod
    def _priority_emoji(priority: EventPriority) -> str:
        return {
    ...
    @classmethod
    def render_section(cls, insight: Insight) -> Optional[str]:
        """One Insight -> one text block, or None if has_content() is
        False. CRITICAL importance gets the section title spelled out
        in caps ahead of the prose (matches the old verdict/conflict
        lines' urgency treatment) — HIGH/NORMAL/LOW read as plain
        prose, since InsightImportance is a presentation-priority
        signal, not a fact the reader needs restated eight times a
        message."""
        if not insight.has_content():
            return None
        emoji = cls._SECTION_EMOJI.get(insight.type, "•")
        text = cls._escape_md(insight.narrative)
        if insight.importance == InsightImportance.CRITICAL:
            return f"{emoji} *{insight.title.upper()}:* {text}"
        return f"{emoji} {text}"

    @classmethod
    def render_evidence(cls, mi: MarketIntelligence) -> Optional[str]:
        """mi.evidence was being collected by the Builder (pre_move/
        reversal raw labels, AI contradictions/blind-spots, conflict
        flags, thesis rating tag) but never actually rendered anywhere
        — a real gap, not by design. Deliberately NOT a dump of the
        whole list: the doc's #1 complaint about the OLD format was
        exactly "print every label" turning into a wall of tags. Only
        CRITICAL/HIGH items surface here — the ones NORMAL-importance
        prose above hasn't already said in its own words (a conflict
        flag or an AI contradiction is worth a second, blunter
        mention; "OI_EXPANSION" is not, the SITUATION prose already
        covers it). NORMAL/LOW evidence stays in the object for
        anyone inspecting MarketIntelligence directly (chart renderer,
        logs) without cluttering the Telegram text."""
        notable = [e for e in mi.evidence if e.importance in (InsightImportance.CRITICAL, InsightImportance.HIGH)]
        if not notable:
            return None
        lines = [cls._escape_md(e.narrative) for e in notable]
        return "🔎 " + " · ".join(lines)

    @classmethod
    def render_intelligence(cls, mi: MarketIntelligence) -> str:
        """The eight sections, doc-order (MarketIntelligence.sections()
        is the single source of that order), each on its own paragraph
        — blank line between them so RESOLUTION (the verdict) always
        reads as a distinct closing line rather than running into
        RISK above it. The notable-evidence line (if any) comes last,
        after the verdict — a closing footnote, not competing with the
        eight-section narrative for top billing."""
        blocks = [cls.render_section(s) for s in mi.sections()]
        body = "\n\n".join(b for b in blocks if b)
        evidence_line = cls.render_evidence(mi)
        if evidence_line:
            body = f"{body}\n\n{evidence_line}" if body else evidence_line
        return body

    @classmethod
    def _header_and_timestamp(cls, event: Event) -> Tuple[str, str]:
        """Shared identity block (symbol/priority/state/direction,
        price-since-detect, timestamp) — factored out of format_event
        so format_caption can build the same header without the two
        ...
        header = (
            f"{cls._priority_emoji(event.priority)} "
            f"*{event.symbol} PERP — {TelegramFormatter._humanize(event.state)}{dir_badge}*"
        )
        timestamp_line = f"🕐 {event.timestamp.strftime('%H:%M:%S UTC')}"
        if event.current_price is not None:
            ...
            timestamp_line = f"{price_line}   {timestamp_line}"
        return header, timestamp_line

    @classmethod
    def format_event(cls, event: Event) -> str:
        """Full replacement for TelegramFormatter.format_event. Header
        is identity only (symbol, priority, state name, direction,
        timestamp) — not analytical content, so it stays outside the
        MarketIntelligence translation per the doc's framing. Body is
        the eight-section narrative. Falls back to a one-line honest
        notice (not a crash, not a silent blank message) if somehow no
        MarketIntelligence was attached — defends the same "build()
        has no failure branches but Event.market_intelligence is still
        Optional" gap noted on that field."""
        mi = event.market_intelligence
        if mi is None:
            return (
                f"{cls._priority_emoji(event.priority)} *{event.symbol} PERP* "
                f"— {TelegramFormatter._humanize(event.state)}\n"
                f"⚠️ No intelligence snapshot available for this event."
            )

        header, timestamp_line = cls._header_and_timestamp(event)
        body = cls.render_intelligence(mi)
    ...
    # Telegram hard-caps sendPhoto captions at 1024 UTF-16 code units;
    # this is a conservative under-cap (not the literal 1024) so a
    # trailing "…" truncation marker plus the header/timestamp lines
    # already spent never pushes the total over the real limit.
    _CAPTION_CHAR_BUDGET = 900

    @classmethod
    def format_caption(cls, event: Event) -> str:
        """Single-message mode: this is now the ONLY text an
        HIGH/CRITICAL event with a chart sends — send_event no longer
        follows the photo with a separate full-breakdown message (that
        was the main driver of the multi-message-per-symbol volume
        that got the bot rate-limited for hours at a time; see the
        chat history around 2026-08-23). The chart carries price
        action; this caption carries the highest-signal prose instead
        of a placeholder + "full breakdown below" pointing at a
        message that no longer exists.

        Picks SITUATION (what) and RESOLUTION (verdict) unconditionally
        when present — the two sections a trader needs even at a
        glance — then fills remaining budget with any CRITICAL-
        importance section (a conflict/contradiction/crowding warning
        should never be silently dropped for space). Falls back to
        format_event_caption's plain header-only behavior if there's
        no MarketIntelligence to draw from.
        """
        mi = event.market_intelligence
        header, timestamp_line = cls._header_and_timestamp(event)
        lines = [header, timestamp_line]
        ...
        priority_sections = [mi.situation, mi.resolution]
        priority_sections += [
            s for s in mi.sections()
            if s not in priority_sections and s.importance == InsightImportance.CRITICAL
        ]

        budget = cls._CAPTION_CHAR_BUDGET - sum(len(l) for l in lines)
        body_parts: List[str] = []
        for insight in priority_sections:
            rendered = cls.render_section(insight)
            if not rendered:
                continue
            if len(rendered) + 2 > budget:
                # Doesn't fit — stop adding sections rather than cut
                # one mid-sentence; whatever fit already is still a
                # coherent read.
                break
...
class TelegramFormatter:
    ...
    @staticmethod
    def _mi_tag(evidence: List[str], key: str) -> Optional[str]:
        """Insight.evidence entries are short 'key=value' tags (see
        Insight's docstring — e.g. "thesis_rating=BEST",
        "macro_trend=BULLISH"), not full sentences. This is the one
        place that parses them back out, used by render_radar_chart's
        badge/header (P3.8-D) to read the exact same MarketIntelligence
        Event carries instead of a second chart-only computation of
        the same facts. Returns None on no match — every caller here
        already treats that as "skip this line", same fail-soft
        posture as the rest of chart rendering."""
        prefix = f"{key}="
        for tag in evidence:
        ...
        try:
            ...
            for z in radar_zones.zones:
                ...
                if z.kind in zone_style:
                    ...
                    edge_lw = 0.6
                    edge_ls = "solid"
                    # P4.1 (Liquidity Persistence): a FRESH wall/gap is
                    # drawn visually lighter and dashed — "provisional,
                    # not yet proven" — while DEFENDED renders at full
                    # weight with a solid edge. Without this, a wall that
                    # appeared one scan ago looked pixel-identical to one
                    # that's held for 30, which is the exact "confident-
                    # looking but fragile" chart problem the war-room
                    # review raised. FORMING sits between the two.
                    if z.kind in ("liquidity_wall", "liquidity_gap") and z.confidence:
                        if z.confidence == "FRESH":
                            fill_alpha = st["alpha"] * 0.45
                            edge_alpha = 0.35
                            edge_ls = "dotted"
                        elif z.confidence == "FORMING":
                            fill_alpha = st["alpha"] * 0.75
                            edge_alpha = 0.45
                            edge_ls = "dashed"
                        else:  # DEFENDED
                            fill_alpha = st["alpha"] * 1.15
                            edge_alpha = 0.65
                            edge_lw = 0.9
                            edge_ls = "solid"
                    ax.axhspan(z.price_low, z.price_high, color=st["color"], alpha=fill_alpha, zorder=1)
                    ax.axhline(
                        z.price_high, color=st["edge"], linewidth=edge_lw, alpha=edge_alpha,
                        linestyle=edge_ls, zorder=1,
                    )
                    # at_risk (side currently under an open TESTED/PULLED
                    # episode right now, independent of how long the wall
                    # has held) gets its own marker — a hazard-orange
                    # hatch — rather than folding into the confidence
                    # color scheme, since "old but currently being tested"
                    # is a different fact than "how old is it".
                    if z.kind == "liquidity_wall" and z.at_risk:
                        ax.axhspan(
                            z.price_low, z.price_high, facecolor="none",
                            edgecolor="#f0883e", hatch="///", linewidth=0.0, alpha=0.35, zorder=1,
                        )
            ...
            #
            # P3.8-D: these values now come from the same MarketIntelligence
            # Event.market_intelligence carries, not a second chart-only
            # computation — read off Insight.evidence tags (thesis_rating=X
            # on .thesis, market_agreement=X also on .thesis, macro_trend=X/
            # setup_stage=X/compression_state=X on .structure) via the
            # _mi_tag helper rather than re-deriving any of these here.
            rating_style = {
                "BEST":   ("#3fb950", "BEST"),
                "WATCH":  ("#58a6ff", "WATCH"),
                "WAIT":   ("#d29922", "WAIT"),
            ...
            mi = radar_zones.market_intelligence
            thesis_tags = mi.thesis.evidence if mi is not None else []
            structure_tags = mi.structure.evidence if mi is not None else []
            mi_thesis_rating = TelegramFormatter._mi_tag(thesis_tags, "thesis_rating")
            mi_market_agreement = TelegramFormatter._mi_tag(thesis_tags, "market_agreement")

            if mi_thesis_rating in rating_style:
                badge_color, badge_text = rating_style[mi_thesis_rating]
                agreement_note = ""
                if mi_market_agreement:
                    agreement_note = f"  ({mi_market_agreement.lower()})"
                ax.text(
            ...
            header_bits = []
            mi_macro_trend = TelegramFormatter._mi_tag(structure_tags, "macro_trend")
            mi_compression_state = TelegramFormatter._mi_tag(structure_tags, "compression_state")
            mi_setup_stage = TelegramFormatter._mi_tag(structure_tags, "setup_stage")
            if mi_macro_trend:
                header_bits.append(f"1H {mi_macro_trend}")
            if mi_compression_state:
                header_bits.append(f"5M {mi_compression_state}")
            if mi_setup_stage:
                header_bits.append(f"15M {mi_setup_stage}")
            if header_bits:
                ax.text(
...
class TelegramAdapter:
    ...
    async def send_event(self, event: Event, chart_bytes: Optional[bytes] = None):
        """Single-message mode (revised — see NarrativeRenderer.
        format_caption's docstring for the incident this fixes): a
        HIGH/CRITICAL event with a chart gets exactly ONE Telegram
        message — the photo with a MarketIntelligence-derived caption
        — not a photo followed by a separate full-breakdown text. The
        prior "always send both" design was the primary driver of a
        multi-message-per-symbol volume that got the bot rate-limited
        by Telegram for hours at a stretch (dozens of symbols x 2-3
        messages each per scan cycle). An event without a chart (chart
        fetch/render failed, or matplotlib unavailable) still gets the
        full NarrativeRenderer.format_event text — that's not a second
        message, it's the ONLY message for that event, so no
        information is lost, just consolidated.
        """
        if chart_bytes is not None:
            caption = NarrativeRenderer.format_caption(event)
            sent = await self._send_photo(chart_bytes, caption)
            if not sent:
                logger.warning(f"{event.symbol}: photo send failed, sending full text alert instead")
                await self._send_text(NarrativeRenderer.format_event(event))
            return
...
class CryptoneV3:
    ...
    def __init__(self, config: Optional[Config] = None):
        ...
        # Audit fix: this only tracks whether the FALLBACK filled
        # anchor_symbols (vs. the operator setting it explicitly via
        # correlation_cfg's own config path) — used below so the
        # auto-anchor-detection block (in run(), after the first live
        # snapshot) knows whether it's safe to re-sync correlation_cfg's
        # anchors to the live-detected ones, or whether doing so would
        # silently override an operator's explicit choice.
        self._correlation_anchors_follow_config = not correlation_cfg.anchor_symbols
        ...
        # P3.8-0 Gap 5 fix: every section's load_state() already
        # tolerates a missing/extra key via .get(key, {}) with a
        # default (see baseline/state_machine/validation/economic_
        # calendar/cheap_screening/regime/compression/pre_move/
        # correlation/micro above) — that's genuine backward-compat
        # infrastructure that already existed, just never used because
        # the version gate below discarded the WHOLE checkpoint on ANY
        # version drift instead of trusting it. min_compatible_state_
        # version marks the oldest saved-file version this process can
        # still load (via those same tolerant per-section defaults).
        # Bump _state_version on additive changes (new key) without
        # bumping min_compatible_state_version — old files still load,
        # new keys just come back empty via .get() same as day one.
        # Bump min_compatible_state_version too only for an actual
        # breaking shape change (a field's type changes, a key gets
        # renamed) where old data would be actively wrong to load, not
        # just incomplete.
        self._min_compatible_state_version = 1
        ...
        # P3.8-0 audit fix (Gap 18): set when run() gives up after
        # max_consecutive_scan_failures — lets a caller/wrapper script
        # tell "stopped, here's why" apart from a clean shutdown.
    ...
    async def _handle_trades(self, payload):
        ...
        try:
            ...
            for item in items:
                ...
                # P3.8-0 Gap 19 fix: .get('px', 0)/.get('sz', 0) used to
                # silently turn a MISSING field (e.g. Hyperliquid
                # renaming px->price upstream) into a real $0 trade fed
                # straight into the microstructure engine — worse than
                # an error, since it's wrong data that looks valid.
                # Require both fields present and non-null; skip+log
                # the single item otherwise, which is the failure mode
                # this should actually have (one bad item lost, not one
                # fabricated zero-price trade polluting every engine
                # downstream of self.micro).
                if item.get('px') is None or item.get('sz') is None:
                    logger.warning(f"Trade payload missing px/sz for {symbol or '?'}: {item}")
                    continue
    ...
    async def _handle_l2book(self, payload):
        ...
        try:
            ...
            def parse_levels(raw_levels):
                ...
                for item in raw_levels[:20]:
                    if isinstance(item, dict):
                        # P3.8-0 Gap 19 fix: same reasoning as
                        # _handle_trades — a level missing px/sz
                        # shouldn't silently become a fake $0 price
                        # distorting the book's best-bid/ask reads.
                        if item.get('px') is None or item.get('sz') is None:
                            continue
        ...
        *next* process (next GitHub Actions job run, next reconnect after a
        ...
        try:
            ...
            payload = self._json_safe({
                ...
                "cheap_screening": self.cheap_screening.export_state(),
                # P3.8-0 Gap 1 fix: the 4 engines whose adaptive state
                # was previously lost on every restart (see each
                # engine's export_state docstring for why these 4 and
                # not the others audited in the same pass).
                "regime": self.regime_engine.export_state(),
                "compression": self.compression.export_state(),
                "pre_move": self.pre_move_engine.export_state(),
                "correlation": self.correlation.export_state(),
                "micro": self.micro.export_state(),
            })
            tmp_path = self.state_path + ".tmp"
            with open(tmp_path, "w") as f:
                json.dump(payload, f)
            ...
            # Audit fix: NarrativeWatchlist previously only saved in
            # cleanup() and at the end of NarrativeIntelScheduler.run_batch()
            # — a long-running process with use_narrative_intelligence=False
            # (or one that crashes between batches) could lose watch-entry
            # status/footprint changes that were never flushed. Own file
            # (narrative_watchlist.json, by design — see that class's
            # docstring), own try/except, atomic tmp+replace — calling it
            # here just makes it checkpoint on the same cadence as
            # everything else instead of on a separate, sparser schedule.
            self.narrative_watchlist.save()
            spread_ready = sum(
                1 for hist in payload["cheap_screening"].values() if len(hist) >= 5
            )
            logger.info(
                ...
                f"{spread_ready}/{len(payload['cheap_screening'])} symbols with LIQUIDITY baseline ready)"
            )
            ...
            # P3.8-1 (laporan concern #2): same once-per-checkpoint
            # cadence as lead-time above — this is the cumulative,
            # restart-surviving FRESH/FORMING/DEFENDED mix behind every
            # target_map reading actually used, watching layer vs chart/
            # Telegram layer separately. Purely observational, doesn't
            # feed back into any gate.
            try:
                wm_summary = self.micro.wall_mention_summary()
                if wm_summary:
                    logger.info("liquidity wall mentions — " + wm_summary)
            except Exception as e:
                logger.debug(f"wall mention summary log skipped: {e}")
        except Exception as e:
    ...
    def load_state(self):
        ...
        try:
            ...
            if not isinstance(file_version, int) or file_version < self._min_compatible_state_version:
                logger.warning(
                    f"state file version {file_version} is older than the "
                    f"oldest loadable version {self._min_compatible_state_version} "
                    f"— starting fresh"
                )
                return
            if file_version > self._state_version:
                # A newer process wrote this file than the one loading it
                # now (e.g. a rollback) — refuse rather than guess at a
                # schema this code doesn't know about yet.
                logger.warning(
                    f"state file version {file_version} is newer than this "
                    f"process's {self._state_version} — starting fresh"
                )
                return
            if file_version != self._state_version:
                logger.info(
                    f"state file version {file_version} != current "
                    f"{self._state_version} but within compatible range "
                    f"[{self._min_compatible_state_version}, {self._state_version}] "
                    f"— loading anyway (sections use tolerant defaults for "
                    f"any keys added since)"
                )

            self.baseline.load_state(payload.get("baseline", {}))
            self.state_machine.load_state(payload.get("state_machine", {}))
            # .get("cheap_screening", {}) defaults to {} for checkpoints
            # saved before this feature existed — same tolerant posture
            # as validation/economic_calendar below. Restores LIQUIDITY
            # domain warm-up so a relaunch continues from where the
            # last process left off instead of re-cold-starting every
            # symbol's spread baseline from zero.
            self.cheap_screening.load_state(payload.get("cheap_screening", {}))
            ...
            # P3.8-0 Gap 1 fix: .get(..., {})/None-default here matches
            # every other section's tolerant posture — a checkpoint
            # saved before this fix just cold-starts these 4 engines'
            # adaptive state, same as today, instead of failing restore.
            self.regime_engine.load_state(payload.get("regime") or {})
            self.compression.load_state(payload.get("compression") or {})
            self.pre_move_engine.load_state(payload.get("pre_move") or {})
            self.correlation.load_state(payload.get("correlation") or {})
            self.micro.load_state(payload.get("micro") or {})

            n_candidates = len(self.state_machine.get_active())
            n_symbols = len(payload.get("baseline", {}))
            spread_state = payload.get("cheap_screening", {})
            n_spread_ready = sum(1 for hist in spread_state.values() if len(hist) >= 5)
            ...
            logger.info(
                ...
                f"{n_spread_ready}/{len(spread_state)} symbols with LIQUIDITY baseline carried over "
                f"(no cold-start warmup needed for those)"
            )
        except Exception as e:
    ...
    async def initialize(self):
        ...
        _, first_snapshot = await asyncio.gather(
            self.ws.connect(),
            self.hyperliquid.snapshot_all(),
        )

        # Anchors are supposed to be "whatever the market's actual majors
        # are right now" (for correlation/regime reads), not a symbol
        # list someone typed once and never revisited. If the operator
        # didn't pin ANCHOR_SYMBOLS explicitly, derive it live from this
        # first snapshot: rank by OI+volume (USD), same liquidity signal
        # DiscoveryEngine already trusts elsewhere, and take the top N
        # (N = however many placeholder anchors were configured, so
        # ANCHOR_COUNT-style tuning still works via ANCHOR_SYMBOLS).
        if self.config.anchors.auto and first_snapshot:
            n = max(len(self.config.anchors.symbols), 1)
            ranked = sorted(
                first_snapshot.values(),
                key=lambda d: d.open_interest + d.volume_24h,
                reverse=True,
            )
            live_anchors = [d.symbol for d in ranked[:n]]
            if live_anchors:
                logger.info(
                    f"anchors: auto-selected [{', '.join(live_anchors)}] "
                    f"by live OI+volume ranking (was placeholder "
                    f"[{', '.join(self.config.anchors.symbols)}])"
                )
                self.config.anchors.symbols = live_anchors
                # Audit fix (gap: CorrelationEngine anchors never
                # propagate): CorrelationEngine.cc.anchor_symbols was
                # copied from self.config.anchors.symbols back in
                # __init__, BEFORE this auto-detect ever ran — a plain
                # `list(...)` copy, not a shared reference, so mutating
                # self.config.anchors.symbols above never touched it.
                # CorrelationEngine kept scoring decoupling against the
                # placeholder ["BTC","ETH","SOL"] defaults forever, even
                # on the success path, not just when the first snapshot
                # failed (the scenario originally flagged — turns out
                # it was broader than that). Only re-syncs when
                # anchor_symbols was following AnchorConfig by default
                # (the flag set in __init__) — an operator who pinned
                # CorrelationConfig's own anchor_symbols explicitly
                # keeps their choice, auto-detection never overrides it.
                if self._correlation_anchors_follow_config:
                    self.correlation.cc.anchor_symbols = list(live_anchors)
                    logger.info(
                        f"CorrelationEngine anchors synced to live selection: "
                        f"[{', '.join(live_anchors)}]"
                    )
    ...
    async def cleanup(self):
        ...
        # P3.8-0 Gap 15 fix: _pending_events survives a scan() exception
        # (it's an instance attr, not reset at scan() start — see
        # _flush_scan_events), so it isn't lost on the next scan
        # attempt. But it WAS lost on a real shutdown (SIGTERM,
        # KeyboardInterrupt, or the new max_consecutive_scan_failures
        # stop) since nothing here ever called _flush_scan_events —
        # events already generated this run just vanished instead of
        # reaching Telegram. Flushed before the final checkpoint so a
        # send failure here still doesn't block save_state.
        if self._pending_events:
            try:
                await self._flush_scan_events()
            except Exception as e:
                logger.warning(f"cleanup: failed to flush {len(self._pending_events)} pending events: {e}")
        self.save_state()  # final checkpoint so the next run continues from here
        await self.ws.disconnect()
        # P3.8-0 audit fix (Gap 17): these used to be 12 sequential bare
        # awaits — one __aexit__ raising meant every resource after it in
        # the list silently never got closed. gather(return_exceptions=True)
        # guarantees every close is attempted regardless of earlier
        # failures; failures are logged, not swallowed silently.
        closers = {
            "hyperliquid": self.hyperliquid.__aexit__(),
            "telegram": self.telegram.__aexit__(),
            "macro_sentiment": self.macro_sentiment.__aexit__(),
            "market_breadth_provider": self.market_breadth_provider.__aexit__(),
            "news_provider": self.news_provider.__aexit__(),
            "logo_provider": self.logo_provider.__aexit__(),
            "economic_calendar": self.economic_calendar.__aexit__(),
            "rss_headlines": self.rss_headlines.__aexit__(),
            "llm_headlines": self.llm_headlines.__aexit__(),
            "ai_context_engine": self.ai_context_engine.__aexit__(),
            "capability_bridge": self.capability_bridge.__aexit__(),
            "narrative_intel_engine": self.narrative_intel_engine.__aexit__(),
        }
        results = await asyncio.gather(*closers.values(), return_exceptions=True)
        for name, result in zip(closers.keys(), results):
            if isinstance(result, Exception):
                logger.warning(f"cleanup: {name}.__aexit__ failed (continuing): {result}")
        self.narrative_watchlist.save()
    ...
    async def scan(self):
        ...
        for symbol in symbols_to_score:
            ...
            # P3.8-0 Gap 8 fix: PreMoveEngine (new positioning) and
            # ReversalEngine (exhaustion of an existing move) can name
            # opposite directions for the same symbol — e.g. PreMove
            # says long positioning is building while Reversal says the
            # current up-move is exhausted and about to fade. Neither
            # engine is "wrong": they're reading different things. This
            # doesn't suppress either signal or pick a winner — it just
            # makes the disagreement visible on the candidate so
            # notification-side code and format_event's evidence list
            # can flag it instead of a trader seeing two contradictory
            # alerts back-to-back with no indication they're related.
            candidate.pre_move_reversal_conflict = bool(
                candidate.pre_move is not None
                and candidate.reversal is not None
                and candidate.pre_move.direction in ("long", "short")
                and candidate.reversal.direction in ("long", "short")
                and candidate.pre_move.direction != candidate.reversal.direction
            )
            ...
            # own thesis) runs later in this same scan pass, once
            # ai_context_result is available — see the collaboration
            # engine call further down. Wrapped defensively so a bug
        ...
        for candidate in scored_candidates:
            ...
            if candidate.quality > self.opportunity.config.notification_min_quality and candidate.is_active:
                ...
                # P3.8-0 Gap 8 fix: surface the PreMove/Reversal directional
                # conflict the same way — a visible label, not a silent
                # tie-break. Neither engine's read is suppressed or
                # reprioritized; a trader just gets told two engines
                # disagree on direction for this symbol right now.
                if candidate.pre_move_reversal_conflict:
                    evidence_labels = evidence_labels + ["PREMOVE_REVERSAL_CONFLICT"]
                ...
                # P3.8-1: compute once here (rather than inline in the
                # generate_event() call below) so the same reading can
                # both populate target_map and get counted in the wall
                # mention stats — a single build_target_map call feeding
                # both, not two.
                _tm = build_target_map(
                    symbol, candidate.direction, self.micro.get_liquidity_map(symbol),
                )
                if _tm is not None:
                    self.micro.record_wall_mention(_tm.nearest_wall_confidence, "watching")
                ...
                event = self.event_engine.generate_event(
                    ...
                    target_map=_tm.to_dict() if _tm else None,
            ...
            for event in urgent:
                try:
                    ...
                    if self.config.chart.radar_zones_enabled:
                        ...
                        #
                        # P3.8-D: all_contexts is no longer fetched here —
                        # macro_trend/setup_stage now come from
                        # event.market_intelligence.structure.evidence
                        # (built once in EventEngine.generate_event off
                        # candidate.market_structure, which IS this exact
                        # get_all_contexts() output already, just read off
                        # the Candidate instead of re-fetched here).
                        ...
                        radar_zones = compute_radar_zones(
                            ...
                            orderbook=orderbook,
                            data_quality=data_quality,
                            market_intelligence=event.market_intelligence,
                        ...
                        if direction in ("long", "short"):
                            ...
                            if enriched_tm is not None:
                                ...
                                # P3.8-1: this is the reading that actually
                                # ships to Telegram for HIGH/CRITICAL
                                # events, so tag it "chart" — the number
                                # to watch for laporan concern #2.
                                self.micro.record_wall_mention(enriched_tm.nearest_wall_confidence, "chart")
                                # Audit fix: event.market_intelligence was
                                # built in generate_event() BEFORE this
                                # richer target_map existed, using only the
                                # liquidity-only reading — so its
                                # .expectation section (what NarrativeRenderer
                                # actually sends to Telegram) was missing the
                                # structural target level even though
                                # event.target_map right above now has it.
                                # Re-run just that one section, not the
                                # whole MarketIntelligence — the other seven
                                # sections don't depend on target_map, and
                                # rebuilding all eight here would silently
                                # re-derive them from whatever cand looks
                                # like NOW (post-scan-loop mutations) instead
                                # of the state generate_event() actually
                                # captured the event at.
                                if cand is not None and event.market_intelligence is not None:
                                    event.market_intelligence.expectation = (
                                        MarketIntelligenceBuilder._build_expectation(
                                            cand, target_map=event.target_map,
                                        )
                                    )
    ...
    async def run(self, max_duration_seconds: Optional[float] = None):
        ...
        while self.running:
            try:
                ...
                max_failures = self.config.runtime.max_consecutive_scan_failures
                if max_failures and self._scan_failure_count >= max_failures:
                    logger.error(
                        f"scan #{self.scan_count} crashed: {e} — "
                        f"{self._scan_failure_count} consecutive failures, "
                        f"hit max_consecutive_scan_failures={max_failures}. "
                        f"Stopping (likely a permanent error, not a transient "
                        f"one — check credentials/config before restarting)."
                    )
                    self._fatal_error = e
                    break
...
async def quick_check(symbols: Optional[List[str]] = None):
    ...
    # 2. WS quick test
    # Audit fix: this used to only exercise the "trades" subscription
    # type. A working trades feed doesn't guarantee l2Book (order book
    # — feeds MicrostructureEngine, LiquidityMap, target_map) or candle
    # (feeds BaselineEngine/RegimeEngine/chart rendering) are equally
    # healthy — different subscription types, can fail independently
    # on Hyperliquid's side. Now checks all three per anchor symbol.
    if not WEBSOCKETS_AVAILABLE:
        ...
        for sym in symbols:
            try:
                ...
                print(f"✅ WS {sym} trades: {msg[:100]}")
                await ws.close()
            except Exception as e:
                print(f"❌ WS {sym} trades: {e}")
            ...
            try:
                ...
                msg = await asyncio.wait_for(ws.recv(), timeout=5)
                print(f"✅ WS {sym} l2Book: {msg[:100]}")
                await ws.close()
            except Exception as e:
                print(f"❌ WS {sym} l2Book: {e}")
            ...
            try:
                ...
                msg = await asyncio.wait_for(ws.recv(), timeout=5)
                print(f"✅ WS {sym} candle(1m): {msg[:100]}")
                await ws.close()
            except Exception as e:
                print(f"❌ WS {sym} candle(1m): {e}")
