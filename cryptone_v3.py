...
@dataclass
class ContextConfig:
    """P1: ContextEngine's EMA trend classification parameters."""

    ema_fast: int = 5
    ema_slow: int = 20
    context_history_bars: int = 30
    min_bars_required: int = 10

    trend_bull_threshold: float = 1.005   # ema_short > ema_long * this
    trend_bear_threshold: float = 0.995   # ema_short < ema_long * this
...
class ContextEngine:
    ...
    def get_context(self, symbol: str, timeframe: str = '15m') -> Dict:
        ...
        closes = [c.close for c in candles[-cc.context_history_bars:]]

        ema_short = self._ema(closes, cc.ema_fast)
        ema_long = self._ema(closes, cc.ema_slow)

        if ema_short > ema_long * cc.trend_bull_threshold:
            strength = min((ema_short / ema_long - 1) * cc.strength_multiplier, 1.0)
            return {'trend': 'BULLISH', 'strength': strength, 'bars': len(candles), 'close': closes[-1]}
        elif ema_short < ema_long * cc.trend_bear_threshold:
            strength = min((ema_long / ema_short - 1) * cc.strength_multiplier, 1.0)
...
class StateMachine:
    ...
    def stage_category(state: PrimaryState, pre_move: Optional["PreMoveSignal"]) -> str:
        ...
        DEVELOPING     — still gathering evidence, not yet actionable.
        ENTRY_WINDOW    — actionable: either StateMachine reached
                          HIGH_CONVICTION/THESIS, or PreMoveEngine
                          independently confirmed positioning+compression
                          ahead of StateMachine catching up.
...
class PreMoveEngine:
    ...
    def evaluate(
        ...
        trend_key = context.get("trend")
        context_aligned = (
            (direction == "long" and trend_key in ("BULLISH", "NEUTRAL")) or
            (direction == "short" and trend_key in ("BEARISH", "NEUTRAL"))
        )
        if context_aligned:
            support_count += 1
            support_labels.append(f"CONTEXT_{trend_key}")
...
class CryptoneV3:
    ...
    def __init__(self, config: Optional[Config] = None):
        ...
        self.context = ContextEngine(self.config.context)
    ...
    async def scan(self):
        ...
        for symbol in symbols_to_score:
            ...
            if candidate.quality > 0.5 and candidate.is_active:
                ...
                # P1: DECOUPLED and DECOUPLED_UNCONFIRMED are both surfaced
                # — the unconfirmed case is exactly the ADA/NEAR-style
                # fakeout risk the user flagged, and it stays *visible* as
                # a distinct label rather than being silently dropped, so
                # it can be judged manually rather than the engine quietly
    ...
    async def _flush_scan_events(self):
        ...
        candlestick chart for each HIGH/CRITICAL event specifically — the
        routine WATCHING-tier table stays text-only as before. Chart
        fetch/render happens here (not inside TelegramAdapter) because
        this is where hyperliquid access already lives; each symbol is
        wrapped in its own try/except so one API hiccup only costs that
        symbol its chart, not the whole batch's charts or the alert itself.
        """
        if not self.config.telegram_enabled:
        ...
        if self.config.chart.enabled:
            for event in urgent:
                try:
                    ...
                    stage_note = cand.episode_stage if cand else None
                    png = ChartRenderer.render_candlestick(
                        candles, event.symbol, direction=direction, annotation=stage_note,
                    )
                    if png:
                        charts[id(event)] = png
                except Exception as e:
