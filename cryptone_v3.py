    ...
    async def fetch_l2book_rest(self, symbol: str) -> Optional["OrderBook"]:
        ...
        try:
            async with self._session.post(
                ...
                if resp.status != 200:
                    logger.warning(f"fetch_l2book_rest({symbol}): HTTP {resp.status}")
                    return None
                raw = await resp.json()
        except Exception as e:
            logger.warning(f"fetch_l2book_rest({symbol}) failed: {type(e).__name__}: {e}")
            return None
    ...
    async def fetch_funding_history(
        ...
        try:
            async with self._session.post(
                ...
                if resp.status != 200:
                    logger.warning(f"fetch_funding_history({symbol}): HTTP {resp.status}")
                    return []
                raw = await resp.json()
        except Exception as e:
            logger.warning(f"fetch_funding_history({symbol}) failed: {type(e).__name__}: {e}")
            return []
            ...
            try:
                if not self.baseline._funding_history.get(symbol):
                    ...
                    if rates:
                        ...
                        logger.info(f"_backfill_new_symbols: {symbol} funding seeded with {len(rates)} samples")
                    else:
                        logger.warning(f"_backfill_new_symbols: {symbol} funding REST returned no history (will fill from live scans instead)")
            except Exception as e:
                logger.warning(f"_backfill_new_symbols: {symbol} funding backfill failed (will fill from live scans instead): {type(e).__name__}: {e}")
            ...
            for tf in backfill_timeframes:
                try:
                    ...
                    for candle in candles:
                        self.context.update_candle(candle)
                    if candles:
                        logger.info(f"_backfill_new_symbols: {symbol} {tf} seeded with {len(candles)} candles")
                        ...
                        logger.warning(f"_backfill_new_symbols: {symbol} {tf} REST returned no candles (will fill from WS instead)")
                except Exception as e:
                    logger.warning(f"_backfill_new_symbols: {symbol} {tf} backfill failed (will fill from WS instead): {type(e).__name__}: {e}")
