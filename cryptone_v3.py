...
class Event:
    ...
    # the MarketAgreement status behind it were already wired into
    # RadarZones (chart badge) via compute_radar_zones(thesis_rating=...)
    # but never carried onto Event, so format_event's Telegram text had
    # no way to know what the chart badge next to it was already saying.
...
class MarketIntelligenceBuilder:
    ...
    def build(
        ...
        *,
        ...
        sentiment/bias, news) — same reason they were already separate
        kwargs on EventEngine.generate_event rather than Candidate
        fields. Builder stays a pure function; it just now takes a
        couple more explicit inputs instead of silently having no way
        to see them. Every one is Optional/default-None so build()
        called with just a candidate (existing tests, any future
        caller that doesn't have this scan-cycle context) still works
        unchanged.
        """
        return MarketIntelligence(
            symbol=candidate.symbol,
            situation=cls._build_situation(candidate),
            structure=cls._build_structure(candidate),
    ...
    # ---- market structure --------------------------------------------
    @staticmethod
    def _build_structure(c: Candidate) -> Insight:
        r = c.regime
        if r is None:
            return Insight(type=InsightType.STRUCTURE, title="Structure", narrative="", state=DataState.ABSENT)
        ...
        if state == DataState.UNKNOWN:
            return Insight(
                ...
                importance=InsightImportance.LOW, state=DataState.UNKNOWN,
            )
    ...
    def _build_thesis(c: Candidate) -> Insight:
        ...
        evidence = [f"engine_belief={belief.level}", f"market_agreement={agreement.status}"]
