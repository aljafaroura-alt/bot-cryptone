...
#
# Primary: https://app.hyperliquid.xyz/coins/{symbol}.svg — Hyperliquid's
# own icon CDN, covers 600+ assets and is keyed by the exact same `coin`
# symbol this bot already trades against (see hyperliquid-coin-svgs on
# npm for independent confirmation this endpoint is public/stable), so
# there's no ticker-mapping guesswork the way a third-party CDN needs.
# This is what gets nearly every symbol the radar actually fires on.
#
# Fallback: https://assets.coincap.io/assets/icons/{symbol}@2x.png —
# kept for the rare case SVG rendering isn't available in this runtime
# (see SVGLIB_AVAILABLE) or Hyperliquid's own CDN doesn't have a symbol
# yet (very new listing). Both are free, keyless, no rate-limit
# handshake. Same posture as NewsProvider throughout: strictly optional,
# per-symbol cached, fail-open — a chart must never fail to send because
# every logo source 404'd or hiccuped.
# =====================================================================
...
class CoinLogoProvider:
    ...
    which of the two sources actually served the icon) or None.
    """
    ...
    def __init__(self, cache_seconds: int = 3600):
        self.cache_seconds = cache_seconds
        self._session: Optional["aiohttp.ClientSession"] = None
        # negative hits (symbol has no icon on either CDN) are cached
        # too, under the same TTL, so a delisted/exotic-perp symbol
        # doesn't get re-requested every single scan it's active for.
    ...
    async def get_logo(self, symbol: str) -> Optional[bytes]:
        ...
        neither source has it / can't be decoded. Never raises; a
        missing logo just means the chart header falls back to
        ...
        # Primary: Hyperliquid's own CDN, exact-symbol match.
        svg_bytes = await self._fetch(self.HL_ENDPOINT.format(symbol=base))
        if svg_bytes:
            data = self._svg_to_png(svg_bytes)

        # Fallback: CoinCap, only reached if HL had nothing or the SVG
...
class TelegramFormatter:
    ...
    def format_batch_watch(events: list, funding_extreme_abs: float = 0.003) -> str:
        ...
        for e in events:
            ...
            # movement.
            # Extreme fix: gated purely on real magnitude
            ...
            # noise). A flat rate can no longer earn ⚠️ no matter what
            # the z-score says.
            if e.funding_rate is not None:
                funding = f"{e.funding_rate:+.6f}"
                if abs(e.funding_rate) >= FUNDING_EXTREME_ABS_DISPLAY:
                    funding += "⚠️"
            else:
                funding = "-"
        ...
        try:
            ...
            if logo_bytes:
                try:
                    ...
                    logo_img[mask, 3] = 0
                    logo_box = matplotlib.offsetbox.OffsetImage(logo_img, zoom=0.24)
