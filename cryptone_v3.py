...
class SubscriptionManager:
    ...
    Two tiers, matching what evidence/execution actually need:
      TIER_CANDLE — WATCHING-and-below coverage: candles only
                     (ws_base_interval, e.g. "1m"), enough for
                     DiscoveryEngine/ContextEngine to keep scoring a
                     symbol that isn't confirmed yet.
      TIER_FULL   — ACTIVE/HIGH_CONVICTION/THESIS coverage: candles +
                     trades + l2Book, the full feed MicrostructureEngine/
                     LiquidityMap/execution need once a symbol is a real
                     candidate.

    Anchors (BTC/ETH/SOL etc.) are seeded at TIER_FULL once, outside this
    class, at startup (see CryptoneV3.start()) and never downgraded —
    update() only manages non-anchor symbols, since anchors aren't part
    of StateMachine's candidate set anyway.

    _current_tier is this manager's own bookkeeping of what it believes
    is currently subscribed, purely to avoid redundant subscribe/
    unsubscribe calls each scan — WSConnection.subscriptions is the real
    source of truth (and already resubscribes from memory on reconnect,
    per WSConnection's own FIX #8 handling), so a mismatch here is only
    ever a wasted network call, never a coverage gap.
    """
    ...
    def __init__(self, ws: "WSConnection", anchors: List[str], timeframe_config: "TimeframeConfig"):
        self.ws = ws
        self.anchors = set(anchors)
        self.timeframe_config = timeframe_config
        self._current_tier: Dict[str, str] = {}

    def _target_tier(self, candidate: "Candidate") -> str:
        if candidate.symbol in self.anchors:
            return self.TIER_FULL  # never downgraded; kept in sync defensively
        if candidate.is_active:
            return self.TIER_FULL
        return self.TIER_CANDLE

    async def update(self, active_candidates: List["Candidate"]) -> None:
        """Reconcile subscriptions against the current candidate set.
        Called once per scan (see CryptoneV3.scan()). Anchors are
        excluded from downgrade/removal entirely — they keep TIER_FULL
        for the life of the process regardless of what StateMachine says,
        since they're not candidates and always need full coverage for
        CorrelationEngine/ContextEngine's macro reads."""
        target: Dict[str, str] = {sym: self.TIER_FULL for sym in self.anchors}
        for cand in active_candidates:
            if cand.symbol in self.anchors:
                continue
            target[cand.symbol] = self._target_tier(cand)

        interval = self.timeframe_config.ws_base_interval

        # Upgrades and fresh candle-only subscriptions
        to_candle = [
            sym for sym, tier in target.items()
            if tier in (self.TIER_CANDLE, self.TIER_FULL)
            and self._current_tier.get(sym) not in (self.TIER_CANDLE, self.TIER_FULL)
        ]
        if to_candle:
            await self.ws.subscribe("candle", to_candle, interval=interval)

        to_full = [
            sym for sym, tier in target.items()
            if tier == self.TIER_FULL and self._current_tier.get(sym) != self.TIER_FULL
        ]
        if to_full:
            await self.ws.subscribe("trades", to_full)
            await self.ws.subscribe("l2Book", to_full)

        # Downgrades: FULL -> CANDLE drops trades/l2Book but keeps candle
        to_downgrade = [
            sym for sym, tier in target.items()
            if tier == self.TIER_CANDLE and self._current_tier.get(sym) == self.TIER_FULL
        ]
        if to_downgrade:
            await self.ws.unsubscribe("trades", to_downgrade)
            await self.ws.unsubscribe("l2Book", to_downgrade)

        # Removals: symbol no longer in the active set at all — drop everything
        dropped = [
            sym for sym in self._current_tier
            if sym not in target and sym not in self.anchors
        ]
        if dropped:
            candle_keys = [s for s in dropped if self._current_tier.get(s) in (self.TIER_CANDLE, self.TIER_FULL)]
            full_keys = [s for s in dropped if self._current_tier.get(s) == self.TIER_FULL]
            if candle_keys:
                await self.ws.unsubscribe("candle", candle_keys, interval=interval)
            if full_keys:
                await self.ws.unsubscribe("trades", full_keys)
                await self.ws.unsubscribe("l2Book", full_keys)
            for sym in dropped:
                self._current_tier.pop(sym, None)

        for sym, tier in target.items():
            self._current_tier[sym] = tier
