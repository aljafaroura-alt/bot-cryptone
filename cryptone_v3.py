...
class MacroBiasEngine:
        ...
        if macro_sentiment is not None:
            ...
            if reading is not None:
                ...
                if value <= sc.bias_macro_neutral_lo:
                    ...
                    return "long", confidence, f"Macro sentiment {label} ({value})"
                if value >= sc.bias_macro_neutral_hi:
                    ...
                    return "short", confidence, f"Macro sentiment {label} ({value})"
    ...
    async def compute_symbol_bias(
        ...
        NewsProvider itself). Direction is a crude lexical read (upgrade/
        listing/partnership → long-leaning; hack/exploit/delist/lawsuit →
        short-leaning) — deliberately coarse since this only ever
        contributes a small bonus on top of a symbol's own data, never a
        standalone signal.
        ...
        if h.published_at is not None:
            ...
            confidence = max(0.0, 1.0 - (age_hours / sc.bias_news_freshness_hours))
        else:
            confidence = 0.25  # unknown age: capped, not assumed fresh
        ...
        if source_kind == "official" and confidence > 0:
        ...
        if any(k in title_lower for k in bullish_kw):
            return "long", confidence, f"{prefix}{h.title[:80]} ({h.source})"
        if any(k in title_lower for k in bearish_kw):
            return "short", confidence, f"{prefix}{h.title[:80]} ({h.source})"
        return None, 0.0, None
    ...
    async def compute(
        ...
        for symbol in candidate_symbols[:max_news_lookups]:
            ...
            if cached is not None and (now - cached[0]).total_seconds() < self.symbol_bias_cooldown_seconds:
                ...
                direction, confidence, reason = await self.compute_symbol_bias(symbol, news, rss)
...
class ValidationTracker:
    ...
    def summary(self) -> List[StageValidation]:
        ...
        either way."""
        return self._summarize(self._outcomes)
    ...
    def _summarize(self, outcomes: List[PreMoveOutcome]) -> List[StageValidation]:
        results = []
    ...
    def lead_time_summary(self) -> List[LeadTimeSummary]:
        ...
        return self._lead_time_summarize(self._outcomes)
    ...
    def _lead_time_summarize(self, outcomes: List[PreMoveOutcome]) -> List[LeadTimeSummary]:
        results = []
...
class TelegramFormatter:
    ...
    def _build_narrative(event: "Event") -> Optional[str]:
        """Synthesizes microstructure + PreMove + ReversalEngine +
        macro sentiment into one short, plain-language sentence — the
        "WLFI bla bla bla" line the user asked for. Returns None if
        there isn't enough signal to say anything substantive (a bare
        WATCHING candidate with no positioning/reversal/pre_move read
        yet), rather than manufacturing a sentence out of nothing.

        Priority order, richest signal first: ReversalEngine (it's the
        most specific, timing-relevant read — exhaustion of a leg
        happening right now) > PreMoveEngine (new positioning building)
        > generic positioning/funding evidence from the base evidence
        list. Macro sentiment is appended as a clause only when it
        actually agrees with or contradicts the primary read, not just
        printed for its own sake — a Fear&Greed number with nothing to
        say about the specific setup is noise, not narrative.
        """
        symbol = event.symbol
        parts: List[str] = []
        ...
        if event.reversal_stage and event.reversal_play:
            ...
            if support_phrases:
                parts.append("Backing it: " + ", ".join(support_phrases[:3]) + ".")
            if "FUNDING_HOSTILE" in rv or "MACRO_HOSTILE" in rv:
                parts.append("Worth noting: funding/macro backdrop isn't fully aligned yet, so size accordingly.")

        elif event.pre_move_stage and event.pre_move_direction:
            dir_word = "longs" if event.pre_move_direction == "long" else "shorts"
            conf_pct = f"{event.pre_move_confidence:.0%}" if event.pre_move_confidence is not None else None
            stage_verb = {
            ...
            parts.append(f"{symbol}: " + ", ".join(humanized) + ".")

        # --- structure/context clause: folds what used to be the
        # separate 1H/15M/Context "structure card" block into one plain
        # sentence, only when it actually agrees with (or conflicts
        # with) the primary thesis above — a structure read that's just
        # "still warming up" or genuinely neutral has nothing to add
        # here and is skipped rather than restated. This is the fix for
        # the "wall of separate labeled sections" complaint: the
        # underlying ContextEngine reads aren't dropped, they're now one
        # of the clauses in the same paragraph instead of their own
        # divider-separated block.
        if event.market_context in ("LONG_SUPPORTED", "SHORT_SUPPORTED"):
            parts.append("Structure backs it: 1H and 15M both point the same way.")
        elif event.market_context == "CONFLICT":
            parts.append("Structure is mixed though — 1H and 15M don't agree yet.")

        # --- expectation clause: MicrostructureEngine's origin/intent/
        # confirmation read, condensed to one clause instead of its own
        # 5-line block. Only the live, useful parts (what's happening,
        # what would confirm it) — invalidation condition is dropped
        # from the narrative itself (still available via evidence/raw
        # data) since "what would prove this wrong" is secondary to "what
        # is this saying" in a first-read summary.
        ...
        if exp_n and exp_n.get("status") == "PENDING":
            ...
            parts.append(f"That expectation just confirmed — {exp_n.get('expected_path', '').lstrip('→ ')}.")

        # --- target map clause: the defended/open read, one sentence.
        ...
        # --- track record clause: SymbolFootprint, folded in as a
        # trailing clause rather than its own footnote line, same
        # "clause not section" treatment as the reads above.
        fp = event.symbol_footprint
        if fp and fp.get("description"):
            parts.append(fp.get("description") + ".")

        # --- macro clause: only when it actually agrees/disagrees with
        # the primary read above, not printed unconditionally.
        #
        # Fix: this used to gate purely on event.reversal_play, so any
        # event without a live ReversalEngine read (e.g. a plain
        # positioning/PreMove event, or the generic evidence fallback
        # above) could never get a macro clause even when a genuinely
        # extreme Fear&Greed reading was sitting right there and the
        # event had a clear long/short direction to check it against.
        # That's why symbols like ZORA — ACTIVE with a direction, but no
        # ReversalEngine play firing that scan — showed no macro context
        # at all. Generalized to event.direction (long/short), which is
        # already set for every non-reversal event with a directional
        # read, so the same extreme-only agree/disagree logic now
        # applies uniformly instead of being reversal-only. A reversal
        # play, when present, still takes priority for which "side"
        # (dip/capitulation vs rally/bounce) is being checked, since
        # it's the more specific read of the two.
        if event.macro_sentiment_value is not None and event.macro_sentiment_label:
            ...
            if is_bullish_thesis and mv <= 30:
                parts.append(f"Broader market sentiment is also {event.macro_sentiment_label} ({mv}), consistent with a real flush rather than a symbol-specific breakdown.")
            elif is_bearish_thesis and mv >= 70:
                parts.append(f"Broader market sentiment is {event.macro_sentiment_label} ({mv}) — this isn't just {symbol}, the whole market looks stretched.")
            elif is_bullish_thesis and mv >= 70:
                parts.append(f"Caveat: broader sentiment is still {event.macro_sentiment_label} ({mv}), so this may just be noise in an otherwise greedy market.")
            elif is_bearish_thesis and mv <= 30:
                parts.append(f"Caveat: broader sentiment is {event.macro_sentiment_label} ({mv}), which cuts against a fade/short thesis.")

        # --- market breadth clause: BTC dominance / total market cap
        # trend, a distinct read from the Fear&Greed clause above (see
        # MarketBreadthProvider's class docstring). Skipped for BTC
        # itself — "capital rotating into/out of BTC" is a circular
        # thing to say about a BTC-directional call, it only means
        # something for an altcoin thesis. Also skipped whenever
        # describe_for_altcoin already decided there's nothing
        # meaningful to add (small mcap move, no direction, etc.) —
        # same "only speak when it agrees or disagrees with the
        # specific thesis" posture as the macro clause above, not a
        # second unconditional line.
        if event.market_breadth is not None and symbol.upper() != "BTC":
            ...
            if breadth_clause:
                parts.append(breadth_clause)

        # --- news clause: only the freshest matching headline, stated
        # as-is (title + source), never interpreted or attributed as
        # THE reason for the move — the bot has no way to verify
        # causality between a headline and price action, only that both
        # are happening around the same time. Kept last so it reads as
        # "here's relevant context" after the microstructure read has
        # already made its case, not as the lead driver. See NewsProvider
        # for coverage caveats (best-effort, not guaranteed for small/new
        # tickers).
        if event.news_headline:
            # News source/title are external text (RSS/API-sourced) and
            # not from any fixed internal vocabulary — must be escaped
            # before landing in a Markdown-parsed message, or a headline
            # containing "_", "*", "`", or "[" produces Telegram's
            # "can't find end of the entity" 400 and drops the alert.
            src = f" ({TelegramFormatter._escape_md(event.news_source)})" if event.news_source else ""
            # Defensive truncation: a headline's title field is normally
            # short, but nothing upstream guarantees that — cap it so a
            # pathological long "title" can't blow the caption budget or
            # read like a reproduced excerpt rather than a headline.
            headline = event.news_headline
            if len(headline) > 120:
                headline = headline[:117].rsplit(" ", 1)[0] + "…"
            headline = TelegramFormatter._escape_md(headline)
            parts.append(f'Recent headline{src}: "{headline}"')
    ...
    def format_event(event: Event) -> str:
        ...
        # Narrative headline: same synthesis as format_event_caption
        # (ReversalEngine/PreMoveEngine/positioning/macro sentiment
        # collapsed into plain language), shown up top here since this
        # is the full-detail format — a trader should get the "why" in
        # one read before scrolling into any raw sections below.
        narrative = TelegramFormatter._build_narrative(event)
        if narrative:
            lines.append("")
    ...
    def format_event_caption(event: Event) -> str:
        ...
        honesty line from format_event, up to 3 evidence labels, and a
        one-line narrative synthesizing ReversalEngine/PreMoveEngine/
        macro sentiment into plain language (see _build_narrative).
        """
        priority_emoji = {
            EventPriority.LOW: "ℹ️",
            EventPriority.MEDIUM: "👀",
        ...
        narrative = TelegramFormatter._build_narrative(event)
        if narrative:
            # Telegram sendPhoto caption hard limit is 1024 chars total —
            # truncate defensively so a long narrative can never push the
            # whole caption over the limit and silently fail to send.
            budget = 1024 - sum(len(l) + 1 for l in lines) - 10
            if budget > 40:
                if len(narrative) > budget:
                    narrative = narrative[:budget - 1].rsplit(" ", 1)[0] + "…"
                lines.append("")
                lines.append(narrative)
    ...
    @staticmethod
    def format_validation_digest(
        stage_summaries: List["StageValidation"],
        lead_time_summaries: List["LeadTimeSummary"],
        min_trustworthy_samples: int = 15,
        displacement_summary: Optional[Dict[str, Dict[str, float]]] = None,
    ) -> Optional[str]:
        """P5 (Validation Visibility) completion: the once-daily "is
        Cryptone's PreMove signal actually working" report — the reason
        ValidationTracker.summary()/lead_time_summary() exist at all.
        Both were being computed correctly every scan already; this is
        the first time either is rendered somewhere a person actually
        reads (see the ValidationConfig.daily_digest_enabled docstring
        for why this was still missing — everything upstream of
        formatting already existed).

        Deliberately plain-language and upfront about sample size —
        this is being read while the radar is young, and a false-
        confident "83% hit rate" off 6 signals is worse than useful,
        it's actively misleading (the exact failure mode SymbolFootprint.
        describe() already guards against per-symbol; this applies the
        same posture at the stage level). Stages below
        min_trustworthy_samples get an explicit caveat rather than
        being hidden — "not enough data yet" is itself the honest
        answer for a young system, not a reason to suppress the row.

        Returns None when there's nothing resolved at all yet (radar
        too young to have any resolved outcome) — matches the "nothing
        to report" posture of build_expectation_narrative/
        symbol_footprint returning None in the equivalent case, rather
        than sending an empty/pointless digest message.
        """
        if not stage_summaries:
            return None

        # One row per stage — pick the shortest horizon with any samples,
        # same "read it the way a trader would: how's it doing so far,
        # not 4 hours later" reasoning as SymbolFootprint.symbol_footprint.
        by_stage: Dict[str, "StageValidation"] = {}
        for s in stage_summaries:
            if s.sample_count <= 0:
                continue
            existing = by_stage.get(s.stage)
            if existing is None or s.horizon_minutes < existing.horizon_minutes:
                by_stage[s.stage] = s
        if not by_stage:
            return None

        lead_by_stage = {ls.stage: ls for ls in lead_time_summaries}
        displacement_summary = displacement_summary or {}
        stage_order = ["EARLY", "BUILDING", "ACTIVATING", "CONFIRMED", "LATE"]

        lines = ["📈 *Validation Digest — is PreMove actually leading?*", ""]
        any_thin = False
        for stage in stage_order:
            sv = by_stage.get(stage)
            if sv is None:
                continue
            thin = sv.sample_count < min_trustworthy_samples
            any_thin = any_thin or thin
            hit_str = f"{sv.hit_rate * 100:.0f}%" if sv.hit_rate is not None else "n/a"
            meaningful_str = (
                f"{sv.meaningful_hit_rate * 100:.0f}%" if sv.meaningful_hit_rate is not None else "n/a"
            )
            marker = " ⚠️" if thin else ""
            lines.append(
                f"*{stage.capitalize()}*{marker} — n={sv.sample_count} "
                f"({sv.horizon_minutes}m horizon)"
            )
            lines.append(f"  hit rate: {hit_str}  ·  meaningful move: {meaningful_str}")
            lt = lead_by_stage.get(stage)
            if lt is not None and lt.resolved_count > 0:
                if lt.median_lead_time_minutes is not None:
                    lines.append(
                        f"  led {lt.led_count}/{lt.resolved_count} "
                        f"(median {lt.median_lead_time_minutes:.0f}m ahead)"
                    )
                else:
                    lines.append(f"  led 0/{lt.resolved_count} — no confirmed anticipation yet")
            # P6 (diagnostic): if this stage typically fires well after
            # the episode's price already moved, forward "led" numbers
            # have a structural ceiling — worth surfacing so a thin/bad
            # led-count doesn't get misread as noise when it's actually
            # "there was nothing left to lead."
            disp = displacement_summary.get(stage)
            if disp is not None and disp["sample_count"] > 0:
                lines.append(
                    f"  already moved {disp['median_pct']:.2f}% (median) before "
                    f"this stage fired"
                )
            lines.append("")
        ...
        if any_thin:
            lines.append(
                f"⚠️ stages under n={min_trustworthy_samples} are too thin to "
                "trust yet — shown for visibility, not as a settled read."
            )
...
class RadarZone:
    ...
    kind: str            # 'compression' | 'structural_high' | 'structural_low'
                          # | 'liquidity' | 'liquidity_tested' | 'liquidity_replenished'
                          # | 'liquidity_pulled' | 'liquidity_wall' | 'liquidity_gap'
                          # | 'invalidation' | 'event_origin'
    price_low: float
    price_high: float
    label: str
...
class RadarMarketState(Enum):
    """P1 Adaptive Resolution (war-room #4, full version): the explicit
    'what kind of moment is this' read that timeframe selection should
    hang off of, instead of an unlabeled chain of if/elif on raw event
    fields. Named after the 4 states from the war-room review:
      QUIET             — no distinguishing signal; nothing urgent or
                           slow-building in particular.
      COMPRESSION_BUILD  — a setup is forming (pre_move EARLY/BUILDING,
                           or CompressionFeature says COMPRESSED) but
                           hasn't triggered yet — the "watch this" phase.
      FAST_EXPANSION     — displacement/trigger is actually happening now
                           (pre_move ACTIVATING/CONFIRMED, or compression
                           says DISPLACED) — the setup fired.
      VERY_FAST          — a right-now, candle-by-candle exhaustion read
                           (reversal_stage set) — the tightest, most
                           reactive read Cryptone has.
    This enum is the single vocabulary; RESOLUTION_MAP below is the only
    place that translates a state into an actual timeframe/lookback, so
    the mapping can't drift between call sites the way three independent
    if/elif ladders eventually would.
    """
    QUIET = "QUIET"
    COMPRESSION_BUILD = "COMPRESSION_BUILD"
    FAST_EXPANSION = "FAST_EXPANSION"
    VERY_FAST = "VERY_FAST"
    MACRO_PERSISTENT = "MACRO_PERSISTENT"  # slow, higher-timeframe-only setup


@dataclass
class RadarResolution:
    """Output of classify_radar_resolution: which timeframe/lookback to
    read the market on, plus WHY — the 'why' is what makes this an
    explainable resolution selector rather than a silent pick, and is
    cheap to surface later (chart footer, log line) without recomputing
    anything."""
...
def classify_radar_resolution(
    ...
    """Single source of truth for 'what resolution should Cryptone look
    at this market on right now' — P1 Adaptive Resolution, full version
    (war-room #4). Reuses ONLY signals that already exist elsewhere
    (event.reversal_stage/pre_move_stage/horizon_context/
    horizon_onset_bars, plus the compression_state RadarZones already
    computes) — no new market judgment happens here, purely a routing
    decision over evidence that's already been decided.

    Priority order (most specific/urgent first — first match wins,
    mirrors the old _select_chart_timeframe ladder but now named):
      1. reversal_stage set        -> VERY_FAST      (compression tf, tightest lookback)
      2. displacement actually firing (compression_state == DISPLACED,
         or pre_move_stage in ACTIVATING/CONFIRMED)
                                    -> FAST_EXPANSION (compression tf)
      3. macro-only agreement (horizon_context PERSISTENT or
         horizon_onset_bars >= 100) -> MACRO_PERSISTENT (macro tf)
      4. setup building (pre_move_stage EARLY/BUILDING, or
         compression_state == COMPRESSED)
                                    -> COMPRESSION_BUILD (setup tf)
      5. nothing distinguishing     -> QUIET (chart_config default tf/lookback,
                                       behavior-preserving fallback)
    """
    tf = timeframe_config
    cc = chart_config

    reversal_stage = getattr(event, "reversal_stage", None) if event else None
    pre_move_stage = getattr(event, "pre_move_stage", None) if event else None
    horizon_context = getattr(event, "horizon_context", None) if event else None
    horizon_onset_bars = getattr(event, "horizon_onset_bars", None) if event else None

    if reversal_stage:
        t = tf.compression
        return RadarResolution(
            RadarMarketState.VERY_FAST, t, cc.lookback_for(t),
            reason=f"reversal_stage={reversal_stage} — right-now exhaustion read",
        )

    if compression_state == "DISPLACED" or pre_move_stage in ("ACTIVATING", "CONFIRMED"):
        t = tf.compression
        return RadarResolution(
            RadarMarketState.FAST_EXPANSION, t, cc.lookback_for(t),
            reason=f"pre_move_stage={pre_move_stage}, compression_state={compression_state} — displacement firing",
        )

    macro_persistent = bool(horizon_context) and "PERSISTENT" in horizon_context
    long_onset = horizon_onset_bars is not None and horizon_onset_bars >= 100
    if macro_persistent or long_onset:
        t = tf.macro_context
        return RadarResolution(
            RadarMarketState.MACRO_PERSISTENT, t, cc.lookback_for(t),
            reason=f"horizon_context={horizon_context}, onset_bars={horizon_onset_bars} — macro-only agreement",
        )

    if pre_move_stage in ("EARLY", "BUILDING") or compression_state == "COMPRESSED":
        t = tf.setup_context
        return RadarResolution(
            RadarMarketState.COMPRESSION_BUILD, t, cc.lookback_for(t),
            reason=f"pre_move_stage={pre_move_stage}, compression_state={compression_state} — setup building",
        )

    return RadarResolution(
        RadarMarketState.QUIET, cc.timeframe, cc.lookback_bars,
        reason="no distinguishing signal — default resolution",
    )
...
class RadarZones:
    ...
    setup_stage: Optional[str] = None       # 15M pre-move stage (EARLY/BUILDING/ACTIVATING/CONFIRMED/LATE)
...
def compute_structural_high_low(
    ...
    """Shared structural-high/structural-low definition — factored out of
    compute_radar_zones so any other caller (e.g. the text-alert scan
    loop building a TargetMapReading, which has no reason to run the
    full zone-drawing pass just to get these two numbers) uses the
    exact same confirmed-fractal-swing definition the chart's
    STRUCTURAL HIGH/LOW zones and ContextEngine's own HH/HL/LH/LL trend
    read both already use — one source of truth, not a second
    "structural" definition that could silently drift from the first.
    Falls back to highest-high/lowest-low over the window when there
    aren't yet enough confirmed swings, same as compute_radar_zones.
    """
    window = candles[-structural_lookback_bars:] if candles else []
    if not window:
        return 0.0, 0.0
    ctx_cc = context_config or ContextConfig()
    swing_highs, swing_lows = ContextEngine._find_swings(window, ctx_cc.swing_left, ctx_cc.swing_right)
    structural_high = swing_highs[-1][1] if swing_highs else max(c.high for c in window)
    structural_low = swing_lows[-1][1] if swing_lows else min(c.low for c in window)
    return structural_high, structural_low
...
def compute_radar_zones(
    ...
    # --- Structural high/low: P0 fix (war-room #2) — this used to be
    # max(high)/min(low) over the window, i.e. the highest-high /
    # lowest-low candle extreme. That's a DIFFERENT definition of
    # "structural" than what ContextEngine's own HH/HL/LH/LL trend read
    # uses (confirmed fractal swings via _find_swings), so the chart
    # could label something "STRUCTURAL HIGH" that the intelligence
    # layer wouldn't recognize as a swing at all — two silently
    # diverging sources of truth for the same word. Now both draw from
    # the identical swing detection ContextEngine.get_context() uses
    # (same _find_swings, same swing_left/right/min_swings_required),
    # so "STRUCTURAL HIGH" on the chart means exactly what it means to
    # the engine: a confirmed fractal swing high, not just the tallest
    # wick in the window. Falls back to the old highest-high/lowest-low
    # read (fail-soft, same posture as every other optional element
    # here) only when there aren't yet enough confirmed swings in the
    # window to pick from. ---
    window = candles[-cc.structural_lookback_bars:]
    ctx_cc = context_config or ContextConfig()
    structural_high, structural_low = compute_structural_high_low(
        candles, cc.structural_lookback_bars, ctx_cc,
    )
    ...
    if compression is not None:
        ...
        lc = getattr(compression, "lifecycle_state", "UNKNOWN")
        # P2 (Adaptive Compression) fix: this zone used to only draw when
        # is_compressed (the fixed 0.55-ratio gate) was True — which
        # meant CONTRACTING, the adaptive read's earliest and most
        # actionable state (percentile-low AND still actively falling,
        # not just "happens to be small right now"), was invisible on
        # every symbol whose ratio hadn't yet also crossed the universal
        # cutoff. That's backwards: CONTRACTING is exactly the case
        # adaptive_enabled exists to catch ahead of the fixed threshold,
        # per the write-up's Case A vs Case B distinction — a squeeze
        # relative to THIS symbol's own history is worth seeing before
        # it happens to also satisfy a one-size-fits-all ratio. Now the
        # zone draws on is_compressed OR lifecycle_state=='CONTRACTING',
        # so the two conditions surface independently, exactly as
        # PreMoveEngine's own CORE gate (still literal is_compressed,
        # untouched — see PreMoveEngine._generate_signal) is left
        # deciding entries on its own, unrelated axis. Visibility only;
        # nothing about what PreMove/StateMachine act on changes here.
        if compression.is_compressed or lc == "CONTRACTING":
            # P2-A: append lifecycle_state when known — tells the viewer
            # whether this is a static squeeze (COMPRESSED) or still
            # actively contracting (CONTRACTING), same distinction as
            # the write-up's Case A vs Case B. Falls back to just the
            # ratio when lifecycle_state is UNKNOWN (fresh symbol).
            lc_suffix = f" · {lc}" if lc and lc != "UNKNOWN" else ""
            result.zones.append(RadarZone(
                kind="compression", price_low=comp_low, price_high=comp_high,
                label=f"COMPRESSION {compression.compression_ratio:.2f}{lc_suffix}",
            ))
    ...
    if orderbook is not None and orderbook.bids and orderbook.asks:
        ...
        # --- Liquidity lifecycle markers (P1, war-room #15): the band
        # above is just an observed L2 snapshot — it says nothing about
        # whether that liquidity is being defended or actually consumed.
        # This draws the SAME TESTED → REPLENISHED → PULLED state
        # MicrostructureEngine._classify_liquidity_side already tracks
        # (and that the P0 sweep fix now confirms sweeps against) as an
        # explicit marker at the real bid/ask price — single source of
        # truth with the sweep logic, not a second/independent read of
        # "is this level interesting". Only drawn when there's an actual
        # open or just-resolved test (STABLE means nothing worth
        # flagging is happening on that side right now), so the chart
        # doesn't clutter every render with a marker that's usually
        # not there. ---
        if liquidity_flow:
            _LIQ_EVENT_STYLE = {
                # (zone kind, human label)
                "TESTED": ("liquidity_tested", "TESTED"),
                "REPLENISHED": ("liquidity_replenished", "DEFENDED"),
                "PULLED": ("liquidity_pulled", "PULLED"),
            }
            is_sweep = bool(liquidity_flow.get("is_sweep"))
            sweep_side = liquidity_flow.get("sweep_side")

            bid_state = liquidity_flow.get("liquidity_behavior")
            if bid_state in _LIQ_EVENT_STYLE:
                kind, tag = _LIQ_EVENT_STYLE[bid_state]
                label = f"BID LIQUIDITY {tag}"
                if bid_state == "PULLED" and is_sweep and sweep_side == "bid":
                    label += " — SWEEP CONFIRMED"
                result.zones.append(RadarZone(
                    kind=kind, price_low=best_bid, price_high=best_bid, label=label,
                ))

            ask_state = liquidity_flow.get("liquidity_behavior_ask")
            if ask_state in _LIQ_EVENT_STYLE:
                kind, tag = _LIQ_EVENT_STYLE[ask_state]
                label = f"ASK LIQUIDITY {tag}"
                if ask_state == "PULLED" and is_sweep and sweep_side == "ask":
                    label += " — SWEEP CONFIRMED"
                result.zones.append(RadarZone(
                    kind=kind, price_low=best_ask, price_high=best_ask, label=label,
                ))

        # --- Liquidity Map walls (P4): unlike the LIQUIDITY band above
        # (a fixed ±% box around mid — always the same size regardless
        # of what's actually resting in the book) and the TESTED/
        # REPLENISHED/PULLED markers above (state *changes*, not a map
        # of where size sits right now), these zones draw the REAL
        # depth-derived clusters from MicrostructureEngine.get_liquidity_map:
        # THICK = a wall (real resistance/support candidate), THIN = a
        # gap (undefended path). Capped to the nearest few of each side
        # so a noisy book doesn't turn the chart into a wallpaper of
        # boxes — only the walls/gaps actually closest to price are the
        # ones a trader reading this chart needs to see first. Purely
        # additive: liquidity_map is None or invalid (thin book, no
        # snapshot) simply means these zones are skipped, same
        # fail-soft posture as every other optional overlay here. ---
        if liquidity_map is not None and liquidity_map.is_valid:
            _MAX_WALLS_PER_SIDE = 2
            _MAX_GAPS_PER_SIDE = 1
            for side_clusters, side_name in (
                (liquidity_map.bid_clusters, "bid"), (liquidity_map.ask_clusters, "ask"),
            ):
                walls = sorted(
                    (c for c in side_clusters if c.kind == "THICK"),
                    key=lambda c: abs(c.pct_from_mid),
                )[:_MAX_WALLS_PER_SIDE]
                gaps = sorted(
                    (c for c in side_clusters if c.kind == "THIN"),
                    key=lambda c: abs(c.pct_from_mid),
                )[:_MAX_GAPS_PER_SIDE]
                for c in walls:
                    result.zones.append(RadarZone(
                        ...
                        # Chart-legibility fix: shortened from
                        # "{SIDE} WALL (0.15%, 1.8x avg)" — same two
                        # numbers (distance from mid, relative depth),
                        # just without the words around them, since the
                        # right-margin panel this renders into only has
                        # room for short labels before text starts
                        # clipping off the edge of the image. "WALL"/
                        # "GAP" plus color already carries the
                        # THICK/THIN distinction; "avg" was implied by
                        # the "x" multiplier and cost the most width for
                        # the least information.
                        label=f"{side_name.upper()} WALL {abs(c.pct_from_mid):.2f}% · {c.relative_depth:.1f}x",
                    ))
                for c in gaps:
                    result.zones.append(RadarZone(
                        kind="liquidity_gap",
                        price_low=c.price_low, price_high=c.price_high,
                        label=f"{side_name.upper()} GAP {abs(c.pct_from_mid):.2f}%",
                    ))
...
class TelegramAdapter:
    ...
    async def _send_text(self, text: str, *, _is_plain_retry: bool = False):
    ...
    async def _send_photo(self, image_bytes: bytes, caption: str, *, _is_plain_retry: bool = False) -> bool:
    ...
    async def send_validation_digest(self, text: str):
        """P5 (Validation Visibility) completion: once-daily "is PreMove
        actually leading" report — see TelegramFormatter.
        format_validation_digest. Plain text, not a photo (this is
        inherently a small table of numbers, not a visual calendar
        layout like send_calendar_digest) — sent via the same
        _send_text every other text alert in this class uses, so it
        gets the same Markdown-parse-failure retry safety net for free.
        Unlike send_calendar_digest/send_calendar_reminder, this
        doesn't return a success bool: _send_text itself doesn't expose
        one (same as format_batch_watch's call below, which is the
        closest existing precedent for a text-table digest) — the
        caller marks the digest sent-for-today unconditionally after
        calling this, matching that existing posture rather than
        inventing a new tracked-success text-send path for one caller.
        """
        await self._send_text(text)
...
class CryptoneV3:
    ...
    async def _check_validation_digest(self):
        """P5 (Validation Visibility) completion: once per scan cycle,
        send the once-daily "is PreMove actually leading" report — the
        digest ValidationConfig.daily_digest_enabled/daily_digest_hour_utc
        were already added for (see that config's docstring), mirroring
        _check_economic_calendar's exact dedup-by-UTC-date shape above.
        Fully independent of market snapshots/discovery, same posture as
        the calendar check — informational only, never blocks scan()."""
        vc = self.config.validation
        if not vc.daily_digest_enabled:
            return

        try:
            now = utc_now()
            if (
                now.hour >= vc.daily_digest_hour_utc
                and not self.validation_tracker.daily_digest_already_sent_today()
            ):
                stage_summaries = self.validation_tracker.summary()
                lead_summaries = self.validation_tracker.lead_time_summary()
                displacement_summary = self.validation_tracker.pre_stage_displacement_summary()
                text = TelegramFormatter.format_validation_digest(
                    stage_summaries, lead_summaries,
                    min_trustworthy_samples=vc.digest_min_trustworthy_samples,
                    displacement_summary=displacement_summary,
                )
                if text:
                    await self.telegram.send_validation_digest(text)
                # Marked sent either way (text present or not) — an empty
                # digest (radar too young, nothing resolved yet) shouldn't
                # retry every scan cycle until midnight, same "nothing to
                # send today" posture as the calendar check's empty-day
                # branch above.
                self.validation_tracker.mark_daily_digest_sent()
        except Exception as e:
            logger.warning(f"_check_validation_digest failed: {type(e).__name__}: {e}")
    ...
    async def scan(self):
        ...
        await self._check_validation_digest()
        ...
        for symbol in symbols_to_score:
            ...
            # Computed BEFORE the validation_tracker.record() call below so
            # the regime label at signal time can be captured on the
            # outcome (P2-B breakdown-by-regime).
