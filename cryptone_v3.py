...
class TelegramFormatter:
    ...
    def format_event(event: Event) -> str:
        ...
        # Full structured mode (per Jafar): every engine section renders
        # unconditionally now, even when the prose narrative above
        # already summarized the same field in plain language. This
        # used to be gated off (show_raw_blocks = not clauses) to avoid
        # saying the same thing twice — but that meant most of what
        # RegimeEngine/ExhaustionDetector/PriceMemoryEngine/Collaboration/
        # ExternalIntel actually compute never reached Telegram at all,
        # since a chart-attached alert almost always has narrative
        # clauses. The prose stays as the fast top-line read; the cards
        # below are the full data behind it, always visible.
        show_raw_blocks = True
    ...
    def format_event_caption(event: Event) -> str:
        ...
        if event.direction == "long":
            ...
            dir_badge = "  ⚠️ NO DIRECTION"
        lines = [f"{priority_emoji} *{event.symbol} PERP — {TelegramFormatter._humanize(event.state)}{dir_badge}*  Q{event.quality:.2f}"]
