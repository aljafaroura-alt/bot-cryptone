...
try:
    # svglib+reportlab: converts Hyperliquid's SVG coin icons to PNG for
    # the chart header logo (see CoinLogoProvider). Chosen over cairosvg
    # specifically because reportlab ships prebuilt wheels with no
    # system libcairo dependency — matters for a CI runner that can't
    # assume an apt-get step ran first. Purely optional: no logo source
    # is required for the bot's core alerting to function.
...
class MacroSentimentProvider:
    """Thin, cached wrapper around alternative.me's Fear & Greed Index.
    One HTTP call per cache window (default 30 min — the index only
    updates daily anyway, so polling faster than that just wastes a
    request), reused across every symbol's ReversalEngine.evaluate() call
    in a scan cycle rather than fetched per-symbol.
    """
    ...
    async def get_index(self) -> Optional[Tuple[int, str]]:
        ...
        try:
            ...
            self._last_error = f"malformed response: {e}"
            logger.warning(f"MacroSentimentProvider: {self._last_error}")
            return None
...
#   1. https://app.hyperliquid.xyz/coins/{symbol}.svg — Hyperliquid's own
...
class CoinLogoProvider:
    ...
    itself, via svglib+reportlab, so the renderer never has to care
    which of the three sources actually served the icon) or None.
    """

    HL_ENDPOINT = "https://app.hyperliquid.xyz/coins/{symbol}.svg"
    COINGECKO_SEARCH_ENDPOINT = "https://api.coingecko.com/api/v3/search?query={query}"
    ...
    def _svg_to_png(svg_bytes: bytes) -> Optional[bytes]:
        """svglib+reportlab rather than cairosvg deliberately: reportlab
        ships prebuilt wheels with no system libcairo dependency, which
        matters for a self-relaunching GitHub Actions runner that can't
        assume an apt-get step ran first. Returns None (never raises) on
        any malformed/unsupported SVG."""
        ...
        try:
            ...
            logger.debug(f"CoinLogoProvider: SVG->PNG decode failed: {type(e).__name__}: {e}")
            return None
    ...
    async def _fetch_coingecko_checked(self, base: str) -> Tuple[Optional[bytes], bool]:
        """Tier 2 wrapper mirroring `_fetch_checked`'s transient-vs-
        definitive distinction, since CoinGecko's free /search endpoint
        rate-limits aggressively under a 232-symbol scan loop."""
        try:
            async with self._session.get(
                self.COINGECKO_SEARCH_ENDPOINT.format(query=base),
            ...
            logger.debug(f"CoinLogoProvider: CoinGecko search failed for {base}: {type(e).__name__}: {e}")
            return None, True

        try:
            coins = payload.get("coins", []) if isinstance(payload, dict) else []
            match = next(
                (c for c in coins if str(c.get("symbol", "")).upper() == base.upper()),
                None,
            )
            if match is None:
                return None, False  # definitive: CoinGecko has no such symbol
            image_url = match.get("large") or match.get("thumb")
            if not image_url:
                return None, False
        except (AttributeError, TypeError):
            return None, False

        return await self._fetch_checked(image_url)
...
class ContextEngine:
    ...
    def _reconcile_horizons(horizons: Dict[int, Dict], ordered_horizons: Tuple[int, ...]) -> str:
        ...
        """
        available = [n for n in ordered_horizons if horizons[n]['structure'] != 'INSUFFICIENT_DATA']
        if not available:
...
class TelegramFormatter:
    ...
    def _build_narrative(event: "Event") -> Optional[str]:
        ...
        if event.macro_sentiment_value is not None and event.macro_sentiment_label:
            ...
            is_dip_or_capitulation = event.reversal_play in ("DIP_LONG", "FADE_CAPITULATION")
            is_rally_or_bounce = event.reversal_play in ("FADE_RALLY", "FADE_BOUNCE")
            if is_dip_or_capitulation and mv <= 30:
            ...
            elif is_rally_or_bounce and mv >= 70:
            ...
            elif is_dip_or_capitulation and mv >= 70:
                parts.append(f"Caveat: broader sentiment is still {event.macro_sentiment_label} ({mv}), so this dip may just be noise in an otherwise greedy market.")
            elif is_rally_or_bounce and mv <= 30:
                parts.append(f"Caveat: broader sentiment is {event.macro_sentiment_label} ({mv}), which cuts against a euphoria-fade thesis.")
        ...
        if event.news_headline:
            src = f" ({event.news_source})" if event.news_source else ""
    ...
    def format_event(event: Event) -> str:
        ...
        lines = [
            ...
            "",
            f"State:          {event.state}",
            f"Direction:      {event.direction.upper() if event.direction else 'UNDETERMINED'}",
        ...
        if event.macro_structure or event.setup_structure or event.market_context or event.horizon_context \
                or (event.resolution_state and event.resolution_state != "QUIET") or event.regime_label:
        ...
        if event.pre_move_stage and event.pre_move_direction:
            ...
            if pm_detail:
                ...
                lines.append(f"  micro:        {', '.join(humanized)}")
            lines.append("")
        ...
        if exp_n:
            ...
            lines.append(f"  origin:       {exp_n.get('origin')}")
            lines.append(f"  intent:       {exp_n.get('intent')}")
            lines.append(f"  confirmation: {exp_n.get('confirmation')}")
            lines.append(f"  expected:     {exp_n.get('expected_path')}")
    ...
    def format_batch_watch(events: list, funding_extreme_abs: float = 0.003) -> str:
        ...
        col_px = max(len("PRICE"), max(len(r[2]) + len(r[3]) for r in rows))

        # Numeric columns right-aligned (Q, PRICE) — reads like an
        # actual price ladder instead of a left-justified text dump;
        # SYM stays left-aligned since it's the row's label, not data.
        header = f"{'SYM':<{col_sym}}  {'Q':>{col_q}}  {'PRICE':>{col_px}}   FUND"
        sep = "─" * len(header)
        table_lines = [header, sep]
        for sym, q, px, chg_str, fund in rows:
            px_full = px + chg_str
            table_lines.append(f"{sym:<{col_sym}}  {q:>{col_q}}  {px_full:>{col_px}}   {fund}")
...
class ChartRenderer:
    ...
    def render_radar_chart(
        ...
        try:
            ...
            if logo_bytes:
                try:
                    ...
                    logo_ab = matplotlib.offsetbox.AnnotationBbox(
                        logo_box, (0.0, 1.145), xycoords="axes fraction",
            ...
            ax.text(
                title_x, 1.13, title, transform=ax.transAxes,
