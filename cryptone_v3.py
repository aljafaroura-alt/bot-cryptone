...
class MicrostructureEngine:
    ...
    def _calculate_book_metrics(self, symbol: str):
        ...
        liquidity_behavior = self._classify_liquidity_side(
        ...
        liquidity_behavior_ask = self._classify_liquidity_side(
        ...
        # P1 follow-up (war-room #11): genuine sweep = liquidity actually
        # CONSUMED (PULLED, survived the full test window without
        # recovering — not just a transient TESTED snapshot) on one side,
        # AND aggressive trade flow on the matching side at roughly the
        # same time. delta_pct here is the existing flow_metrics value
        # (set by trade-stream processing elsewhere in this class, not
        # recomputed here) — reading it is a cross-check against a
        # DIFFERENT data source (trades) than liquidity_behavior (book),
        # which is the whole point: book-only depth loss and trade-only
        # aggression can each be noise alone, but agreeing is a real signal.
        existing_delta_pct = self.flow_metrics[symbol].get('delta_pct', 0.0)
        is_sweep = False
        sweep_side = None
        if liquidity_behavior == 'PULLED' and existing_delta_pct <= -self.mc.sweep_aggression_delta_pct:
            is_sweep, sweep_side = True, 'bid'
        elif liquidity_behavior_ask == 'PULLED' and existing_delta_pct >= self.mc.sweep_aggression_delta_pct:
...
class CoinLogoProvider:
    ...
    async def _fetch_coingecko(self, base: str) -> Optional[bytes]:
        """Tier 2: CoinGecko's /search endpoint returns image URLs
        inline per match — no separate coin-id lookup round trip needed.
        Picks the first result whose own symbol matches `base` exactly
        (case-insensitive), since /search does fuzzy name matching too
        and an unfiltered first-result pick could grab a same-named-
        different-project logo. Returns None (not an error) if nothing
        matches or the JSON shape isn't what's expected."""
        try:
            async with self._session.get(
    ...
    async def get_logo(self, symbol: str) -> Optional[bytes]:
        ...
        svg_bytes = await self._fetch(self.HL_ENDPOINT.format(symbol=base))
        if svg_bytes:
        ...
        if data is None:
            data = await self._fetch_coingecko(base)

        # Tier 3: CoinCap, last resort.
        if data is None:
            data = await self._fetch(self.COINCAP_ENDPOINT.format(symbol=base.lower()))
            ...
            if logo_bytes:
                try:
                    ...
                    if logo_img.shape[2] == 3:
                        alpha = np.ones((h, w, 1), dtype=logo_img.dtype)
                        logo_img = np.concatenate([logo_img, alpha], axis=2)
                    logo_img = logo_img.copy()
                    logo_img[mask, 3] = 0
                    # zoom is a multiplier on the *native pixel* size of the
                    # source image, not an absolute display size — so a
                    # logo CDN that returns a higher-res asset (e.g. 200px
                    # vs another coin's 64px) rendered way too large at a
                    # fixed zoom. Normalize against native size so every
                    # logo lands at the same ~34px on-figure footprint
                    # regardless of source resolution.
                    _TARGET_LOGO_PX = 34.0
...
class CryptoneV3:
    ...
    def _select_chart_timeframe(self, event: "Event") -> Tuple[str, int]:
        """Adaptive chart timeframe (fix, not a new feature): the chart
        used to always render on config.chart.timeframe (hardcoded "5m")
        no matter what actually triggered the event, so a slow, hours-
        building positioning setup and a right-now candle-wick reversal
        both got the exact same 5-hour 5m window — too zoomed-in for the
        former, arguably fine for the latter but not chosen on purpose.

        Reuses the three timeframes Config already defines meaning for
        (TimeframeConfig.compression/setup_context/macro_context) rather
        than inventing a new one, and picks between them from data
        already sitting on the event:
          - reversal_stage set  -> this is a right-now, candle-by-candle
            exhaustion read (wicks/volume-fade on recent candles) ->
            compression tf (default 5m), tightest lookback.
          - horizon_onset_bars large (structure only agrees with macro
            after a long window) or horizon_context flags a *_PERSISTENT
            read -> this is a slow/macro setup -> macro tf (default 1h).
          - pre_move_stage set, or anything else with a real signal ->
            setup tf (default 15m), the general-purpose middle ground.
        Falls back to config.chart.timeframe/lookback_bars untouched
        when nothing distinguishing is present, so behavior for events
        with no reversal/pre_move/horizon read is unchanged.
        """
        cc = self.config.chart
        tf = self.config.timeframes

        if event.reversal_stage:
            return tf.compression, cc.lookback_bars

        macro_persistent = bool(event.horizon_context) and "PERSISTENT" in event.horizon_context
        long_onset = event.horizon_onset_bars is not None and event.horizon_onset_bars >= 100
        if macro_persistent or long_onset:
            return tf.macro_context, cc.lookback_bars

        if event.pre_move_stage:
            return tf.setup_context, cc.lookback_bars

        return cc.timeframe, cc.lookback_bars
    ...
    async def _flush_scan_events(self):
        ...
        if self.config.chart.enabled:
            for event in urgent:
                try:
                    chart_timeframe, chart_lookback = self._select_chart_timeframe(event)
